...
///
void OptionalArgsFunction(int *a0, int *a1, int *a2, int *a3, int *a4, int *a5,
  ...
  *a0 = 0;
  ...
  if (nonnull_count != number_of_nonnull_arguments) *a0 *= -1;
}
class TestModule : public TestSolver, public COM_Object {
 public:
  ...
  virtual ~TestModule() {
    ...
    if (!other_window_name.empty()) {
      ...
      int other_window_handle =
          COM_get_window_handle(other_window_name.c_str());
    ...
    if (!fortran_window_name.empty()) {
      ...
      int fortran_window_handle =
          COM_get_window_handle(fortran_window_name.c_str());
      if (fortran_window_handle > 0)
        COM_UNLOAD_MODULE_STATIC_DYNAMIC(COMFTESTMOD,
                                         fortran_window_name.c_str());
    }
  ...
  void Function2(void *string_data) {
    if (!other_window_name.empty()) {
      int other_window_handle =
          COM_get_window_handle(other_window_name.c_str());
      if (other_window_handle > 0) {
        int other_function_handle =
            COM_get_function_handle((other_window_name + ".Function0").c_str());
  ...
  void Function4(void *string_data) {
    ...
    if (!fortran_window_name.empty()) {
      int fortran_window_handle =
          COM_get_window_handle(fortran_window_name.c_str());
      if (fortran_window_handle > 0) {
        int fortran_function_handle = COM_get_function_handle(
            (fortran_window_name + ".Function0").c_str());
  ...
  void InitSolver(void *config_file) {
    ...
    if (config_file) config_name = static_cast<std::string *>(config_file);
    ...
    if (!Inf) {
      std::cerr
          << "TestModule::InitModule:Error: Could not open configuration file, "
  ...
  void ValidateDataItemAddress(void *name_of_dataitem, void *address,
                               int *result) {
    std::string *name_string = static_cast<std::string *>(name_of_dataitem);
    if (Solution().GetFieldData(*name_string).data() ==
  ...
  void Step() {
    ...
    for (int i = 0; i < nelem; i++) pressures[i] = centroids[i * 3 + 2];
  };
  ...
  void DumpSolverSolution(void *config_file, int *use_timestamp) {
    ...
    if (config_file)
      ...
      std::cerr << "TestModule::DumpSolution:Error: No filename given."
                << std::endl;
      return;
    }
    ...
    if (!Ouf) {
      std::cerr
          << "TestModule::DumpSolution:Error: Could not open output file, "
          << Ostr.str() << "." << std::endl;
      return;
    }
  ...
  /// which will be the address of the object registered to
  /// the ".global" DataItem.
  ///
  ...
  static void Load(const std::string &name) {
    std::cout << "Loading TestModule with name " << name << "." << std::endl;
    ...
    types[1] = COM_INT;
    COM_set_member_function(
        (name + ".get_communicator_module").c_str(),
    ...
    COM_set_function((name + ".IntFunction").c_str(), (Func_ptr)(&IntFunction),
                     "b", &types[0]);
    ...
    COM_set_function((name + ".OptionalArgsFunction").c_str(),
                     (Func_ptr)(&OptionalArgsFunction), "bIIIIIIIIIIII",
                     &types[0]);
  ...
  static void Unload(const std::string &name) {
    std::cout << "Unloading TestModule with name " << name << "." << std::endl;
    TestModule *test_module_pointer = NULL;
    std::string global_name(name + ".global");
    COM_get_object(global_name.c_str(), 0, &test_module_pointer);
    COM_assertion_msg(test_module_pointer->validate_object() == 0,
                      "Invalid object");
 ...
 private:
  ...
  std::string
      fortran_window_name;  /// Tracks the fortran external window's name
};
}  // namespace COM
