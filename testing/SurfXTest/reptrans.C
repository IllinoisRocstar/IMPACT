...
int main(int argc, char *argv[]) {
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    COM_new_dataitem((wnames[k] + ".fs").c_str(), 'n', COM_DOUBLE, 3, "m");
    COM_new_dataitem((wnames[k] + ".errors").c_str(), 'n', COM_DOUBLE, 3, "m");

    COM_resize_array((wnames[k] + ".fs").c_str());
    COM_resize_array((wnames[k] + ".errors").c_str());

    COM_new_dataitem((wnames[k] + ".nrms").c_str(), 'w', COM_DOUBLE, 3, "m");
  ...
  int w1_fs = COM_get_dataitem_handle((wnames[0] + ".fs").c_str());
  int w1_fs1 = COM_get_dataitem_handle((wnames[0] + ".1-fs").c_str());
  int w1_err = COM_get_dataitem_handle((wnames[0] + ".errors").c_str());
  int w1_err1 = COM_get_dataitem_handle((wnames[0] + ".1-errors").c_str());
  int w1_nrms = COM_get_dataitem_handle((wnames[0] + ".nrms").c_str());
  ...
  int w2_fs = COM_get_dataitem_handle((wnames[1] + ".fs").c_str());
  ...
  if (initial == 0) {
    ...
    COM_call_function(OUT_write, (wnames[0] + buf).c_str(), &w1_fs,
                      wnames[0].c_str(), buf);
    COM_call_function(OUT_write, (wnames[1] + buf).c_str(), &w2_fs,
                      wnames[1].c_str(), buf);
  } else {
    COM_call_function(IN_read, (wnames[0] + buf).c_str(), &w1_fs,
                      wnames[0].c_str(), buf);
    COM_call_function(IN_obtain, &w1_mesh, &w1_mesh);
    COM_call_function(IN_obtain, &w1_fs, &w1_fs);

    COM_call_function(IN_read, (wnames[1] + buf).c_str(), &w2_fs,
                      wnames[1].c_str(), buf);
