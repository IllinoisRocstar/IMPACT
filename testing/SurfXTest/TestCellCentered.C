...
void initUnstructuredMesh(std::vector<double> &coors, std::vector<int> &elmts,
                          int nrow, int ncol, int rank, int nproc, int type,
  ...
  if (type == 0) {
    for (int i = 0; i < nrow - 1; ++i) {
      for (int j = 0; j < ncol - 1; ++j) {
        ...
        coors[3 * (count + i * (ncol - 1) + j) + 0] =
            xSpacing * j + xSpacing / 2 + col * length;
        coors[3 * (count + i * (ncol - 1) + j) + 1] =
            ySpacing * i + ySpacing / 2 + row * width;
...
template <class TYPE>
void ReportDataVector(std::vector<std::vector<std::vector<TYPE> > > data,
                      int stride) {
  for (unsigned int i = 0; i < data.size(); i++) {
    std::cout << "Window " << i << ":" << std::endl;
    for (unsigned int j = 0; j < data[i].size(); j++) {
      std::cout << "Pane " << j << ":" << std::endl;
      for (unsigned int k = 0; k < data[i][j].size() / stride; k++) {
...
void ReportUnstructuredMesh(std::vector<std::vector<double> > &coordinates,
                            ...
                            int element_size, std::ostream &outStream) {
...
double area2d(std::vector<int> &elements, std::vector<double> &coords,
              int index, int meshType, std::vector<double> &centroid) {
  double area = 0;
...
TEST(SurfXTests, CellCenteredCalculationsAndOverlay) {
  int comm_rank = 0;
  int comm_size = 1;
  int npanes = 4;
  int nwindows = 3;
  ...
  if (comm_rank == 0) std::cout << "Initializing COM and SurfX" << std::endl;
  COM_init(&ARGC, &ARGV);
  ...
  /*SurfX does not support extrapolation anymore
   */

  EXPECT_NE(-1, RFC_clear)
      << "Function handle RFC.clear_overlay was not found\n";
  EXPECT_NE(-1, RFC_read) << "Function handle RFC.read_overlay was not found\n";
  EXPECT_NE(-1, RFC_write)
      << "Function handle RFC.write_overlay was not found\n";
  EXPECT_NE(-1, RFC_overlay) << "Function handle RFC.overlay was not found\n";
  EXPECT_NE(-1, RFC_transfer)
      << "Function handle RFC.least_squares_transfer was not found\n";
  EXPECT_NE(-1, RFC_interp)
      << "Function handle RFC.interpolate was not found\n";
  // EXPECT_NE(-1, RFC_extrap) << "Function handle RFC.extrapolate was not
  // found\n";
  ...
  for (int i = 0; i < nwindows; i++) {
    ...
    ASSERT_NO_THROW(makeWindow(windowNames[i], i))
        << "An error occurred when creating window: " << windowNames[i]
        << std::endl;
  }
  ...
  for (int i = 0; i < nwindows; i++) {
    ...
    for (int j = 0; j < npanes; j++) {
      ...
      initUnstructuredMesh(coords[i][j], elements[i][j], nrow, ncol, j, npanes,
                           meshType, nnodes, nelems);
      ...
      for (int k = 0; k < nelems; k++) {
        double area =
            area2d(elements[i][j], coords[i][j], k, meshType, centroid);
      ...
      EXPECT_NO_THROW(COM_set_size((windowName + ".nc"), pane_id, nnodes))
          << "An error occurred when resizing " << windowName << std::endl;
      EXPECT_NO_THROW(
          COM_set_array((windowName + ".nc"), pane_id, &coords[i][j][0]))
          << "An error occurred when setting the coordinates in " << windowName
          << std::endl;

      std::string connectivityName;
      connectivityName =
          ((meshType < 2) ? (windowName + ".:t3:") : (windowName + ".:q4:"));
      EXPECT_NO_THROW(COM_set_size(connectivityName, pane_id, nelems))
          << "An error occurred when resizing " << windowName << std::endl;
      EXPECT_NO_THROW(
          COM_set_array(connectivityName, pane_id, &elements[i][j][0]))
          << "An error occurred when setting the elements in " << windowName
          << std::endl;

      EXPECT_NO_THROW(
          COM_set_array((windowName + ".soln"), pane_id, &soln[i][j][0]))
          << "An error occurred when setting the solution in " << windowName
          << std::endl;
      EXPECT_NO_THROW(
          COM_set_array((windowName + ".comp"), pane_id, &comp[i][j][0]))
          << "An error occurred when setting the computation in " << windowName
          << std::endl;

    }  // j Loop over panes
    EXPECT_NO_THROW(COM_window_init_done(windowName.c_str()));
    if (verbLevel > 1)
  ...
  EXPECT_NE(-1, tri0_mesh) << "Dataitem handle Window0.mesh was not found\n";
  EXPECT_NE(-1, tri1_mesh) << "Dataitem handle Window1.mesh was not found\n";
  EXPECT_NE(-1, quad_mesh) << "Dataitem handle Window2.mesh was not found\n";
  ...
  if (comm_size == 1) {
    ...
    ASSERT_NO_THROW(COM_call_function(RFC_overlay, &tri0_mesh, &tri1_mesh))
        << "An error occurred when executing RFC_overlay function between tri0 "
           "and tri1 meshes\n";
    std::cout << "Overlay 01 done." << std::endl;
    ASSERT_NO_THROW(COM_call_function(RFC_write, &tri0_mesh, &tri1_mesh,
                                      "tri01", "tri10", format))
        << "An error occurred when executing RFC_write function after \n";
    ASSERT_NO_THROW(COM_call_function(RFC_clear, "Window0", "Window1"))
        << "An error occurred when executing RFC_clear function \n";

    ASSERT_NO_THROW(COM_call_function(RFC_overlay, &tri0_mesh, &quad_mesh))
        << "An error occurred when executing RFC_";
    ASSERT_NO_THROW(COM_call_function(RFC_write, &tri0_mesh, &quad_mesh,
                                      "tri02", "quad20", format))
        << "An error occurred when executing RFC_";
    ASSERT_NO_THROW(COM_call_function(RFC_clear, "Window0", "Window2"))
        << "An error occurred when executing RFC_";

    ASSERT_NO_THROW(COM_call_function(RFC_overlay, &tri1_mesh, &quad_mesh))
        << "An error occurred when executing RFC_";
    ASSERT_NO_THROW(COM_call_function(RFC_write, &tri1_mesh, &quad_mesh,
                                      "tri12", "quad21", format))
        << "An error occurred when executing RFC_";
    ASSERT_NO_THROW(COM_call_function(RFC_clear, "Window1", "Window2"))
        << "An error occurred when executing RFC_";
  }
  ...
  ////////////////////////////////////////////////////////////////
  EXPECT_NO_THROW(COM_call_function(RFC_read, &tri0_mesh, &tri1_mesh, NULL,
                                    "tri01", "tri10", format));
  /////////////////////////////////////////////////////////////////
  std::cout << "Transfer Window 1 to Window 0" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &tri1_soln, &tri0_comp))
      << "An error occurred during the overlay between two triangular meshes!"
      << std::endl;
  if (verbLevel > 1) {
  ...
  if (verbLevel > 1) std::cout << "check_sums: (sums comparison)" << std::endl;
  for (int j = 0; j < npanes; j++) {
    check_sum = 0.0;
    for (unsigned int k = 0; k < comp[check_to][j].size(); k++) {
      double area = area2d(elements[check_to][j], coords[check_to][j], k,
                           check_to, centroid);
      check_sum += area * comp[check_to][j][k];
    }
    double diff = fabs(check_sum - sums[check_from][j]);
    if (verbLevel > 1) {
      std::cout << std::setprecision(12) << check_sum << " ("
                << sums[check_from][j] << ")" << std::endl;
      std::cout << "diff =  " << diff << std::endl;
    }
    EXPECT_LT(diff, tolerance)
        << "The computed value differs from the solution by more than "
        << tolerance << std::endl;
    if (diff > tolerance) pass = false;
  }
  std::cout << "Cell centered solution from Window " << check_from
            << " to Window " << check_to << " " << (pass ? "passed" : "failed")
            << "." << std::endl;
  /////////////////////////////////////////////////////////////////
  std::cout << "Transfer Window 0 to Window 1" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &tri0_soln, &tri1_comp))
      << "An error occurred during the least squares transfer!" << std::endl;
  if (verbLevel > 1) {
    std::cout << "comp: " << std::setprecision(12) << std::endl;
    ReportDataVector(comp, 1);
  }
  ...
  std::cout << "Cell centered solution from Window " << check_from
            ...
            << "." << std::endl;
  ////////////////////////////////////////////////////////////////
  EXPECT_NO_THROW(COM_call_function(RFC_clear, "Window0", "Window1"))
      << "An error occurred when clearing the overlay data!" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_read, &tri0_mesh, &quad_mesh, NULL,
                                    "tri02", "quad20", format))
      << "An error occured when reading the overlay data!" << std::endl;
  /////////////////////////////////////////////////////////////////
  std::cout << "Transfer Window 0 to Window 2" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &tri0_soln, &quad_comp))
      << "An error occured when transfering overlay data!" << std::endl;
  if (verbLevel > 1) {
    std::cout << "comp: " << std::setprecision(12) << std::endl;
    ReportDataVector(comp, 1);
  }
  ...
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &quad_soln, &tri0_comp))
      << "An error occured when transferring data between windows!"
      << std::endl;
  if (verbLevel > 1) {
    std::cout << "comp: " << std::setprecision(12) << std::endl;
    ReportDataVector(comp, 1);
  ...
  std::cout << "Cell centered solution from Window " << check_from
            ...
            << "." << std::endl;
  /////////////////////////////////////////////////////////////////
  EXPECT_NO_THROW(COM_call_function(RFC_clear, "Window0", "Window2"))
      << "An error occurred when clearing the overlay data!" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_read, &tri1_mesh, &quad_mesh, NULL,
                                    "tri12", "quad21", format))
      << "An error occurred when reading the overlay data!" << std::endl;
  /////////////////////////////////////////////////////////////////
  std::cout << "Transfer Window 1 to Window 2" << std::endl;
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &tri1_soln, &quad_comp))
      << "An error occurred when transferring the overlay data!" << std::endl;
  if (verbLevel > 1) {
    std::cout << "comp: " << std::setprecision(12) << std::endl;
    ReportDataVector(comp, 1);
  }
  ...
  EXPECT_NO_THROW(COM_call_function(RFC_transfer, &quad_soln, &tri1_comp))
      << "An error occurred when transferring overlay data!" << std::endl;
  if (verbLevel > 1) {
    std::cout << "comp: " << std::setprecision(12) << std::endl;
    ReportDataVector(comp, 1);
  }
  ...
  std::cout << "Cell centered solution from Window " << check_from
            ...
            << "." << std::endl;
  /////////////////////////////////////////////////////////////////
  EXPECT_NO_THROW(COM_call_function(RFC_clear, "Window1", "Window2"))
      << "An error occurred when clearing the overlay data!" << std::endl;
  /////////////////////////////////////////////////////////////////
