...
TEST(SurfXTests, VariousManualMeshGenerationDataTransfer) {
  int comm_rank = 0;
  int comm_size = 1;
  int npanes = 4;
  int nwindows = 3;
  ...
  ASSERT_NE(-1, RFC_clear)
      << "An error occurred when finding the RFC.clear_overlay function"
      << std::endl;
  ASSERT_NE(-1, RFC_read)
      << "An error occurred when finding the RFC.read_overlay function"
      << std::endl;
  ASSERT_NE(-1, RFC_write)
      << "An error occurred when finding the RFC.write_overlay function"
      << std::endl;
  ASSERT_NE(-1, RFC_overlay)
      << "An error occurred when finding the RFC.overlay function" << std::endl;
  ASSERT_NE(-1, RFC_transfer) << "An error occurred when finding the "
                                 "RFC.least_squares_transfer function"
                              << std::endl;
  ASSERT_NE(-1, RFC_interp)
      << "An error occurred when finding the RFC.interpolate function"
      << std::endl;
  // ASSERT_NE(-1, RFC_extrap) << "An error occurred when finding the
  // RFC.extrapolate function" << std::endl;
  ...
  for (int i = 0; i < nwindows; i++) {
    ...
    for (int j = 0; j < npanes; j++) {
      ...
      for (unsigned int k = 0; k < soln[i][j].size(); k++) {
      ...
      ASSERT_NO_THROW(COM_set_size((windowName + ".nc"), pane_id, nnodes));
      ASSERT_NO_THROW(
      ...
      ASSERT_NO_THROW(COM_set_size(connectivityName, pane_id, nelems));
      ASSERT_NO_THROW(
    ...
    ASSERT_NO_THROW(COM_window_init_done(windowName.c_str()));
  ...
  COM_call_function(RFC_read, &tri0_mesh, &tri1_mesh, NULL, "tri01", "tri10",
                    format);

  COM_call_function(RFC_transfer, &tri1_soln, &tri0_comp);
  int check_id = 0;
  std::vector<std::vector<double> >::iterator paneIt = comp[check_id].begin();
  std::vector<std::vector<double> >::iterator paneIt2 =
      coords[check_id].begin();
  bool pass = true;
  double compTol = 1e-5;
  while (paneIt != comp[check_id].end()) {
    ...
    while (pcIt != paneComp.end()) {
      ...
      EXPECT_LT(solnDiff, compTol)
          << *pcIt << " != " << *pcIt2 << " (" << solnDiff << ")" << std::endl;
      if (solnDiff > compTol) {
        pass = false;
      }
      *pcIt++ = -1;
  ...
  ASSERT_NO_THROW(COM_call_function(RFC_transfer, &tri0_soln, &tri1_comp));
  check_id = 1;
  ...
  ASSERT_NO_THROW(COM_call_function(RFC_read, &tri0_mesh, &quad_mesh, NULL,
                                    "tri02", "quad20", format));
  ASSERT_NO_THROW(COM_call_function(RFC_transfer, &tri0_soln, &quad_comp));
  check_id = 2;
  paneIt = comp[check_id].begin();
  paneIt2 = coords[check_id].begin();
  pass = true;
  ...
  ASSERT_NO_THROW(COM_call_function(RFC_transfer, &quad_soln, &tri0_comp));
  check_id = 0;
  paneIt = comp[check_id].begin();
  paneIt2 = coords[check_id].begin();
  pass = true;
  ...
  ASSERT_NO_THROW(COM_call_function(RFC_read, &tri1_mesh, &quad_mesh, NULL,
                                    "tri12", "quad21", format));
  ASSERT_NO_THROW(COM_call_function(RFC_transfer, &tri1_soln, &quad_comp));
  check_id = 2;
  paneIt = comp[check_id].begin();
  paneIt2 = coords[check_id].begin();
  pass = true;
  ...
  ASSERT_NO_THROW(COM_call_function(RFC_transfer, &quad_soln, &tri1_comp));
  check_id = 1;
  paneIt = comp[check_id].begin();
  paneIt2 = coords[check_id].begin();
  pass = true;
