...
void read_file(const char *fname, const std::string &wname, double alpha) {
  ...
  std::cout << "Obtained window " << wname << " from file " << fname
            << std::endl;
...
TEST(SurfXTest, RFC) {
  ...
  if (ARGC < 3) {
    std::cerr << "Usage: " << ARGV[0]
              << " <HDF|RocinControlFile1> <HDF|RocinControlFile2> "
                 "[<out_prefix>] [<RocfaceControlFile>]\n\n"
              << "\t<HDF|RocinControl File1> specifies the files for the first "
                 "window\n"
              << "\t<HDF|RocinControl File2> specifies the files for the "
                 "second window\n"
              << "\t<out_prefix> specifies a prefix for output files. "
                 "\n\t\tDefault is the current directory\n"
              << "\t<RocfaceControlFile> specifies a file name for Rocface "
                 "control parameters. \n"
              << "\nExample:\t" << ARGV[0]
              << " \"ifluid*.hdf\" \"isolid*.hdf\" "
              << "\n\t\t" << ARGV[0]
              << " Rocflo/Rocin/\"ifluid*.txt\" Rocfrac/Rocin/\"isolid*.txt\" "
                 "Rocman/RocfloRocfrac/"
              << "\n\t\t" << std::endl;
    exit(-1);
  }
  ...
  std::string fnames[2] = {std::string(ARGV[1]), std::string(ARGV[2])};
  const std::string wnames[2] = {std::string("solid"), std::string("fluid")};
  ...
  int w1_mesh = COM_get_dataitem_handle((wnames[0] + ".mesh").c_str());
  int w2_mesh = COM_get_dataitem_handle((wnames[1] + ".mesh").c_str());
  ...
  int w1_nc = COM_get_dataitem_handle((wnames[0] + ".nc").c_str());
  int w1_flux = COM_get_dataitem_handle((wnames[0] + ".flux").c_str());
  int w1_velo = COM_get_dataitem_handle((wnames[0] + ".velo").c_str());

  int w2_nc = COM_get_dataitem_handle((wnames[1] + ".nc").c_str());
  int w2_flux = COM_get_dataitem_handle((wnames[1] + ".flux").c_str());
  int w2_velo = COM_get_dataitem_handle((wnames[1] + ".velo").c_str());
  ...
  if (initial == 0) {
    ...
    COM_call_function(OUT_write, (wnames[0] + buf).c_str(), &w1_flux,
                      wnames[0].c_str(), buf);
    COM_call_function(OUT_write, (wnames[1] + buf).c_str(), &w2_flux,
                      wnames[1].c_str(), buf);
  } else {
    int IN_read = COM_get_function_handle("IN.read_window");
    int IN_obtain = COM_get_function_handle("IN.obtain_dataitem");

    COM_call_function(IN_read, (wnames[0] + buf).c_str(), &w1_flux,
                      wnames[0].c_str(), buf);
    COM_call_function(IN_obtain, &w1_mesh, &w1_mesh);
    COM_call_function(IN_obtain, &w1_flux, &w1_flux);

    COM_call_function(IN_read, (wnames[1] + buf).c_str(), &w2_flux,
                      wnames[1].c_str(), buf);
