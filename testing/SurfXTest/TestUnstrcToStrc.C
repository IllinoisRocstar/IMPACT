...
TEST(SurfXTests, XTest1) {
  ...
  ASSERT_NO_THROW(COM_new_dataitem("unstr.velo", 'n', COM_DOUBLE, 3, "m/s"));
  ASSERT_NO_THROW(COM_new_dataitem("unstr.disp", 'n', COM_DOUBLE, 3, "m"));
  ASSERT_NO_THROW(COM_new_dataitem("unstr.burnr", 'e', COM_DOUBLE, 3, "m/s"));

  for (int pid = first_pane_id - 1; pid < first_pane_id - 1 + npanes; ++pid) {
    init_unstructure_mesh(coors_s[pid], elmts[pid], nrow, ncol, pid,
                          total_npanes);
    ...
    ASSERT_NO_THROW(COM_set_array("unstr.velo", pid + 1, &velcts_s[pid][0][0]));
    ASSERT_NO_THROW(COM_set_array("unstr.disp", pid + 1, &disps_s[pid][0][0]));
  ...
  ASSERT_NO_THROW(COM_new_dataitem("str.velo", 'n', COM_DOUBLE, 3, "m/s"));
  ASSERT_NO_THROW(COM_new_dataitem("str.disp", 'n', COM_DOUBLE, 3, "m"));
  ASSERT_NO_THROW(COM_new_dataitem("str.burnr", 'e', COM_DOUBLE, 3, "m/s"));
  ...
  for (int pid = first_pane_id - 1; pid < first_pane_id - 1 + npanes; ++pid) {
    ...
    ASSERT_NO_THROW(COM_set_array("str.velo", pid + 1, &velcts_f[pid][0][0]));
    ASSERT_NO_THROW(COM_set_array("str.disp", pid + 1, &disps_f[pid][0][0]));
  ...
  int unstr_mesh = COM_get_dataitem_handle("unstr.mesh");
  int RFC_clear = COM_get_function_handle("RFC.clear_overlay");
  ASSERT_NE(-1, str_mesh) << "Dataitem str.mesh was not found!\n";
  ASSERT_NE(-1, unstr_mesh) << "Dataitem unstr.mesh was not found!\n";
  ASSERT_NE(-1, RFC_clear) << "Dataitem RFC.clear_overlay was not found!\n";
  ...
  ASSERT_NE(-1, RFC_write) << "Function RFC.write_overlay was not found!\n";
  if (comm_size == 1) {  // Perform mesh overlay.
    int RFC_overlay = COM_get_function_handle("RFC.overlay");
    ASSERT_NE(-1, RFC_overlay) << "Function RFC.overlay was not found!\n";

    std::cout << "Overlaying meshes..." << std::endl;
    ASSERT_NO_THROW(COM_call_function(RFC_overlay, &str_mesh, &unstr_mesh));
    ASSERT_NO_THROW(COM_call_function(RFC_write, &str_mesh, &unstr_mesh, "str",
                                      "unstr", format));
    ASSERT_NO_THROW(COM_call_function(RFC_clear, "str", "unstr"));
  }

  int RFC_read = COM_get_function_handle("RFC.read_overlay");
  ASSERT_NE(-1, RFC_read) << "Function RFC.read_overlay was not found!\n";
  ASSERT_NO_THROW(COM_call_function(RFC_read, &str_mesh, &unstr_mesh, NULL,
                                    "str", "unstr", format));
  ...
  if (ARGC > 1) {
    if (comm_rank == 0) std::cout << "Perform data transfer..." << std::endl;
    std::sprintf(prefix1, "SurfXTest1str_dispResults%d", comm_rank);
    std::sprintf(prefix2, "SurfXTest1Unstr_dispResults%d", comm_rank);
    ...
    int str_disp = COM_get_dataitem_handle("str.disp");
    int unstr_disp = COM_get_dataitem_handle("unstr.disp");
    ASSERT_NE(-1, OUT_write) << "Function OUT.write_dataitem was not found!\n";
    ASSERT_NE(-1, OUT_set) << "Function OUT_set was not found!\n";
    ASSERT_NE(-1, str_disp) << "Dataitem str_disp was not found!\n";
    ASSERT_NE(-1, unstr_disp) << "Dataitem unstr.disp was not found!\n";
    ...
    ASSERT_NE(-1, RFC_transfer)
        << "Function RFC.least_squares_transfer was not found!\n";
    ASSERT_NO_THROW(COM_call_function(RFC_transfer, &str_disp, &unstr_disp));
    ASSERT_NO_THROW(COM_call_function(RFC_transfer, &unstr_disp, &str_disp));

    ASSERT_NO_THROW(COM_call_function(OUT_set, "mode", "a"));
    ASSERT_NO_THROW(
