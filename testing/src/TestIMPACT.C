...
namespace IMPACT{
  ...
  int Test(int argc,char *argv[])
  {
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(verblevel > 2)
        std::cout << "IMPACT::Test: Exiting test function (success)" << std::endl;
      return(0);
    }
    if(clerr){
      ...
      if(verblevel > 2)
        std::cout << "IMPACT::Test: Exiting test function (fail)" << std::endl;
      return(1);
    }
    ...
    if(!OutFileName.empty()){
      ...
      if(!Ouf){
        std::cout << "IMPACT::Test> Error: Could not open output file, " 
    ...
    if(verblevel > 2)
      std::cout << "IMPACT::Test: Entering test function" << std::endl;
    
    // Make an instance of the IMPACT testing object, IMPACT::TestingObject
    IMPACT::TestingObject<IMPACT::TestResults> test;
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        std::cout << "IMPACT::Test> Error: Could not open list of tests in file " 
    ...
    *Out << results << std::endl;
    ...
    if(verblevel > 2)
      *Out << "IMPACT::Test: Exiting test function (success)" << std::endl;
