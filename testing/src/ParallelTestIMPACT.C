...
/// @brief Implements a command-line interface for the IMPACT tests.
///
...
namespace IMPACT{
  ...
  ///
  /// Drives the IMPACT::TestObject. 
  /// 
  /// @param argc number of string command line tokens
  /// @param argv string command line tokens
  /// @returns 0 if successful, 1 otherwise
  ///
  /// Drives the IMPACT::TestObject, which should encapsulate
  /// all the tests for the IMPACT namespace (and thus the project).
  /// 
  ...
  int ParallelTest(int argc,char *argv[])
  {
    ...
    IMPACT::CommType communicator(&argc,&argv);
    int rank  = communicator.Rank();
    int nproc = communicator.Size();
    bool do_stdout = !rank;

    // This line creates the IMPACT::TestComLine object and passes in 
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(do_stdout){
        ...
        if(verblevel > 1)
          std::cout << "IMPACT::ParallelTest: Exiting test function (success)" 
                    << std::endl;
      }
    ...
    if(clerr){
      if(do_stdout){
        ...
        if(verblevel > 2)
          std::cout << "IMPACT::ParallelTest: Exiting test function (fail)" << std::endl;
      }
      communicator.SetExit(1);
    }
    if(communicator.Check())
    ...
    if(!OutFileName.empty()){
      if(Out){
        ...
        if(!Ouf){
          std::cout << "IMPACT::ParallelTest> Error: Could not open output file, " 
    ...
    if(verblevel > 1 && Out)
      *Out << "IMPACT::ParallelTest: Entering test function" << std::endl;
    
    // Make an instance of the IMPACT testing object, IMPACT::ParallelTestingObject
    IMPACT::ParallelTestingObject<IMPACT::CommType,IMPACT::TestResults> test(communicator);
    // Make an instance of the IMPACT results object, IMPACT::TestResults
    IMPACT::TestResults results;
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        if(Out)
          *Out << "IMPACT::ParallelTest> Error: Could not open list of tests in file " 
      ...
      // This call runs all the tests for the IMPACT namespace.
    ...
    if((verblevel > 1) && Out)
      *Out << "IMPACT::ParallelTest: Exiting test function (success)" << std::endl;
...
{
  return(IMPACT::ParallelTest(argc,argv));
}
