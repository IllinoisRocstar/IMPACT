...
#define LOAD_MODULE_WRAPPER( mixed, lower, UPPER)			\
  ...
  void COM_new_dataitem( const char *wa_str,
			  const char loc,
			  const int  type,
			  int  ncomp,
			  const char *unit);
  ...
  void COM_use_dataitem( const char *wname, const char *attr, 
			  int with_ghost, const char *ptnname, int val);

  /** Clone the subset of panes of another window 
   *  of which the given pane dataitem has value val. */
  void COM_clone_dataitem( const char *wname, const char *attr, 
			    int with_ghost, const char *ptnname, int val);

  /** Copy an dataitem onto another. */
  void COM_copy_dataitem( const char *wname, const char *attr, 
			   int with_ghost, const char *ptnname, int val);

  void COM_copy_dataitem_handles( int trg_hdl, int src_hdl, 
