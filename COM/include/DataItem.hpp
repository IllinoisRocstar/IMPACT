...
class DataItem {
 ...
 public:
   ...
   */
  DataItem(Pane *pane, const std::string &name, int id, Shorter_size loc,
           int type, const int ncomp, const std::string &unit)
      : _pane(pane),
        _parent(NULL),
        _name(name),
        _id(id),
   ...
   *  \param id dataitem index.
   */
  DataItem(Pane *pane, DataItem *parent, const std::string &name, int id);
  ...
  void copy_array(void *buf, int strd, int nitem, int offset = 0,
 ...
 protected:
  ...
  /// Inherit from parent. If depth>0, then the procedure is for the
  /// subcomponents.
  void inherit(DataItem *a, bool clone, bool withghost, int depth = 0);
