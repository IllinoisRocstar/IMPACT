...
class Pane {
 public:
  ...
  Size size_of_nodes() const { return _attr_set[COM_NC]->size_of_items(); }
  ...
  Size size_of_elements() const { return _attr_set[COM_CONN]->size_of_items(); }

  /// Get the maximum number of elements allowed in the pane (including ghost
  /// elements)
  ...
  /// Get the maximum number of real elements allowed in the pane (excluding
  /// ghost elements).
  ...
  int *pane_connectivity() { return (int *)_attr_set[COM_PCONN]->pointer(); }
  ...
  Size size_i() const { return !is_structured() ? 0 : _cnct_set[0]->size_i(); }
  /// Get the number of nodes in j-dimension if the mesh is structured.
  Size size_j() const { return !is_structured() ? 0 : _cnct_set[0]->size_j(); }
  /// Get the number of nodes in k-dimension if the mesh is structured.
  Size size_k() const { return !is_structured() ? 0 : _cnct_set[0]->size_k(); }
  //\}
  ...
  void connectivities(std::vector<Connectivity *> &es) { es = _cnct_set; }

  void connectivities(std::vector<const Connectivity *> &es) const {
    const_cast<Pane *>(this)->connectivities((std::vector<Connectivity *> &)es);
  }
 ...
 protected:  // The following functions to be called by the Window class.
  ...
  DataItem *new_dataitem(const std::string &aname, int aid, const char loc,
                         const int type, int ncomp, const std::string &unit);

  /** Insert an dataitem onto the pane */
  void insert(DataItem *attr);

  /// Delete an existing dataitem with given id.
  void delete_dataitem(int id);

  void reinit_dataitem(int aid, OP_Init op, void **addr, int strd, int cap);
  ...
  Connectivity *connectivity(const std::string &a, bool insert = false);

  void reinit_conn(Connectivity *con, OP_Init op, int **addr, int strd,
                   int cap);

  /// Inherit an dataitem from another pane onto the current pane:
  DataItem *inherit(DataItem *from, const std::string &aname, int mode,
                    bool withghost);

  /// Set the size of an dataitem
  void set_size(DataItem *a, int nitems, int ng);
