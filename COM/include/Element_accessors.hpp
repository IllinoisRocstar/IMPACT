...
class Element_node_enumerator {
 public:
  ...
  int type() const {
    if (_conn)
      return _conn->element_type();
    else if (_pane->dimension() == 2)
      return Connectivity::ST2;
    else
      return Connectivity::ST3;
  }

  /// Check whether the element is quadratic.
  int is_quadratic() const {
    if (_conn)
      return _conn->is_quadratic();
    else
      return false;
  }
...
class Element_node_vectors_k_const {
 public:
  ...
  void set(const DataItem *a, Element_node_enumerator &ene) {
...
class Element_node_vectors_k : public Element_node_vectors_k_const<Value> {
 ...
 public:
  ...
  void set(DataItem *a, Element_node_enumerator &ene) { Base::set(a, ene); }
