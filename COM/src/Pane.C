...
DataItem *Pane::dataitem(const std::string &a) {
...
DataItem *Pane::new_dataitem(const std::string &aname, int aid, const char loc,
...

{
  ...
  DataItem *a = new (atts) DataItem(this, aname, aid, loc, type, ncomp, unit);
  insert(a);
  ...
  if (ncomp > 1)
    for (int i = 1; i <= ncomp; ++i) {
      ...
      DataItem *ai =
          new (&atts[i]) DataItem(this, newname, aid + i, loc, type, 1, unit);
      insert(ai);
    }

  if (_id && loc == 'w') {  // Inherit window dataitem
    reinterpret_cast<DataItem_friend *>(a)->inherit(_window->dataitem(aid),
                                                    INHERIT_USE, true);
  }
  return a;
}

void Pane::insert(DataItem *attr) {
...
void Pane::delete_dataitem(int id) {
...
void Pane::reinit_dataitem(int aid, OP_Init op, void **addr, int strd,
                           int cap) {
  switch (aid) {
    case COM_CONN: {
  ...
  if (!a->is_windowed() && !_id) {
    if (op == OP_SET || op == OP_SET_CONST)
      throw COM_exception(COM_ERR_PANE_NOTEXIST,
                          append_frame(_window->name(), Pane::reinit_dataitem));
    else
      return;
  }
...
DataItem *Pane::inherit(DataItem *from, const std::string &aname, int mode,
                        bool withghost) {
  if (from == NULL)
    throw COM_exception(
  ...
  switch (from->id()) {  // Process keywords
    ...
    case COM_CONN: {
      ...
      if (mode != INHERIT_COPY) {
        ...
        if (mode == INHERIT_USE) {
          ((DataItem_friend *)_attr_set[COM_CONN])
  ...
  if (mode != INHERIT_COPY) {
    if (_id == 0) {
      ...
      CATCHEXP_APPEND(Pane::inherit)
      CATCHBADALLOC_APPEND(Pane::inherit);
  ...
  COM_assertion_msg(from->location() == a->location(),
                    (std::string("Location of dataitems ") + from->fullname() +
    ...
    for (int j = (s_nc > 1), nj = s_nc - (s_nc == 1); j <= nj; ++j) {
      ...
      if (dest_data->pointer() && src_data->pointer()) {
        COM_assertion_msg(
            ...
            (std::string("Number of items of dataitems ") + from->fullname() +
             " and " + a->fullname() + " do not match during copying.")
                .c_str());
        try {
...
void Pane::set_size(DataItem *a, int nitems, int ng) {
  if (nitems < ng)
    throw COM_exception(COM_ERR_INVALID_SIZE,
  ...
  if (a->is_nodal()) {
    COM_assertion_msg(a->id() == COM_NC,
                      (std::string("Cannot set size for nodal dataitem ") +
                       a->fullname() + ". Must use the nc dataitem")
                          .c_str());
    ...
    COM_assertion_msg(a->id() == COM_CONN,
                      (std::string("Cannot set size for elemental dataitem ") +
                       a->fullname() + ". Must use the conn dataitem")
                          .c_str());
