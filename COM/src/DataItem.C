...
const char *DataItem::_keywords[COM_NUM_KEYWORDS] = {
    ...
    "ridges", "1-ridges", "2-ridges", "pmesh", "data", "all"};

/// The locations of keywords.
const char DataItem::_keylocs[] = {'n', 'n', 'n', 'n', 'e', 'p', 'p',
                                   'p', 'p', 'p', 'p', 'p', 'p'};
...
const COM_Type DataItem::_keysizes[] = {3, 1, 1, 1, 0, 0, 1, 2, 1, 1, 0, 0, 0};

//\}

DataItem::DataItem(Pane *pane, DataItem *parent, const std::string &name,
                   int id)
...
const void *DataItem::get_addr(int i, int j) const {
  ...
  if (i < 0 || i >= size_of_items())
    throw COM_exception(COM_ERR_INDEX_OUT_OF_BOUNDS,
                        append_frame(fullname(), DataItem::get_addr));
...
void DataItem::set_size(int nitems, int ngitems) {
  if (_parent)
...
void DataItem::set_pointer(void *p, int strd, int cap, int offset,
                           bool is_const) {
...
void DataItem::copy_array(void *buf, int strd, int n, int offset,
                          int direction) {
...
void DataItem::append_array(const void *from, int strd, int nitem) {
...
void *DataItem::allocate(int strd, int cap, bool force) {
  int nitems = size_of_items(), ncomp = size_of_components();
  ...
  CATCHEXP_APPEND(DataItem::allocate)
  CATCHBADALLOC_APPEND(DataItem::allocate);
  return _ptr;
}

void DataItem::inherit(DataItem *parent, bool clone, bool withghost,
                       int depth) {
  DataItem *root = parent->root();
  _loc = root->_loc;
  _ncomp = root->_ncomp;
...
bool DataItem::compatible_types(COM_Type t1, COM_Type t2) {
  ...
  if (DataItem::get_sizeof(t1, 1) != DataItem::get_sizeof(t2, 1)) return false;
