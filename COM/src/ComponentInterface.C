...
void ComponentInterface::set_function(const std::string &fname, Func_ptr func,
                                      const std::string &intents,
                                      const COM_Type *types, DataItem *a,
                                      bool is_f90) {
  if (_func_map.find(fname) != _func_map.end())
    throw COM_exception(
        COM_WARN_DUP_FUNC,
        append_frame(_name + "." + fname, ComponentInterface::init_function));
...
DataItem *ComponentInterface::new_dataitem(const std::string &aname,
                                           const char loc, const int type,
                                           int ncomp, const std::string &unit) {
  // Special handing for connectivity
  if (Connectivity::is_element_name(aname) || DataItem::is_digit(aname[0]))
    throw COM_exception(
        COM_ERR_INVALID_DATAITEM_NAME,
        append_frame(_name + "." + aname, ComponentInterface::new_dataitem));
  if (ncomp <= 0)
    throw COM_exception(
...
void ComponentInterface::delete_dataitem(const std::string &aname) {
  Attr_map::iterator it = _attr_map.find(aname);
  if (it == _attr_map.end())
    throw COM_exception(
        COM_ERR_DATAITEM_NOTEXIST,
        append_frame(_name + "." + aname, ComponentInterface::delete_dataitem));
...
void ComponentInterface::set_size(const std::string &aname, int pid, int nitems,
                                  int ng) {
  if (Connectivity::is_element_name(aname)) {
    ...
    if (a == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(_name + "." + aname, ComponentInterface::set_size));
    ...
    if (a->is_windowed())
      ...
      for (Pane_map::iterator it = _pane_map.begin(), iend = _pane_map.end();
        ...
        ((Pane_friend *)it->second)
            ->set_size(it->second->dataitem(a->id()), nitems, ng);
      }
    } else
...

{
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::set_array));
    ...
    reinit_dataitem(a, is_const ? Pane::OP_SET_CONST : Pane::OP_SET, &addr,
                    strd, cap);
  }
}

void ComponentInterface::alloc_array(const std::string &aname,
                                     const int pane_id, void **addr, int strd,
                                     int cap) {
  if (Connectivity::is_element_name(aname)) {
    Pane &pn = pane(pane_id, true);
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::alloc_array));
...
void ComponentInterface::resize_array(const std::string &aname,
                                      const int pane_id, void **addr, int strd,
                                      int cap) {
  if (Connectivity::is_element_name(aname)) {
    Pane &pn = pane(pane_id, true);
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::resize_array));
...
void ComponentInterface::append_array(const std::string &aname,
                                      const int pane_id, const void *val,
  ...
  if (a == NULL)
    throw COM_exception(
        COM_ERR_DATAITEM_NOTEXIST,
        append_frame(name() + "." + aname, ComponentInterface::append_array));
  try {
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(COM_ERR_DATAITEM_NOTEXIST,
                          append_frame(name() + "." + aname,
                                       ComponentInterface::dealloc_array));
...

{
  ...
  if (with_conn && aname.size() && from->name() != aname)
    throw COM_exception(COM_ERR_INCOMPATIBLE_DATAITEMS,
                        append_frame(from->fullname() + " and " + aname,
                                     ComponentInterface::inherit));

  if (cond && cond->window() != from->window()) {
    throw COM_exception(
        COM_ERR_INCOMPATIBLE_DATAITEMS,
  ...
  for (Pane_map::iterator ppit = ps.begin(); ppit != ps.end(); ++ppit) {
    ...
    if ((!cond && (val == 0 || val == ppn->id())) || (cnd && *cnd == val)) {
      ...
      if (pn) pn->inherit(ppn->dataitem(from->id()), aname, mode, withghost);
    }
  }
  return a;
}

void ComponentInterface::init_done(bool pane_changed) {
  ...
  if (!pane_changed) {
    if (npanes > int(_pane_map.size())) {
      throw COM_exception(COM_ERR_INIT_DONE_PANEMAP,
                          append_frame(_name, ComponentInterface::init_done));
    }
    return;
  }
...
DataItem *ComponentInterface::get_dataitem(const std::string &aname, char *loc,
                                           int *type, int *ncomp,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (it == _attr_map.end())
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(_name + "." + aname, ComponentInterface::get_dataitem));
...

{
  if (pid == 0 && a->location() != 'w')
    throw COM_exception(
        COM_ERR_NOT_A_WINDOW_DATAITEM,
        append_frame(a->fullname(), ComponentInterface::get_size));

  if (nitem) *nitem = a->size_of_items();
  if (ng) *ng = a->size_of_ghost_items();
}

void ComponentInterface::get_size(const std::string &aname, int pid, int *nitem,
                                  int *ng) const {
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (conn == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::get_size));
...
int ComponentInterface::get_status(const std::string &aname, int pid) const {
...
void ComponentInterface::get_parent(const std::string &aname, int pid,
  ...
  try {
    ...
    if (Connectivity::is_element_name(aname)) {
      ...
      if (conn == NULL)
        throw COM_exception(
            COM_ERR_DATAITEM_NOTEXIST,
            append_frame(name() + "." + aname, ComponentInterface::get_parent));
...
void get_array_common(const Attr *a, int pid,
                      ComponentInterface::Pointer_descriptor &addr, int *strd,
                      int *cap, bool is_const) {
  if (!is_const && a->is_const())
    throw COM_exception(
        COM_ERR_DATAITEM_CONST,
        append_frame(a->fullname(), ComponentInterface::get_array));
...
void ComponentInterface::get_array(const std::string &aname, const int pane_id,
                                   Pointer_descriptor &addr, int *strd,
                                   int *cap, bool is_const) {
  Pane_friend *pn;
  try {
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::get_array));
...
inline void copy_array_common(const Attr *a, int pid, void *val, int v_strd,
  ...
  if (pid == 0 && a->location() != 'w')
    throw COM_exception(
        COM_ERR_NOT_A_WINDOW_DATAITEM,
        append_frame(a->fullname(), ComponentInterface::copy_size));
...
void ComponentInterface::copy_array(const std::string &aname, const int pane_id,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (conn == NULL)
      throw COM_exception(
          COM_ERR_DATAITEM_NOTEXIST,
          append_frame(name() + "." + aname, ComponentInterface::copy_array));
...
void ComponentInterface::reinit_dataitem(DataItem *a, OP_Init op, void **addr,
                                         int strd, int cap) {
  int aid = a->id();
...
void ComponentInterface::reinit_conn(Connectivity *con, OP_Init op, int **addr,
                                     int strd, int cap) {
  Pane *pn = con->pane();
  if (pn->id() == 0) {
    COM_assertion(op != Pane::OP_SET && op != Pane::OP_SET_CONST);
...
Pane &ComponentInterface::pane(const int pid, bool insert) {
...
const Pane &ComponentInterface::pane(const int pid) const {
  if (pid == 0) return _dummy;
...
DataItem *ComponentInterface::dataitem(const std::string &aname) {
  if (!DataItem::is_digit(aname[0])) {
    ...
    if (start == aname.npos)
      throw COM_exception(
          COM_ERR_INVALID_DATAITEM_NAME,
          append_frame(_name + "." + aname, ComponentInterface::dataitem));
...
Function *ComponentInterface::function(const std::string &fname) {
