...
void Control_parameters::read()
{
  ...
  if (outputModule == "SimOUT") {
    MAN_DEBUG(1, ("[%d] Rocman: load_module SimOUT.\n", COMMPI_Comm_rank(communicator)));
...
int check_for_interrupt(Coupling *coup,const Control_parameters &param)
{
  ...
  MPI_Bcast(&interrupt_code,1,MPI_INT,0,param.communicator);
  if(interrupt_code > 0){
...
void rocstar_driver( int verb, int remeshed) {
  ...
  if ( comm_rank == 0 ) {
     printf(" Rocstar version %s-%04d\n", VERSION,BUILD_NUM);
     printf(" Rocstar last auto-built was at: %s\n", CI_DATE);
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  //  if(!comm_rank)
  //    std::cout << "Rocstar: Simpal initd." << std::endl;
