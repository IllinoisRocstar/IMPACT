...
SurfDiver::SurfDiver(FluidAgent *fag, SolidAgent *sag):
  Action( 0, (const char**)NULL, NULL, NULL, "SurfDiver"),
...
void SurfDiver::run( double t, double dt, double alpha) {
  ...
  if (fagent->get_comm_rank() == 0) {
    ...
    int oldcomm = COM_get_default_communicator();
    ...
    std::string fluid_file = fagent->get_rocmod_name()+"/SimOUT/ifluid_in_"+time_str+".txt";
    std::string solid_file = sagent->get_rocmod_name()+"/SimOUT/isolid_in_"+time_str+".txt";
...
void compute_overlay( FluidAgent *fagent, SolidAgent *sagent, double t) {
  ...
  if (fagent->get_comm_rank() == 0) {
    ...
    std::string time_str;
    fagent->get_time_string(t, time_str);
    std::string fluid_file = fagent->get_rocmod_name()+"/SimOUT/ifluid_"+time_str+".txt";
    std::string solid_file = sagent->get_rocmod_name()+"/SimOUT/isolid_"+time_str+".txt";
    ...
    // call Simpal to get deformed data HERE
