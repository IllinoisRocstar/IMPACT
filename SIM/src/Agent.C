...
const char *PhysicsAction::name() 
{ 
  return (const char *)agent->get_agent_name().c_str(); 
}
...
NewDataItem::NewDataItem(Agent *ag, std::string target_window_, std::string attr_, char loc_, int type_, int ncomp_, const char * unit_) :
...
void NewDataItem::create(std::string bufname)
{
  ...
  if (COM_get_dataitem_handle( bufname+attr) > 0) {
    ...
    if (loc_ != loc || type_ != type || ncomp_ != ncomp || unit_ != unit) {
      std::cout << "ROCMAN Error: NewDataItem::create(): Could not create " << bufname+attr << " in two different ways " << std::endl;
...
CloneDataItem::CloneDataItem(Agent *ag, int cond, 
...
void CloneDataItem::create(std::string bufname)
{
  ...
  if (COM_get_dataitem_handle( bufname+attr) > 0) {
    ...
    if (loc1 != loc2 || type1 != type2 || ncomp1 != ncomp2 || unit1 != unit2) {
      std::cout << "ROCMAN Error: CloneDataItem::create(): Could not create " << target_window+attr << " in two different ways " << std::endl;
      MPI_Abort(MPI_COMM_WORLD, -1);
    }
    else {
      MAN_DEBUG(2, ("CloneDataItem::create: %s%s %s%s condition:%d handle:%d SKIPPED.\n", bufname.c_str(), attr.c_str(), parent_window.c_str(), parent_attr.c_str(), condition, COM_get_dataitem_handle( parent_window+parent_attr)));
      return;
    }
  }
...
UseDataItem::UseDataItem(Agent *ag, std::string target_window_, std::string attr_, std::string parent_window_, std::string parent_attr_, int wg_, const char *ptnname_, int val_):  
 		DataItemBase(ag, target_window_, attr_),
		parent_window(parent_window_), parent_attr(parent_attr_),
...
void UseDataItem::create(std::string bufname)
{
  ...
  if (COM_get_dataitem_handle( bufname+attr) > 0) {
    ...
    COM_get_dataitem( parent_window+parent_attr, &loc2, &type2, &ncomp2, &unit2);
    if (loc1 != loc2 || type1 != type2 || ncomp1 != ncomp2 || unit1 != unit2) {
      std::cout << "ROCMAN Error: NewDataItem::create(): Could not create " << target_window+attr << " in two different ways " << std::endl;
      MPI_Abort(MPI_COMM_WORLD, -1);
    }
    else
      return;
    ...
    withgm(wgm), rocmod_name( mod), mod_instance(obj), skipInputIO(skipio)
{
  ...
  inDir = obj;  inDir.append("/SimIN/");
  outDir = obj; outDir.append("/SimOUT/");
  suffix = ".hdf";
...
{
  NewDataItem *newAttr = new NewDataItem(this, target_window_, attr_, loc_, type_, ncomp_, unit_);
...
{
  CloneDataItem *newAttr = new CloneDataItem(this, cond, target_window_, attr_, parent_window_, parent_attr_, wg_, ptnname_, val_);
  dataitemList.push_back(newAttr);
}
...
{
  UseDataItem *newAttr = new UseDataItem(this, target_window_, attr_, parent_window_, parent_attr_, wg_, ptnname_, val_);
  dataitemList.push_back(newAttr);
}
