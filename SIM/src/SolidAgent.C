...
{
  int.data_hdl = COM_get_dataitem_handle_const( solidSurfIN+".data");
  int buf_hdl = COM_get_dataitem_handle( solidBufBase+".data");
  COM_call_function( obtain_attr_handle, .data_hdl, &buf_hdl);

  if ( withALE) {
          // Obtain data for surface propagation if ALE is enabled
        int propBufAll_hdl = COM_get_dataitem_handle( propBufAll+".data");
        COM_call_function( obtain_attr_handle, .data_hdl, &propBufAll_hdl);
