...
Action::Action(void *p, const char *name): action_name(name), attr(NULL),
...
// }
Action::Action(int n, const char **at, int *id, void *p, const char *name): action_name(name), count(n), usr_ptr(p) 
{
  set_attr(n, at, id);
  if (action_name == NULL) action_name = (char*)typeid(*this).name();
}
Action::Action(int n, const std::string at[], int *id, void *p, const char *name): action_name(name), count(n), usr_ptr(p) 
{
  set_attr(n, at, id);
  if (action_name == NULL) action_name = (char*)typeid(*this).name();
}
...
int Action::get_dataitem_handle( int i) {
  COM_assertion_msg ( attr[i]!=NULL, (std::string("DataItem \"")
			      +"\" does not exist").c_str());
  int hdl = COM_get_dataitem_handle( attr[i]);
  COM_assertion_msg ( hdl>0, (std::string("DataItem \"")+attr[i]
			      +"\" does not exist").c_str());
  return hdl;
}
...
int Action::get_dataitem_handle( const std::string str) {
  ...
  COM_assertion_msg ( hdl>0, (std::string("DataItem \"")+str
			      +"\" does not exist").c_str());
  return hdl;
}
