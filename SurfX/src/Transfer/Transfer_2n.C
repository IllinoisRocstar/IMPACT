...
template <class _SDF>
void Transfer_base::interpolate_fe(const _SDF &sDF, Nodal_data &tDF,
                                   bool verbose) {
  double t0 = 0.;
  if (verbose) {
    trg.barrier();
  ...
  for (Pane_iterator pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    ...
    flags.clear();
    flags.resize(p_trg->size_of_nodes() + 1, false);
    ...
    int count = 0,
        nnodes = p_trg->size_of_nodes() - p_trg->size_of_isolated_nodes();
    ...
    for (int i = 1, size = p_trg->size_of_subnodes(); i <= size; ++i) {
      ...
      if (is_nodal(sDF.tag())) {
        ...
        interpolate(Generic_element(3), make_field(sDF, p_src, ene_src), nc, v);
      }
    }
    ...
    for (int k = 1, size = p_trg->size_of_faces(); k <= size; ++k, ene.next()) {
      ...
      switch (ene.size_of_nodes()) {
        ...
        case 9:
          f[8] =
              0.25 * (f[0] + f[1] + f[2] + f[3]);  // Then continue as 8-nodes
        case 8:
          f[4] = 0.5 * (f[0] + f[1]);
          f[5] = 0.5 * (f[1] + f[2]);
          f[6] = 0.5 * (f[2] + f[3]);
  ...
  if (verbose) {
    ...
    trg.barrier();
    if (trg.is_root())
      std::cout << "ROCFACE: Interpolation done in " << get_wtime() - t0
                << " seconds." << std::endl;
  }
}
...
void Transfer_base::element_load_vector(
    const _Data &data_s, const Generic_element &e_s, const Generic_element &e_t,
    const _Points &pnts_s, const _Points &pnts_t, const Point_2 *ncs_s,
    const Point_2 *ncs_t, const Real alpha, const int sne, const Tag &tag,
    _Loads loads, _Loads diag, Real *emm_out, int doa, bool lump) {
  const unsigned int n = e_t.size_of_nodes();

  Generic_element sub_e(sne);
  Vector_n vt(data_s.dimension(), 0);
  Array_n v(vt.begin(), vt.end());
  ...
  for (int i = 0, ni = sub_e.get_num_gp(doa); i < ni; ++i) {
    ...
    if (alpha != 1.)
      a_s *= sub_e.Jacobian_det(ps_s, sub_nc);
    else
      a_s = a_t;
  ...
  if (mu < 0 && !lump && compute_mass) {
    ...
    for (unsigned int j = 0; j < n; ++j) {
      for (unsigned int k = 0; k < j; ++k) {
        t1 += emm[j * n + k] * esf[j * n + k];
        t2 += esf[j * n + k] * esf[j * n + k];
      }
    }
    mu = -t1 / t2;
  }
...
template <class _SDF>
void Transfer_base::compute_load_vector_wra(
    const RFC_Pane_transfer *p_src, RFC_Pane_transfer *p_trg, const _SDF &sDF,
    ENE &ene_src, ENE &ene_trg, int sfid_src, int sfid_trg, const Real alpha,
    Nodal_data &rhs, Nodal_data &diag, int doa, bool lump) {
  ...
  if (needs_diag) pemm = lump ? (Real *)NULL : p_trg->get_emm(ene_trg.id());

  // Invoke the implementation to compute load vector and mass matrix.
  element_load_vector(make_field(sDF, p_src, ene_src), e_src, e_trg, pnts_s,
                      pnts_t, ncs_src, ncs_trg, alpha, 3, sDF.tag(),
                      make_field(rhs, p_trg, ene_trg),
                      make_field(diag, p_trg, ene_trg), pemm, doa, lump);
}

template <class _SDF>
void Transfer_base::init_load_vector(const _SDF &sDF, const Real alpha,
                                     Nodal_data &rhs, Nodal_data &diag, int doa,
                                     bool lump) {
  bool needs_diag = diag.dimension() > 0;
  ...
  for (Pane_iterator pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    ...
    for (int i = 1, size = (*pit)->size_of_subfaces(); i <= size; ++i) {
      ...
      compute_load_vector_wra(p_src, *pit, sDF, ene_src, ene_trg, fid.face_id,
...
template <class _SDF>
void Transfer_base::transfer_2n(const _SDF &sDF, Nodal_data &tDF,
  ...
  if (*iter > 0) {
    ...
    if (ierr) {
      std::cerr << "***ROCFACE::WARNING: PCG did not converge after " << *iter
                << " iterations and relative error is " << *tol << std::endl;
    }
  } else {
  ...
  if (verbose) {
    trg.barrier();
    if (trg.is_root()) {
      std::cout << "ROCFACE: Transfer to nodes done in " << get_wtime() - t0
                << " seconds";
      if (*iter > 0)
        std::cout << " with relative error " << *tol << " after " << *iter
                  << " iterators" << std::endl;
      else
        std::cout << "." << std::endl;
    }
...
void Transfer_n2n::transfer(const Nodal_data_const &sv, Nodal_data &tv,
                            const Real alpha, Real *tol, int *iter, int doa,
                            bool verbose) {
  Base::transfer_2n(sv, tv, alpha, tol, iter, doa, verbose);
}

void Transfer_f2n::transfer(const Facial_data_const &sf, Nodal_data &tv,
                            const Real alpha, Real *tol, int *iter, int doa,
                            bool verbose) {
  Base::transfer_2n(sf, tv, alpha, tol, iter, doa, verbose);
}

void Interpolator::transfer(const Nodal_data_const &sv, Nodal_data &tv,
                            bool verbose) {
  double t0 = 0.;
  if (verbose) {
    trg.barrier();
...
template <class _SDF>
void Transfer_base::loadtransfer(const _SDF &sDF, Nodal_data &tDF,
  ...
  if (verbose) {
    ...
    trg.barrier();
    if (trg.is_root())
      std::cout << "ROCFACE: Load transfer done in " << get_wtime() - t0
                << " seconds." << std::endl;
  }
}

void Transfer_n2n::comp_loads(const Nodal_data_const &sv, Nodal_data &tv,
                              const Real alpha, const int order, bool verbose) {
  Base::loadtransfer(sv, tv, alpha, order, verbose);
}

void Transfer_f2n::comp_loads(const Facial_data_const &sv, Nodal_data &tv,
                              const Real alpha, const int order, bool verbose) {
  Base::loadtransfer(sv, tv, alpha, order, verbose);
}
