...
interpolate_fe( const _SDF &sDF, Nodal_data &tDF, bool verbose) 
{
  ...
  if ( verbose) {
    ...
    trg.barrier();
    if ( trg.is_root())
      std::cout << "ROCFACE: Interpolation done in " 
	     ...
	     Real *tol, int *iter, int doa, bool verbose) 
{
  ...
  if ( *iter>0) {
    ...
    if (ierr) {
      std::cerr << "***ROCFACE::WARNING: PCG did not converge after " 
  ...
  if ( verbose) {
    trg.barrier();
    if ( trg.is_root()) {
      std::cout << "ROCFACE: Transfer to nodes done in " 
	      ...
	      const int order, bool verbose) 
{
  ...
  if ( verbose) {
    ...
    trg.barrier();
    if ( trg.is_root())
      std::cout << "ROCFACE: Load transfer done in " 
		<< get_wtime()-t0 << " seconds." << std::endl;
  }
}
