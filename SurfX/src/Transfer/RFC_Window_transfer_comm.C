...
void RFC_Window_transfer::incident_faces(
    std::map<int, std::vector<int> > &opp_subface_lists) const {
  RFC_assertion(opp_subface_lists.empty());

  // Loop through the panes to generate subface list
  for (Pane_set::const_iterator pi = _pane_set.begin(); pi != _pane_set.end();
       ++pi) {
...
void RFC_Window_transfer::replicate_metadata(
    const RFC_Window_transfer &opp_win) {
  ...
  pane_ids.reserve(opp_subface_lists.size());

  for (std::map<int, std::vector<int> >::const_iterator it =
           opp_subface_lists.begin();
       it != opp_subface_lists.end(); ++it) {
  ...
  for (std::map<int, std::vector<int> >::const_iterator it =
    ...
    pn._recv_faces.insert(pn._recv_faces.end(), face_list.begin(),
                          face_list.end());
    pn._recv_nodes.reserve(node_list.size());
    pn._recv_nodes.insert(pn._recv_nodes.end(), node_list.begin(),
                          node_list.end());
  }
  ...
  for (Pane_set::iterator pi = _pane_set.begin(); pi != _pane_set.end(); ++pi) {
    ...
    for (std::map<int, std::set<int> >::const_iterator it = sfs.begin();
         it != sfs.end(); ++it) {
    ...
    for (std::map<int, std::set<int> >::const_iterator it = sns.begin();
         it != sns.end(); ++it) {
...
void RFC_Window_transfer::replicate_metadata(int *pane_ids, int n) {
  ...
  if (COMMPI_Initialized())
    MPI_Alltoall(&npanes_recv[0], 1, MPI_INT, &npanes_send[0], 1, MPI_INT,
                 _comm);
  ...
  if (COMMPI_Initialized())
    MPI_Alltoallv(&ids_recv[0], &npanes_recv[0], &displs_recv[0], MPI_INT,
                  &ids_send[0], &npanes_send[0], &displs_send[0], MPI_INT,
                  _comm);
...
void RFC_Window_transfer::replicate_data(const Facial_data_const &data,
                                         bool replicate_coor) {
  ...
  for (std::map<int, RFC_Pane_transfer *>::iterator it = _replic_panes.begin(),
                                                    iend = _replic_panes.end();
       it != iend; ++it) {
  ...
  for (std::set<std::pair<int, RFC_Pane_transfer *> >::const_iterator
           it = _panes_to_send.begin(),
           iend = _panes_to_send.end();
       it != iend; ++it) {
    RFC_Pane_transfer *p = it->second;
...
// replicate_coor is true.
void RFC_Window_transfer::replicate_data(const Nodal_data_const &data,
                                         bool replicate_coor) {
  std::vector<MPI_Request> other_requests, recv_requests;
...
void RFC_Window_transfer::reduce_to_all(Nodal_data &data, MPI_Op op) {
  std::vector<void *> ptrs;
  ptrs.reserve(_pane_set.size());
...
void RFC_Window_transfer::reduce_maxabs_to_all(Nodal_data &data) {
  std::vector<void *> ptrs;
  ptrs.reserve(_pane_set.size());
...
bool RFC_Window_transfer::is_root() const {
  int rank;
...
void RFC_Window_transfer::wait_any(int n, MPI_Request *requests, int *index,
                                   MPI_Status *stat) {
  if (n > 0) {
    MPI_Status s;
    if (stat == NULL) stat = &s;
...
void RFC_Window_transfer::allreduce(Real *data, int n, MPI_Op op) const {
...
void RFC_Window_transfer::init_send_buffer(int pane_id, int to_rank) {
  _panes_to_send.insert(
      std::pair<int, RFC_Pane_transfer *>(to_rank, &pane(pane_id)));
}

COM_EXTERN_MODULE(SimIN)

void RFC_Window_transfer::init_recv_buffer(int pane_id, int from_rank) {
  ...
  if (_IO_format == SDV_BINARY) {
    ...
    int hdl_all = COM_get_dataitem_handle((buf_wname + ".all").c_str());
    COM_call_function(hdl_obtain, &hdl_all, &hdl_all, &pane_id);
    hdl_all = COM_get_dataitem_handle((sdv_wname + ".all").c_str());
    COM_call_function(hdl_obtain, &hdl_all, &hdl_all, &pane_id);
...
void RFC_Window_transfer::clear_replicated_data() {
  // Loop through the replicated panes
