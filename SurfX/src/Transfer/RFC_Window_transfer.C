...
RFC_Window_transfer::RFC_Window_transfer(COM::Window *b, int c, MPI_Comm com,
                                         const char *pre, const char *format)
    : Base(b, c, com),
      _buf_dim(0),
      _comm(com),
      _replicated(false),
      _prefix(pre == NULL ? b->name() : pre),
      _IO_format(get_sdv_format(format)) {
  std::vector<Pane *> pns;
  panes(pns);
  ...
  for (COM::Window::Proc_map::const_iterator it = proc_map.begin(),
                                             iend = proc_map.end();
       it != iend; ++it) {
    int p = it->second;
...
RFC_Pane_transfer &RFC_Window_transfer::pane(const int pid) {
...
const RFC_Pane_transfer &RFC_Window_transfer::pane(const int pid) const {
...
void RFC_Window_transfer::init_nodal_buffers(const Nodal_data &nd, int size,
                                             bool init_emm) {
...
Nodal_data RFC_Window_transfer::nodal_buffer(int index) {
  RFC_assertion(index >= 0);
  return Nodal_data(-index - 1, _buf_dim);
}

void RFC_Window_transfer::delete_nodal_buffers() {
...
void RFC_Window_transfer::set_tags(const COM::DataItem *tag) {
  // Loop through the panes to set the tags
  for (Pane_set::iterator pi = _pane_set.begin(); pi != _pane_set.end(); ++pi) {
    RFC_Pane_transfer &pane = (RFC_Pane_transfer &)*pi->second;

    pane._to_recv = (tag == NULL)
                        ? (int *)NULL
                        : (int *)(pane._base->dataitem(tag->id())->pointer());
  }
}

// Initialize buffer spaces for data transfer
void RFC_Window_transfer::init_facial_buffers(const Facial_data &nd, int size) {
  _buf_dim = nd.dimension();
...
Facial_data RFC_Window_transfer::facial_buffer(int index) {
  RFC_assertion(index >= 0);
  return Facial_data(-index - 1, _buf_dim);
}

void RFC_Window_transfer::delete_facial_buffers() {
  // Loop through the panes to remove the buffer spaces
  for (Pane_set::iterator pi = _pane_set.begin(); pi != _pane_set.end(); ++pi) {
    RFC_Pane_transfer &pane = (RFC_Pane_transfer &)*pi->second;
...
void RFC_Window_transfer::incident_panes(std::vector<int> &pane_ids) {
  std::set<int> ids;
  ...
  for (Pane_set::iterator pi = _pane_set.begin(); pi != _pane_set.end(); ++pi) {
    ...
    for (int i = 0, size = pane._subnode_counterparts.size(); i < size; ++i) {
      RFC_assertion(ids.find(pane._subnode_counterparts[i].pane_id) !=
                    ids.end());
    }
  }

  pane_ids.resize(0);
  pane_ids.reserve(ids.size());
