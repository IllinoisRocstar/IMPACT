...
int Transfer_base::pcg(Nodal_data &x, Nodal_data &b, Nodal_data &p,
...
void Transfer_base::precondition_Jacobi(const Nodal_data_const &rhs,
...
void Transfer_base::multiply_mass_mat_and_x(const Nodal_data_const &x,
                                            Nodal_data &y) {
...
Real Transfer_base::norm2(const Nodal_data_const &x) const {
  Real nrm(0);
  ...
  for (Pane_iterator_const pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    ...
    // Loop through the nodes of each pane.
    for (int i = 1, size = (*pit)->size_of_nodes(); i <= size; ++i)
      if ((*pit)->is_primary_node(i)) nrm += square(x.get_value(p, i));
  }
  trg.allreduce(&nrm, MPI_SUM);
  return nrm;
}

Real Transfer_base::dot(const Nodal_data_const &x,
...
void Transfer_base::dot2(const Nodal_data_const &x1, const Nodal_data_const &y1,
...
void Transfer_base::saxpy(const Real &a, const Nodal_data_const &x,
...
void Transfer_base::scale(const Real &a, Nodal_data &x) {
  for (Pane_iterator pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    Real *px = (*pit)->pointer(x.id());
    // Loop through the nodes of each pane.
    for (int i = 1, size = (*pit)->size_of_nodes(); i <= size; ++i)
...
void Transfer_base::invert(Nodal_data &x) {
  for (Pane_iterator pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    Real *px = (*pit)->pointer(x.id());
    // Loop through the nodes of each pane.
    for (int i = 1, size = (*pit)->size_of_nodes(); i <= size; ++i)
...
void Transfer_base::copy_vec(const Nodal_data_const &x, Nodal_data &y) {
  for (Pane_iterator pit = trg_ps.begin(); pit != trg_ps.end(); ++pit) {
    Real *px = (*pit)->pointer(x.id());
    Real *py = (*pit)->pointer(y.id());
...
void Transfer_base::minmax(const RFC_Window_transfer &win,
                           const Nodal_data_const &sDF, Array_n &min_v,
                           Array_n &max_v) {
  ...
  for (std::vector<const RFC_Pane_transfer *>::iterator pit = ps.begin();
       pit != ps.end(); ++pit) {
    ...
    for (int i = 1, size = (*pit)->size_of_nodes(); i <= size; ++i) {
      ...
      for (unsigned int k = 0; k < t.dimension(); ++k) {
        if (!isfinite(t[k])) {
          std::cerr << "ERROR: ****Invalid number " << t[k] << " in "
                    << win.name() << "[" << i - 1 << "][" << k
                    << "] (C Convention). Aborting..." << std::endl;
          RFC_assertion(isfinite(t[k]));
...
void Transfer_base::minmax(const RFC_Window_transfer &win,
                           const Facial_data_const &sDF, Array_n &min_v,
                           Array_n &max_v) {
  min_v = Vector_n(sDF.dimension(), HUGE_VAL);
  max_v = Vector_n(sDF.dimension(), -HUGE_VAL);
...
void Transfer_base::integrate(const RFC_Window_transfer &win,
                              const Facial_data_const &sDF, Array_n &integral,
                              const int doa) {
  ...
  Vector_n vt(sDF.dimension(), 0);
  Array_n t(vt.begin(), vt.end());
  ...
  for (std::vector<const RFC_Pane_transfer *>::iterator pit = ps.begin();
    ...
    ENE ene((*pit)->base(), 1);
    // Loop through the nodes of each pane.
    for (int i = 1, size = (*pit)->size_of_faces(); i <= size;
         ++i, ene.next()) {
...
// Integrate the values over the surface.
void Transfer_base::integrate(const RFC_Window_transfer &win,
                              const Nodal_data_const &sDF, Array_n &integral,
                              const int doa) {
  integral = Vector_n(sDF.dimension(), 0);
