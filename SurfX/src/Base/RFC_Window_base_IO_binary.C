...
inline void write(std::ostream &os, const int t) {
  os.write((char *)&t, sizeof(int));
}

inline void read(std::istream &is, int &t) { is.read((char *)&t, sizeof(int)); }

inline void swap_endian(short int &t) {
  char *c = (char *)&t;
  std::swap(c[0], c[1]);
}

inline void swap_endian(int &t) {
  char *c = (char *)&t;
  std::swap(c[0], c[3]);
  std::swap(c[1], c[2]);
}

inline void swap_endian(float &t) {
  char *c = (char *)&t;
  std::swap(c[0], c[3]);
  std::swap(c[1], c[2]);
}

inline void swap_endian(double &t) {
  char *c = (char *)&t;
  std::swap(c[0], c[7]);
  std::swap(c[1], c[6]);
  std::swap(c[2], c[5]);
...
void RFC_Pane_base::write_binary(std::ostream &os) const {
  ...
  // #nodes and #faces
  write(os, size_of_nodes());
  write(os, size_of_faces());
  ...
  if (_base->is_structured()) {
    ...
    write(os, 2);
    write(os, 1);
    write(os, _base->size_i());
    write(os, _base->size_j());
  } else {
  ...
  write(os, _subnode_parents.size());
  write(os, _subfaces.size());
  ...
  // Output subfaces.
  {
    ...
    os.write((const char *)&_subfaces[0], n * sizeof(_subfaces[0]));
...
void RFC_Pane_base::read_binary(std::istream &is, std::vector<int> *b2v_all,
                                COM::Pane *pn_in) {
  ...
  if (pn != NULL)
    pn->set_size(pn->dataitem(COM::COM_NC), t1, 0);
  else
  ...
  is.read((char *)&_subnode_nat_coors[0], n * sizeof(_subnode_nat_coors[0]));
  if (need_swap)
    for (int j = 0; j < n; ++j) {
  ...
  if (need_swap)
    for (int j = 0; j < n; ++j) swap_endian(_subface_offsets[j]);

  RFC_assertion(_subface_offsets[size_of_faces()] == int(_subfaces.size()));
  ...
  RFC_assertion(t1 ==
                int(_subnode_parents.size() + _subfaces.size()));  // Checksum
