...
void RFC_Pane_base::write_tec_ascii(std::ostream &os,
                                    const COM::DataItem *attr) const {
...
static void write_dataitem(std::ostream &os, const COM::DataItem *attr, int i) {
...
void RFC_Pane_base::write_tec_ij(std::ostream &os,
                                 const COM::DataItem *attr) const {
  ...
  if (attr) {
    ...
    for (int i = 0; i < dim; ++i) os << ", \"" << vname[i] << "\"";
  }
  os << "\nZONE I=" << _base->size_i() << ", J=" << _base->size_j()
     << ", F=POINT\n";
  ...
  for (int i = 0, n = _base->size_of_nodes(); i < n; ++i) {
    ...
    if (attr) write_dataitem(os, attr, i);
    os << std::endl;
  }
  os << std::endl;
}
...
void RFC_Pane_base::write_tec_tri(std::ostream &os, const COM::Connectivity &ec,
                                  const COM::DataItem *attr) const {
  RFC_assertion(_base != NULL);
  ...
  os << "\nZONE N=" << _base->size_of_nodes()
     << ", E=" << _base->size_of_elements() << ", F=FEPOINT, ET=TRIANGLE\n";
  ...
  for (unsigned int i = 0, n = _base->size_of_elements(); i < n; ++i) {
    os << e[nn * i] << ' ' << e[nn * i + 1] << ' ' << e[nn * i + 2]
       << std::endl;
  }
}

// Write out a triangular finite element mesh in FEPOINT format of Tecplot
void RFC_Pane_base::write_tec_mixed(
    std::ostream &os, const std::vector<const COM::Connectivity *> &elems,
    const COM::DataItem *attr) const {
  RFC_assertion(_base != NULL);
...
void RFC_Pane_base::write_tec_sub(std::ostream &os) const {
  RFC_assertion(_base != NULL);
  if (size_of_subnodes() == 0) return;

  os << "TITLE = \"Triangular subdivision mesh\" \n";
  os << "VARIABLES = \"X\", \"Y\", \"Z\"";
  os << "\nZONE N=" << size_of_subnodes() << ", E=" << size_of_subfaces()
  ...
  for (int i = 0, size = size_of_subfaces(); i < size; ++i) {
    os << _subfaces[i][0] << ' ' << _subfaces[i][1] << ' ' << _subfaces[i][2]
       << std::endl;
  }
}
...
/*!
  ...
  \param a DataItem to be writtin out. Default is mesh only.
*/
void RFC_Window_base::write_tec_sub(const char *prefix) const {
  std::ofstream os((std::string(prefix) + ".plt").c_str());
  RFC_assertion(os);
  os.precision(9);
  os.setf(std::ios::scientific, std::ios::floatfield);
  for (Pane_set::const_iterator it = _pane_set.begin(), iend = _pane_set.end();
       it != iend; ++it) {
...
/*!
  ...
  \param a DataItem to be writtin out. Default is mesh only.
*/
void RFC_Window_base::write_tec_ascii(const char *prefix) const {
  std::ofstream os((std::string(prefix) + ".plt").c_str());
  RFC_assertion(os);
  os.precision(9);
  os.setf(std::ios::scientific, std::ios::floatfield);
