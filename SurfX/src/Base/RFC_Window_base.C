...
#include <set>
#include "Generic_element_2.h"  // surfutil
#include "Pane_boundary.h"      // surfmap
#include "Pane_connectivity.h"  // surfmap
...
void RFC_Pane_base::init() {
  ...
  if (!_base->is_structured()) {
    ...
    for (std::vector<const COM::Connectivity *>::const_iterator it =
             _elem_conns.begin();
         it != _elem_conns.end(); ++it) {
  ...
  for (int i = 0, n = _is_border.size(); i < n; ++i) {
    _n_border += _is_border[i];
    _n_isolated += _is_isolated[i];
  }
}
...
bool RFC_Pane_base::coincident(int v1, int v2) const {
...
void RFC_Pane_base::get_nat_coor_in_element(const int sn_id,
  ...
  const ParentEdge_ID &eid = _subnode_parents[sn_id - 1];
 ...
 */
int RFC_Pane_base::parent_type_of_subnode(int id) const {
  const Point_2S &nc = _subnode_nat_coors[id - 1];
  if (nc[0] == 0 && nc[1] == 0) return PARENT_VERTEX;
  if (nc[1] == 0) return PARENT_EDGE;
  return PARENT_FACE;
}

/*! Precondition: The parent type is PARENT_VERTEX.
 */
int RFC_Pane_base::get_parent_node(int id) const {
...
Point_3 RFC_Pane_base::get_point_of_subnode(int i) const {
  const int fid = _subnode_parents[i - 1].face_id;
  Element_node_enumerator ene(_base, fid);
  Point_2 nc;
  get_nat_coor_in_element(i, ene, nc);

  Nodal_coor_const coors;
  Field<const Nodal_coor_const, Element_node_enumerator> ps(coors,
                                                            coordinates(), ene);

  return SURF::Generic_element_2(ene.size_of_edges(), ene.size_of_nodes())
      .interpolate(ps, nc);
}

Bbox_3 RFC_Pane_base::get_bounding_box() const {
  Bbox_3 b;
...
RFC_Pane_base::ParentEdge_ID RFC_Pane_base::get_edge_id(
    const int face_lid, const int vertex_lid) const {
  if (_base->is_structured()) {
    Element_node_enumerator ene(_base, face_lid, (COM::Connectivity *)NULL);
    for (int k = 0; k < 4; ++k)
      if (ene[k] == vertex_lid) return ParentEdge_ID(face_lid, k);
    RFC_assertion(false);
    abort();
  } else {
    for (std::vector<const COM::Connectivity *>::const_iterator it =
      ...
      if (face_lid > int((*it)->index_offset()) &&
        ...
        Element_node_enumerator ene(_base, face_lid - (*it)->index_offset(),
                                    *it);
  ...
  return ParentEdge_ID(face_lid, -1);
}

int RFC_Pane_base::get_lvid(const Element_node_enumerator &ene,
                            const int vid) const {
  int i = ene.vertex(vid);
  if (i < 0) {
    // Resort to a more expensive method.
    for (int j = ene.size_of_nodes() - 1; j >= 0; --j) {
      if (coincident(vid, ene[j])) {
        i = j;
      }
    }
    RFC_assertion(i >= 0);
...
bool RFC_Pane_base::is_primary_node(const int vid) const {
...
int RFC_Pane_base::size_of_primary_nodes() const {
  int n = size_of_nodes();
...
void RFC_Pane_base::build_v2b_from_b2v(const RFC_Window_base *w) {
  //`Loop through the B2v_table
  for (B2v_table::iterator it = _b2v_table.begin(), iend = _b2v_table.end();
       it != iend; ++it) {
    for (int i = 0, size = it->second.size(); i < size; ++i) {
      ...
      if (it->first == id()) {
        ...
        if ((i & 1) == 0 || is_isolated_node(it->second[i - 1])) continue;
        --p.second;
      } else {
...
int RFC_Window_base::size_of_nodes() const {
  int n = 0;
  // Loop through panes
...
int RFC_Window_base::size_of_faces() const {
  int n = 0;
  // Loop through panes
  Pane_set::const_iterator pit = _pane_set.begin(), piend = _pane_set.end();
  for (; pit != piend; ++pit) {
...
Bbox_3 RFC_Window_base::get_bounding_box() const {
  Bbox_3 b;
  Pane_set::const_iterator it = _pane_set.begin(), iend = _pane_set.end();
  for (; it != iend; ++it) b += it->second->get_bounding_box();
  return b;
}

void RFC_Window_base::build_pc_tables() {
  MAP::Pane_connectivity pc(_base->dataitem(COM::COM_MESH),
                            _map_comm.mpi_comm());
  pc.compute_pconn(_base->dataitem(COM::COM_PCONN));
  ...
  for (pane_it = _pane_set.begin(); pane_it != pane_iend; ++pane_it) {
    const COM::DataItem *pconn =
        pane_it->second->_base->dataitem(COM::COM_PCONN);
...
void RFC_Pane_base::comp_nat_coors() {
  ...
  for (int i = 0; i < nsn; ++i) {
    ...
    Point_2 p;
    get_nat_coor_in_element(i + 1, ene, p);
...
void RFC_Window_base::export_window(RFC_Window_base *w) const {
