...
static int start_subvertexID =
    0;                          /* whether the subvertex ids start from 0 or 1*/
static int start_subfaceID = 0; /* whether the subface ids start from 0 or 1*/

/* This function is passed information about subvertices and subfaces to be
   written to .off file Returns 0  : if operation successful Returns 1  : if
...
void SD_write_off_comref(FILE *fhandle, int pid, int n_sv, int n_sf,
                         ...
                         const int sv_parent_vID[], const int *sv_pnID,
                         const int *sf_pnID) {
  int i;
  ...
  fprintf(fhandle,
          "# Coor_x  Coor_y    Coor_z    #!  ParentFaceId NatCoor_xi "
          "NatCoor_eta  CorrespSubvertId  PartitionId  VertexId\n");

  for (i = 0; i < n_sv; i++) {
    fprintf(fhandle, "%f  %f  %f  #!", coor[3 * i], coor[3 * i + 1],
            coor[3 * i + 2]);
  ...
  fprintf(fhandle,
          "# sv_id_1  sv_id_2  sv_id_3  #! ParentFaceId sv1_nc_xi sv1_nc_eta "
          "sv2_nc_xi sv2_nc_eta sv3_nc_xi sv3_nc_eta CorrespSubfaceId "
          "PartitionId \n");
...
/* This function is used to set the starting values od the vertex
 * ,face,subvertex, and subface */
