...
void _solve_3x3(Real a1, Real a2, Real a3, Real b1, Real b2, Real b3, Real c1,
                Real c2, Real c3, Real d1, Real d2, Real d3, Real &x, Real &y,
                Real &z) {
  Real denom = b2 * c1 * a3 - b1 * c2 * a3 + c3 * b1 * a2 + b3 * c2 * a1 -
               c1 * b3 * a2 - b2 * c3 * a1;

  x = -(b2 * c3 * d1 - b2 * c1 * d3 + c1 * b3 * d2 + b1 * c2 * d3 -
        c3 * b1 * d2 - b3 * c2 * d1) /
      denom;

  z = (b2 * d1 * a3 - b2 * a1 * d3 + b1 * a2 * d3 - b1 * d2 * a3 -
       d1 * b3 * a2 + a1 * b3 * d2) /
      denom;

  y = (a2 * c3 * d1 - a2 * c1 * d3 - c2 * d1 * a3 + c2 * a1 * d3 +
       d2 * c1 * a3 - d2 * c3 * a1) /
      denom;
}
...
Vertex_set::Vertex_set(const HEdge &h) : _pane(h.pane()), _ne(count_edges(h)) {
  RFC_precondition(_ne == 3 || _ne == 4);
  Element_node_enumerator ene(_pane->base(), h.id().eid());

  Size nn = ene.size_of_nodes();
  _nodes.resize(nn);
  for (Size i = 0; i < nn; ++i) _nodes[i] = ene[i];

  int r = h.id().lid();
  if (r > 0) {
    std::rotate(&_nodes[0], &_nodes[r], &_nodes[_ne]);
    if (nn > _ne)
      std::rotate(&_nodes[_ne], &_nodes[_ne + r], &_nodes[_ne + _ne]);
  }
}

Vertex_set::Vertex_set(const HEdge &h, Size) : _pane(h.pane()), _ne(3) {
  _nodes.resize(_ne, -1);

  _nodes[0] = h.origin_l().id();
  _nodes[1] = h.destination_l().id();
}
...
bool Overlay_primitives::intersect(HEdge hb, const HEdge &hg, Real start,
                                   Real *c1, Real *c2, Real *orien_match,
                                   Real *normal_match, bool is_opposite,
                                   Real eps_e) const {
  RFC_assertion(!hg.is_border_l());

  // Switch blue edge if it is a border edge to use Point_set.
  bool is_b_border = hb.is_border_g();
  if (is_b_border) hb = hb.opposite_g();

  const Point_set pbs(hb), pgs(hg);
  ...
  *c1 = start;  // At input, set *c1 to start
  ...
  if (std::fabs(*c1) < 1 / eps_e && std::fabs(*c2) < 1 / eps_e) {
    ...
    v1 = acc.get_normal(hb.opposite_g());
    double prod2 = v1 * v2;
    ...
    *normal_match = std::max(prod1, prod2);
    ...
    if (std::fabs(*c1 - 1) <= eps_e)
      *c1 = 1.;
    else if (std::fabs(*c1) <= eps_e)
      *c1 = 0.;

    if (std::fabs(*c2 - 1) <= eps_e)
      *c2 = 1.;
    else if (std::fabs(*c2) <= eps_e)
      *c2 = 0.;
...
void Overlay_primitives::intersect(const Point_set &pbs, const Point_set &pgs,
  ...
  Real error = (v3 - (v3 * v2) / (v2 * v2) * v2).squared_norm() /
               std::max(vb * vb, vg * vg);
...
bool Overlay_primitives::project_onto_element(const Point_3 &p, HEdge *g,
                                              Parent_type *pt, Vector_3 dir,
                                              Point_2 *nc_out, const Real eps_p,
                                              Real eps_np) const {
  ...
  for (;; RFC_assertion(++i < max_nsteps)) {
    ...
    Real u[3];
    _solve_3x3(J[0][0], J[0][1], J[0][2], J[1][0], J[1][1], J[1][2], J[2][0],
               J[2][1], J[2][2], f[0], f[1], f[2], u[0], u[1], u[2]);

    nc = Point_2(nc[0] - u[0], nc[1] - u[1]);
    t -= u[2];

    if (std::fabs(nc[0]) > 1.e3 || std::fabs(nc[1]) > 1.e3) {
      // System is diverging
      break;
    }
  }
  ...
  // Perturb nc by eps_p.
  nc = Point_2(
      (nc[0] <= eps_p && nc[0] >= -eps_np)
          ? 0.
          : ((nc[0] >= 1. - eps_p && nc[0] <= 1. + eps_np) ? 1 : nc[0]),
      (nc[1] <= eps_p && nc[1] >= -eps_np)
          ? 0.
          : ((nc[1] >= 1. - eps_p && nc[1] <= 1. + eps_np) ? 1 : nc[1]));
...
Real Overlay_primitives::project_green_feature(
    const Vector_3 &n,  // Normal direction
  ...
  Real s, u, v;
  _solve_3x3(c1[0], c1[1], c1[2], n[0], n[1], n[2], b[0], b[1], b[2], f[0],
             f[1], f[2], s, u, v);
...
// for s, u, and v and return s;
Real Overlay_primitives::project_blue_feature(
    const Vector_3 &n1, const Vector_3 &n2, const Vector_3 &t1,
    const Vector_3 &t2, const Point_3 &q1, const Point_3 &q2, const Point_3 &p,
    const Real eps_e) const {
  ...
  for (;; RFC_assertion(++i < max_nsteps)) {
    ...
    Real x[3];
    _solve_3x3(c1[0], c1[1], c1[2], c2[0], c2[1], c2[2], c3[0], c3[1], c3[2],
               f[0], f[1], f[2], x[0], x[1], x[2]);
    s -= x[0];
    u -= x[1];
    v -= x[2];
  }
...
void Overlay_primitives::snap_blue_ridge_edge(const HEdge &b, const HEdge &g,
                                              Real *cb, Real *cg,
                                              Parent_type *t, Real tol) const {
  const RFC_Pane_overlay *pn = g.pane();
  ...
  if (*cg <= tol) {
    ...
    if ((is_border && v.is_border_g()) ||
        (!is_border && acc.is_on_feature(v))) {
      *cg = 0;
      *t = PARENT_VERTEX;
    }
  } else if (*cg >= 1 - tol) {
...
void Overlay_primitives::snap_blue_ridge_vertex(const Node &v, HEdge *g,
                                                Parent_type *pt, Point_2 *nc_p,
                                                Real tol) const {
  bool is_border = v.is_border_g();
  COM_assertion(acc.is_on_feature(v) && !g->is_border_g());

  Point_2 &nc = *nc_p;

  HEdge h(*g);
  Vertex_set vs(h);
  RFC_Pane_overlay *pn = (*g).pane();
  ...
  switch (vs.size_of_edges()) {
    case 3: {
      ...
      HEdge hs[3] = {(*g).prev_l(), *g, (*g).next_l()};
      bool is_feas[3];

      if (is_border)
        for (int i = 0; i < 3; ++i)
          is_feas[i] = hs[i].opposite_l().is_border_g();
      else
    ...
    case 4: {
      ...
      HEdge hs[4] = {(*g).prev_l(), *g, (*g).next_l(), (*g).next_l().next_l()};
      bool is_feas[4];
...
Real Overlay_primitives::normalmatch(const HEdge &b, const HEdge &g) const {
  RFC_assertion(acc.is_feature_1(b) && acc.is_feature_1(g));

  const HEdge bopp = b.opposite_g(), gopp = g.opposite_g();
  ...
  Real match[4] = {nrm_b * nrm_g, nrm_bopp * nrm_g, nrm_b * nrm_gopp,
                   nrm_bopp * nrm_gopp};
