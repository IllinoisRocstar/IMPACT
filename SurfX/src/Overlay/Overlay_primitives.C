...
void _solve_3x3(Real a1, Real a2, Real a3, Real b1, Real b2, Real b3,
                Real c1, Real c2, Real c3, Real d1, Real d2, Real d3,
                Real &x, Real &y, Real &z)
{
  Real denom = b2*c1*a3-b1*c2*a3+c3*b1*a2+b3*c2*a1-c1*b3*a2-b2*c3*a1;
...
Vertex_set::Vertex_set( const HEdge &h) 
  : _pane(h.pane()), _ne(count_edges(h)) 
{
...
Vertex_set::Vertex_set( const HEdge &h, Size)
  : _pane(h.pane()), _ne(3) { 
  _nodes.resize(_ne, -1);
...
project_onto_element( const Point_3 &p,
  ...
  for ( ;;RFC_assertion(++i<max_nsteps)) {
    ...
    Real u[3];
    _solve_3x3( J[0][0], J[0][1], J[0][2], J[1][0], J[1][1], J[1][2],
                J[2][0], J[2][1], J[2][2], f[0], f[1], f[2], u[0], u[1], u[2]);
  ...
  if ( *pt == PARENT_EDGE) {
    ...
    else if ( nc[0] > 1.) { nc[0] = nc[1] = 0; *g = (*g).next_l(); }
  ...
  case 3: {
    if ( (nc[0] + nc[1]) >= 1-eps_p && (nc[0] + nc[1]) <= 1+eps_np) {
      ...
      if ( nc[0] < eps_p) { 
	*g = (*g).prev_l(); nc[0] = nc[1] = 0; *pt = PARENT_VERTEX; 
      }
      else {
	*g = (*g).next_l(); nc = Point_2( nc[1], 0.); 
	*pt = (nc[1] == 0.) ? PARENT_VERTEX : PARENT_EDGE; 
	...
	{ *g = (*g).prev_l(); nc = Point_2( 1.-nc[1], 0.); *pt = PARENT_EDGE; }
    }
    else if ( nc[1] == 0.)
  ...
  case 4: {
	...
	*g = (*g).prev_l(); nc = Point_2( 1.-nc[1], 0.);
	*pt = (nc[1] == 1.) ? PARENT_VERTEX : PARENT_EDGE;
      ...
      if ( nc[1] == 1.)
	{ *g = (*g).next_l().next_l(); nc[0] = nc[1] = 0; *pt = PARENT_VERTEX;}
      else {
	*g = (*g).next_l(); nc = Point_2( nc[1], 0.);
	*pt = (nc[1] == 0.) ? PARENT_VERTEX : PARENT_EDGE;
      }
      ...
      { *g=(*g).next_l().next_l(); nc=Point_2(1.-nc[0],0.); *pt=PARENT_EDGE; }
    else 
      { *pt = PARENT_FACE; }
		       ...
		       const Real eps_e) const
{
  ...
  Real s, u, v;
  _solve_3x3( c1[0], c1[1], c1[2], n[0], n[1], n[2],
...
project_blue_feature( const Vector_3 &n1, const Vector_3 &n2,
  ...
  for ( ;;RFC_assertion(++i<max_nsteps)) {
    ...
    Real x[3];
    _solve_3x3( c1[0], c1[1], c1[2], c2[0], c2[1], c2[2],
...
void Overlay_primitives::
snap_blue_ridge_edge( const HEdge &b,
		      const HEdge &g,
		      Real *cb,
		      Real *cg,
		      Parent_type *t, 
  ...
  if ( *cg <= tol) {
    ...
    if (is_border && v.is_border_g() || !is_border && acc.is_on_feature( v)) {
...
void Overlay_primitives::
snap_blue_ridge_vertex( const Node &v,
			HEdge *g,
			Parent_type *pt,
			Point_2 *nc_p,
			Real tol) const {
  ...
  case 3: {
    ...
    if ( is_border)
      for ( int i=0; i<3; ++i) is_feas[i] = hs[i].opposite_l().is_border_g();
    else
  ...
  case 4: {
    ...
    if ( is_border)
      for ( int i=0; i<4; ++i) is_feas[i] = hs[i].opposite_l().is_border_g();
    else
...
normalmatch( const HEdge &b, const HEdge &g) const {
