 ...
 */
void RFC_Window_overlay::print_features() {
  std::cout << "Writing feature info "
            << "..." << std::endl;
  ...
  for (it = _pane_set.begin(); it != iend; ++it) {
    ...
    for (int i = 0, s = pane.size_of_nodes(); i < s; ++i) {
      ...
      if (h0.destination_g() == v) do {
          face_angle[i] =
              std::min(face_angle[i], cos_face_angle(h1, h1.opposite_g()));
...
void RFC_Window_overlay::dump_strong_edges(
    const std::vector<pair<float, HEdge> > &tstrong_edges,
  ...
  for (vector<pair<float, HEdge> >::const_iterator it = tstrong_edges.begin();
    ...
    os << "GEOMETRY T=LINE3D C=GREEN"
       ...
       << it->second.destination_l().point() << endl;
  }
