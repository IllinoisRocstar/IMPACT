...
void Overlay::match_features_0() {
  ...
  for (std::list<Feature_0>::iterator it = bf0.begin(), iend = bf0.end();
       it != iend; ++it) {
    Node v = it->vertex();
      ...
      if (dist_min < s * d2e_ratio_sq) {
        if (verbose2) {
          std::cout << "Matched\t blue vertex "
                    ...
                    << b.pane()->id() << " at " << p
                    << "\n  with\t green vertex "
                    << g.pane()->get_index(g.origin_l()) + 1 << " in pane "
  ...
  for (std::list<Feature_0>::iterator it = gf0.begin(), iend = gf0.end();
       it != iend; ++it) {
    Node v = it->vertex();
    if (acc.get_inode(v) == NULL) {
      if (verbose) {
        std::cout << "\nRocface Warning: Dropping green corner vertex "
                  << v.pane()->get_index(v) + 1 << " in pane " << v.pane()->id()
                  << " at " << v.point() << std::endl;
      }
