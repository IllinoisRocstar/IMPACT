...
INode *Overlay::overlay_init() {
  ...
  if (verbose2) {
    std::cout << "\nFound a seed at blue vertex (" << x.pane()->get_point(x)
              << ") \nin green ";
...
void Overlay::get_green_parent(const Node &v, const HEdge &b, HEdge *h_out,
                               Parent_type *t_out, Point_2 *nc) {
  ...
  if (vec * vec > sq_length(*h_out) + sq_length(v.halfedge_l())) {
    ...
    *t_out = PARENT_NONE;
    ...
    is_opposite =
        (b.pane()->get_normal(b, v) * op.get_face_normal(*h_out, *nc)) < 0;
  }
}
