...
INode *Overlay::project_next_vertex(Halfedge *b, Halfedge *g) {
  ...
  if (B->snap_on_features())
    eps_e = 5.e-3;  // Increase the threshold for RSRM
  else
    eps_e = Overlay::eps_e;

  Real s = op.project_green_feature(gpane->get_normal(g, gdst),
                                    gpane->get_tangent(g, gdst), borg->point(),
                                    bdst->point(), gdst->point(), eps_e);
  INode *x = new INode();
  if (s < 1) {
    ...
    if (s > 1) {
      ...
      t = op.project_blue_feature(
          gpane->get_normal(g, gorg), gpane->get_normal(g, gdst),
          gpane->get_tangent(g, gorg), gpane->get_tangent(g, gdst),
          gorg->point(), gdst->point(), bdst->point(), eps_e);
      if (t > 1) t = 1;
    } else
      t = 1;
...
int Overlay::overlay_features_1() {
  ...
  for (Feature_list_1::iterator it_f_b = lf_b.begin(), iend_f_b = lf_b.end();
    ...
    for (Feature_list_1::iterator it_f_g = lf_g.begin(), iend_f_g = lf_g.end();
      ...
      if (bnode != NULL) {
        f_g = it_f_g;
        RFC_assertion(bn_frt && bn_bck == bnode);
        if (bnode == acc.get_inode(acc.get_origin(it_f_g->front()))) {
          it_g_mid = it_f_g->begin();
          param = 0.;
        } else {
          it_g_mid = --it_f_g->end();
          param = 1.;
      ...
      for (Feature_1::iterator tmp_it_g = it_f_g->begin(), iend = it_f_g->end();
        ...
        Real gsqlen = (gpdst - gporg).squared_norm(),
             sd = gsqlen * d2e_ratio_sq;
        if ((bpdst - gporg).squared_norm() < sd ||
            (bpdst - gpdst).squared_norm() < sd) {
          const RFC_Pane_overlay *gpane = acc.get_pane(g);
          Real t = op.project_blue_feature(
              gpane->get_normal(g, gorg), gpane->get_normal(g, gdst),
              gpane->get_tangent(g, gorg), gpane->get_tangent(g, gdst), gporg,
              gpdst, bpdst, eps_e);
          if (t >= 0 && t <= 1) {
            Point_3 gp = gporg + t * (gpdst - gporg);
            Real sd = (gp - bpdst).squared_norm();
            if (sd < d2e_ratio_sq *
                         (bpdst - b->origin()->point()).squared_norm() &&
                sd < sqd) {
              f_g = it_f_g;
              it_g_mid = tmp_it_g;
              param = t;
    ...
    if (f_g == lf_g.end()) {
      std::cout << "Warning: Could not find match for the blue ridge!"
                << std::endl;
        ...
        is_opp = ((acc.get_pane(b)->get_normal(b, b->destination()) +
                   acc.get_pane(bopp)->get_normal(bopp, bopp->origin())) *
                      (acc.get_pane(g)->get_normal(g, g->destination()) +
                       acc.get_pane(gopp)->get_normal(gopp, gopp->origin())) <
                  0);
    ...
    if (param == 0. && dir_match) {
      if (it_g == f_g->begin()) it_g = f_g->end();
      --it_g;
    ...
    for (Feature_1::iterator it_b = it_b_mid, iend = --f_b->begin();
      ...
      for (;;) {
        ...
        if (x->parent_type(BLUE) == PARENT_VERTEX &&
            ...
            idst &&
                get_edge_parent(*x, *idst, GREEN) != Host_face(0, PARENT_NONE))
          break;
    ...
    for (Feature_1::iterator it_b = it_b_mid, iend = f_b->end();
         ++it_b != iend;) {
      // Loop until the target of the green halfedge projects beyond the
      //    halfedge.
      Halfedge *b = *it_b;
