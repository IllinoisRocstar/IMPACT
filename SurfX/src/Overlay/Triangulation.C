...
static bool do_intersect(const Point_2 &p1, const Point_2 &p2,
  ...
  if (a11 == 0.0 && a12 == 0.0) {
    ...
    if (sqn1 > sqn2) {
      ...
      for (i = 0; i <= 1; i += 2) {
        if (p1[i] != q1[i] || p1[1 + i] != q1[1 + i]) return false;
      }
      return true;
    }
...
bool Triangulation::in_cone(Node_const_iterator u,
                            Node_const_iterator v) const {
...
bool Triangulation::is_diagonalie(Node_const_iterator u,
                                  Node_const_iterator v) const {
  for (Node_const_iterator i = _nodes.begin(); i != _nodes.end(); ++i) {
    Node_const_iterator j = get_next(i);

    if (u != i && u != j && v != i && v != j &&
        do_intersect(get_point(*u), get_point(*v), get_point(*i),
                     get_point(*j)))
      return false;
  }
  return true;
