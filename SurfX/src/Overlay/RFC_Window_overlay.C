...
RFC_Pane_overlay::RFC_Pane_overlay(COM::Pane *b, int color) : Base(b, color) {}

// Constructor and deconstructors
RFC_Window_overlay::RFC_Window_overlay(COM::Window *b, int color,
                                       const char *pre)
    : Base(b, color, MPI_COMM_SELF),
      out_pre(pre ? pre : ""),
...
RFC_Window_overlay::~RFC_Window_overlay() {
  for (int i = _pms.size() - 1; i >= 0; --i) _pms[i] = NULL;
}

HEdge RFC_Window_overlay::get_an_unmarked_halfedge() const {
  HEdge hbrd;

  for (Pane_set::const_iterator pit = _pane_set.begin(); pit != _pane_set.end();
       ++pit) {
...
bool RFC_Window_overlay::is_same_node(const Node &v1, const Node &v2) {
  if (v1 == v2) return true;
  if (!v1.is_border_l() || !v2.is_border_l()) return false;
...
void RFC_Pane_overlay::evaluate_normals() {
...
void RFC_Window_overlay::evaluate_normals() {
  ...
  for (it = _pane_set.begin(); it != iend; ++it) {
    ...
    free_vector(pane._f_nrmls);
    free_vector(pane._f_tngts);
    free_vector(pane._f_n_index);
    free_vector(pane._f_t_index);
    int num_hedgs =
        4 * pane.size_of_faces() +
        pane.size_of_border_edges();  // pane.hds().size_of_halfedges();
  ...
  for (Feature_list_1::iterator it2 = _f_list_1.begin(),
    ...
    for (Feature_1::iterator j = it2->begin(); j != it2->end(); ++j) {
      ...
      do {
        if (!k.is_border_l() &&
            k.pane()->_f_n_index[k.pane()->get_index(k)] == -1) {
...
const Vector_3 &RFC_Pane_overlay::get_normal(HEdge h, const Node &v) const {
  ...
  if (h.id().is_border()) h = h.opposite_g().prev_l();
...
void RFC_Pane_overlay::determine_counterparts() {
    ...
    while (j0 != i0->second.end() && j1 != it->second.end()) {
      if (j0->first == j1->first) {
        ...
        for (k0 = j0; k0 != i0->second.end() && k0->first == j0->first; ++k0) {
          if (found) continue;
          for (k1 = j1; k1 != it->second.end() && k1->first == j1->first;
               ++k1) {
            if (found) continue;
            ...
            map<pair<int, int>, HEdge>::const_iterator b2e_it =
                p._bv2edges.find(e);
...
void RFC_Pane_overlay::construct_bvpair2edge() {
  for (int i = 1, s = size_of_border_edges(); i <= s; ++i) {
    HEdge h = HEdge(this, Edge_ID(i, Edge_ID::BndID)).opposite_l();

    _bv2edges[make_pair(get_lid(h.origin_l()), get_lid(h.destination_l()))] = h;
  }
}

// Determine the counterparts for all pane border edges.
void RFC_Window_overlay::determine_counterparts() {
...
void RFC_Window_overlay::unmark_alledges() {
  Pane_set::iterator it = _pane_set.begin(), iend = _pane_set.end();
  for (; it != iend; ++it) {
...
void RFC_Pane_overlay::delete_overlay_data() {
  ...
  free_vector(_cntrs_hedge);
  free_vector(_primnodes);
  free_vector(_f_nrmls);
  free_vector(_f_tngts);
  free_vector(_f_n_index);
  free_vector(_f_t_index);
  free_vector(_ad_0);
  free_vector(_ea_0);
  free_vector(_fd_1);
}
...
void RFC_Pane_overlay::insert_subface(int idx, int plid, const int *lids,
                                      const ParentEdge_ID *eids,
                                      const Point_2 *nc, int rp_id, int cnt,
                                      const int *rids) {
...
*/
void RFC_Window_overlay::reduce_normals_to_all(MPI_Op op) {
