 ...
 */
void 
Overlay::convert_nat_coordinates( const INode *i, HEdge h, int color, 
				  int pane_id, int &lid, 
  ...
  if ( b.face() != h.face()) {
    ...
    while ( !win.is_same_node( b.origin_l(), h.destination_l())) {
      h = h.next_l(); RFC_assertion( ++count <= 4);
    }
...
number_subfaces() {
  ...
  for ( pi=b_ps.begin(); pi!=b_ps.end(); ++pi) {
	...
	for ( int k=0; k<num_tris; ++k) {
	  ...
	  pane_b->insert_subface(idx_b, pane_b->get_lid( b.face()), 
				 lids_b, eids_b, ncs_b,
				 pane_id_g, idx_g+1, lids_g);
	  pane_g->insert_subface(idx_g, pane_g->get_lid( g.face()), 
				 lids_g, eids_g, ncs_g, 
