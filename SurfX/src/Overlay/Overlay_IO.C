 ...
 * \sa get_subnode_id
 */
void Overlay::set_subnode_id(INode *i, int color, int pane_id, int l_id) {
  if (color == BLUE) {
    int global_id = i->id();
 ...
 * \sa set_subnode_id
 */
int Overlay::get_subnode_id(const INode *i, int color, int pane_id) const {
  if (color == BLUE) {
    int global_id = i->id();
 ...
 */
int Overlay::get_subnode_copies(const INode *i, int color) const {
  if (color == BLUE) {
    int global_id = i->id();
 ...
 * \sa set_subnode_id
 */
void Overlay::convert_nat_coordinates(const INode *i, HEdge h, int color,
                                      int pane_id, int &lid,
                                      RFC_Pane_overlay::ParentEdge_ID &eid,
                                      Point_2 &nc) const {
  ...
  if (b.face() != h.face()) {
    ...
    while (!win.is_same_node(b.origin_l(), h.destination_l())) {
      h = h.next_l();
      RFC_assertion(++count <= 4);
    }
    if (i->parent_type(color) == PARENT_VERTEX)
      eid.edge_id = pane.get_lvid(ene, pane.get_lid(h.destination_l()));
    else {
      eid.edge_id = pane.get_lvid(ene, pane.get_lid(h.origin_l()));
      nc = Point_2(1 - nc[0], 0);
    }
  } else {
    eid.edge_id = pane.get_lvid(ene, pane.get_lid(b.origin_l()));
...
void Overlay::number_a_subnode(INode *i, int color,
                               std::map<int, std::pair<int, int> > &cnts) {
  Parent_type t = i->parent_type(color);
  HEdge h = i->halfedge(color);

  switch (t) {
    case PARENT_FACE: {
      int pane_id = h.pane()->id();
    ...
    default: {
      ...
      do {
        int pane_id = h.pane()->id();
        if (!first_iter &&
            (pane_id == last_id || panes.find(pane_id) != panes.end()))
          continue;
        ...
        if (first_iter && !h.origin_l().is_border_l()) break;
        first_iter = false;
...
void Overlay::count_subnodes(INode *i, int color,
                             std::map<int, std::pair<int, int> > &cnts) {
  if (i->parent_type(color) == PARENT_VERTEX) {
    HEdge h = i->halfedge(color);
    HEdge h0 = h;
...
void Overlay::number_subnodes() {
  ...
  for (std::list<INode *>::const_iterator it = inodes.begin();
       it != inodes.end(); ++it) {
...
void Overlay::number_subfaces() {
  ...
  for (pi = b_ps.begin(); pi != b_ps.end(); ++pi) {
    ...
    for (int i = 1; i <= pane_b->size_of_faces(); ++i) {
      Face fi(pane_b, i);
      ...
      if (ret) {
        std::cerr << "ERROR: Got error code " << ret
                  << " when subdividing face " << i << " with node "
                  << fi.halfedge().origin_l().id() << " in pane "
                  << pane_b->id() << std::endl;
      ...
      if (sub_faces.empty()) {
        std::cerr << "ERROR: Error in enumerating subface in face " << i
                  << " in pane " << pane_b->id() << std::endl;
        RFC_assertion(!sub_faces.empty());
        abort();
      }

      // Count the subfaces in the blue face
      for (Subface_list::iterator si = sub_faces.begin(),
                                  send = sub_faces.end();
           si != send; ++si) {
        int num_tris = si->size() - 2;
        HEdge s = get_parent_face(*si, GREEN);
        RFC_Pane_overlay *pane_g = s.pane();
        if (cnts_it_g->first != pane_g->id())
          cnts_it_g = cnts_g.find(pane_g->id());

        cnts_it_b->second[fi.id() - 1] += num_tris;
        cnts_it_g->second[s.face().id() - 1] += num_tris;
      }
    }
  }
  ...
  for (pi = b_ps.begin(); pi != b_ps.end(); ++pi) {
    ...
    for (int i = 1; i <= (*pi)->size_of_faces(); ++i) {
      Face fi(*pi, i);
      ...
      for (Subface_list::iterator si = sub_faces.begin(),
        ...
        HEdge g = get_parent_face(*si, GREEN);
        ...
        RFC_Pane_overlay *pane_g = g.pane();
        const int pane_id_g = pane_g->id();
        // Insert the triangles into the blue and green panes
        for (int k = 0; k < num_tris; ++k) {
          int lids_b[3], lids_g[3];
          RFC_Pane_overlay::ParentEdge_ID eids_b[3], eids_g[3];
          Point_2 ncs_b[3], ncs_g[3];

          for (int j = 0; j < 3; ++j) {
            COM_assertion_msg((*si)[tris[k][j]], "Empty intersection.");

            convert_nat_coordinates((*si)[tris[k][j]], b, BLUE, pane_id_b,
                                    lids_b[j], eids_b[j], ncs_b[j]);
            convert_nat_coordinates((*si)[tris[k][j]], g, GREEN, pane_id_g,
                                    lids_g[j], eids_g[j], ncs_g[j]);
          }

          if (offsets_it_g->first != pane_g->id())
            offsets_it_g = offsets_g.find(pane_g->id());

          int idx_b = offsets_it_b->second[fi.id() - 1]++;
          int idx_g = offsets_it_g->second[g.face().id() - 1]++;

          pane_b->insert_subface(idx_b, pane_b->get_lid(b.face()), lids_b,
                                 eids_b, ncs_b, pane_id_g, idx_g + 1, lids_g);
          pane_g->insert_subface(idx_g, pane_g->get_lid(g.face()), lids_g,
                                 eids_g, ncs_g, pane_id_b, idx_b + 1, lids_b);
        }
      }
    }
  }
...
void Overlay::export_windows(RFC_Window_base *bw, RFC_Window_base *gw) {
