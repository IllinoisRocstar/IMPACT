 ...
 */
float 
RFC_Window_overlay::cos_face_angle( const HEdge &h) {
 ...
 */
float 
RFC_Window_overlay::cos_face_angle( const HEdge &h, const HEdge &hopp) {
  // Precondition: Face angles are initialized to HUGE_VALF.
  RFC_Pane_overlay *p1=h.pane();
  float &t = p1->get_cos_face_angle(h);
  if ( t<HUGE_VALF) return t;
...
cos_edge_angle( const HEdge &h1, const HEdge &h2) {
 ...
 */
float 
RFC_Window_overlay::comp_angle_defect( const Node &v) {
 ...
 */
bool
RFC_Window_overlay::is_strong_ad( const Node &v) {
 ...
 */
void
RFC_Window_overlay::merge_features_1(const Node &v, 
                                     Feature_1 &f1, Feature_1 &f2) {
  ...
  if ( (dst=f1.back().destination_g()) == f2.front().origin_g() && v==dst)
    f1.splice( f1.end(), f2);
  else if ( (src=f1.front().origin_g()) == f2.back().destination_g() && v==src) {
    f2.splice( f2.end(), f1);
    f1.swap( f2);
  }
...
RFC_Window_overlay::check_false_strong_1( Feature_1 &f1) {
  ...
  if ( _long_falseness_check) {
    ...
    for ( Feature_1::const_iterator it=++f1.begin(); it!=f1.end(); ++it) {
      ...
      // Loop through incident edges of v
      do {
	if ( is_on_feature( h1.origin_g())) { v=Node(); break; }
      } while ( (h1=acc.get_next_around_destination(h1))!=h0);
      if (v.pane()==NULL) continue;
      else return false;
    }
    return true;
  }
...
subdiv_feature_curve( const Feature_1 &f1, 
  ...
  if ( isdangling && divs.empty() ) {
    if ( hfirst!=f1end && cos_min_fa_before>_cos_weakend &&
	 _f0_ranks.find( src) == _f0_ranks.end() ) 
    { divs.push_back( hfirst); set_feature_0( hfirst->origin_g()); }

    if ( hlast!=f1end && hlast!=hfirst && cos_min_fa_after>_cos_weakend &&
	 _f0_ranks.find( trg) == _f0_ranks.end()) 
    { divs.push_back( hlast); set_feature_0( hlast->origin_g()); }
  }
  ...
  if ( isloop) {
    ...
    if ( cos_ea <= _cos_ue || 
	 is_rstrong_ea( f1,f1end,++f1.begin(),cos_ea,isloop)) {
      // When breaking loops, mark breakpoints as corners
      divs.push_back( f1end); set_feature_0( hi->origin_g());
    }
  }
  else
  ...
  for (Feature_list_1::iterator sit=subcur.begin();sit!=subcur.end();++sit) {
	...
	i->pane()->unset_strong_edge(*i); ++dropped;
...
RFC_Window_overlay::remove_feature_1( Feature_1 &f) {
  ...
  for ( Feature_1::iterator it=f.begin(), iend=f.end(); it != iend; ++it) {
    unset_strong_edge( *it);  unset_strong_edge( it->opposite_g());
  }
...
RFC_Window_overlay::identify_features_0() {
  ...
  // Fill 0-feature list
  for ( map<Node,int>::iterator it=_f0_ranks.begin(); 
	it!=_f0_ranks.end(); ++it) {
...
RFC_Window_overlay::detect_features() {
      ...
      do {
        ...
        if ( lopp.is_border_l() || h.id()  < lopp.id()) {
          ...
          if ( pane.id() <= hopp.pane()->id()) {
            ...
            if ( d < _cos_uf) 
              tstrong_edges.push_back( make_pair(d,h));
          }
        }
      } while ( (h=h.next_l()) != h0);
    }
  ...
  vector<std::pair<float,HEdge> > iedges; iedges.reserve(16);
  _f_list_1.clear();
  ...
  t0=get_wtime(); 
  for ( vector< pair<float,HEdge> >::iterator
    ...
    h.pane()->set_strong_edge(h);  acc.mark( h); 
    hopp.pane()->set_strong_edge(hopp); acc.mark( hopp);
	...
	rstrong_edges.push_back(make_pair(cos_face_angle(cos_max.second),
					  cos_max.second));
	...
	acc.mark( h);    h.pane()->set_strong_edge( h);
	acc.mark( hopp); hopp.pane()->set_strong_edge( hopp);
	if (c==0) {
	  f1.push_back( h); 
	  dst = h.destination_g(); 
	}
