...
void Rocface::set_verbose(int *verb) {
  RFC_assertion_msg(verb, "NULL pointer");
  _ctrl.verb = *verb;
}

// Associate two windows given by a1->window() and a2->window().
void Rocface::overlay(const COM::DataItem *a1, const COM::DataItem *a2,
                      const MPI_Comm *comm, const char *path) {
  ...
  if (it1 != _trs_windows.end()) {
    ...
    it1 = _trs_windows.insert(TRS_Windows::value_type(wn1, NULL)).first;
    RFC_assertion(it2 == _trs_windows.end());
    it2 = _trs_windows.insert(TRS_Windows::value_type(wn2, NULL)).first;
  }
...
void Rocface::clear_overlay(const char *m1, const char *m2) {
  COM_assertion_msg(validate_object() == 0, "Invalid object");
  ...
  if (it1 != _trs_windows.end()) {
    delete it1->second;
    _trs_windows.erase(it1);

    TRS_Windows::iterator it2 = _trs_windows.find(wn2);
    RFC_assertion(it2 != _trs_windows.end());
    delete it2->second;
    _trs_windows.erase(it2);
  } else {
...
void Rocface::read_overlay(const COM::DataItem *a1, const COM::DataItem *a2,
                           const MPI_Comm *comm, const char *prefix1,
                           const char *prefix2, const char *format) {
  COM_assertion_msg(validate_object() == 0, "Invalid object");
  ...
  if (it1->second->comm_rank() == 0 && _ctrl.verb) {
    std::cout << "SurfX: Reading in subdivision of window " << n1
              << " from files with prefix \"" << prefix1 << "\"...."
              << std::flush;
  }
  ...
  if (it2->second->comm_rank() == 0 && _ctrl.verb) {
    std::cout << "SurfX: Reading in subdivision of window " << n2
              << " from files with prefix \"" << prefix2 << "\"...."
              << std::flush;
  }
...
void Rocface::write_overlay(const COM::DataItem *a1, const COM::DataItem *a2,
  ...
  if (it1->second->comm_rank() == 0 && _ctrl.verb) {
    std::cout << "SurfX: Writing subdivision of window \"" << n1 << "\"...."
              << std::flush;
  }
  ...
  if (it2->second->comm_rank() == 0 && _ctrl.verb) {
    std::cout << "SurfX: Writing subdivision of window \"" << n2 << "\"...."
              << std::flush;
  }
...
const RFC_Window_transfer *Rocface::get_transfer_window(
    const COM::DataItem *attr) {
  return _trs_windows.find(attr->window()->name())->second;
}
RFC_Window_transfer *Rocface::get_transfer_window(COM::DataItem *attr) {
  return _trs_windows.find(attr->window()->name())->second;
}

void Rocface::set_tags(const COM::DataItem *src, const COM::DataItem *trg,
  ...
  if (it2 == _trs_windows.end()) {
    std::cerr << "SurfX::ERROR: The overlay of window \"" << n1
              << "\" and window \"" << n2 << "\" does not exist" << std::endl;
    RFC_assertion(false);
    MPI_Abort(MPI_COMM_WORLD, -1);
  }
...
class Transfer_traits {
 public:
   ...
   */
  static void transfer(Transfer_type &trans, Source_type &sf, Target_type &tf,
                       const Real alpha, const int order, Real *tol, int *iter,
...
class Transfer_traits<Nodal_data_const, Nodal_data, false> {
 public:
  ...
  static void transfer(Transfer_type &trans, Nodal_data_const &sf,
                       Nodal_data &tf, const Real alpha, const int order,
                       Real *tol, int *iter, const int verbose, const bool) {
...
class Transfer_traits<Nodal_data_const, Facial_data, true> {
 public:
  ...
  static void transfer(Transfer_type &trans, Nodal_data_const &sf,
                       Facial_data &tf, const Real alpha, const int order,
                       Real *tol, int *iter, const int verbose, const bool) {
...
class Transfer_traits<Facial_data_const, Facial_data, true> {
 public:
  ...
  static void transfer(Transfer_type &trans, Facial_data_const &sf,
                       Facial_data &tf, const Real alpha, const int order,
                       Real *tol, int *iter, const int verbose, const bool) {
    trans.transfer(sf, tf, alpha, order, verbose);
  }
...
class Transfer_traits<Facial_data_const, Nodal_data, true> {
 public:
  ...
  static void transfer(Transfer_type &trans, Facial_data_const &sf,
                       Nodal_data &tf, const Real alpha, const int order,
                       Real *tol, int *iter, const int verbose,
                       const bool load) {
    if (!load)
...
void Rocface::transfer(const COM::DataItem *src, COM::DataItem *trg,
  ...
  if (_ctrl.verb) {
    if (w2->comm_rank() == 0) {
      ...
      std::cout << " from " << w1->name() + "." + src->name() << " to "
                << w2->name() + "." + trg->name() << std::endl;
    }
...
void Rocface::least_squares_transfer(const COM::DataItem *src,
                                     COM::DataItem *trg, const Real *alp_in,
                                     const int *ord_in, Real *tol_io,
                                     int *iter_io) {
  COM_assertion_msg(validate_object() == 0, "Invalid object");
  ...
  if (trg->is_nodal()) {
    ...
    if (src->is_nodal()) {
      transfer<Nodal_data_const, Nodal_data, true>(src, trg, alpha, order, &tol,
                                                   &iter);
    } else {
      transfer<Facial_data_const, Nodal_data, true>(src, trg, alpha, order,
                                                    &tol, &iter);
    }
    ...
    if (src->is_nodal()) {
      transfer<Nodal_data_const, Facial_data, true>(src, trg, alpha, order);
    } else {
      transfer<Facial_data_const, Facial_data, true>(src, trg, alpha, order);
    }
  }
}

// Transfer data from a window to another using the traditional interpolation.
void Rocface::interpolate(const COM::DataItem *src, COM::DataItem *trg) {
  COM_assertion_msg(validate_object() == 0, "Invalid object");

  RFC_assertion(trg->is_nodal() && src->is_nodal());

  transfer<Nodal_data_const, Nodal_data, false>(src, trg, 1.);
}

// Transfer data from a window to another using load transfer.
void Rocface::load_transfer(const COM::DataItem *src, COM::DataItem *trg,
                            ...
                            const int *_o)   // Order of integration
{
  ...
  const Real alpha =
      (_a == NULL) ? -1. : *_a;              // Alpha for geometry interpolation
  const int order = (_o == NULL) ? 1 : *_o;  // Order of integration

  if (src->is_nodal()) {
    transfer<Nodal_data_const, Nodal_data, true>(src, trg, alpha, order, NULL,
                                                 NULL, true);
  } else {
    transfer<Facial_data_const, Nodal_data, true>(src, trg, alpha, order, NULL,
                                                  NULL, true);
  }
}
...
void Rocface::init(const std::string &mname) {
  ...
  COM_set_member_function((mname + ".overlay").c_str(),
                          (Member_func_ptr)(&Rocface::overlay), glb.c_str(),
                          "biiII", types);
  ...
  COM_set_member_function((mname + ".set_tags").c_str(),
                          (Member_func_ptr)(&Rocface::set_tags), glb.c_str(),
                          "biii", types);
  ...
  COM_set_member_function((mname + ".interpolate").c_str(),
                          (Member_func_ptr)(&Rocface::interpolate), glb.c_str(),
                          "bio", types);
  ...
  COM_set_member_function((mname + ".set_verbose").c_str(),
                          (Member_func_ptr)(&Rocface::set_verbose), glb.c_str(),
                          "bi", types);
...
void Rocface::read_control_file(const char *fname) {
  ...
  // Set verbosity level
  COM_new_dataitem((ctrlname + ".verbosity").c_str(), 'w', COM_INT, 1, "");
  COM_set_array((ctrlname + ".verbosity").c_str(), 0, &_ctrl.verb);

  // Set snap threshold
  COM_new_dataitem((ctrlname + ".snap_tolerance").c_str(), 'w', COM_DOUBLE, 1,
                   "");
...
// C/C++ binding
extern "C" void SurfX_load_module(const char *name) {
  Rocface::init(std::string(name));
}
extern "C" void SurfX_unload_module(const char *name) {
  Rocface::finalize(std::string(name));
}

// Fortran binding
extern "C" void surfx_load_module(const char *name, long int length) {
  Rocface::init(std::string(name, length));
}
extern "C" void surfx_unload_module(const char *name, long int length) {
...
extern "C" void surfx_load_module_(const char *name, long int length) {
  Rocface::init(std::string(name, length));
}
extern "C" void surfx_unload_module_(const char *name, long int length) {
  Rocface::finalize(std::string(name, length));
}
