...
class Bbox_3 {
 public:
  ...
  Bbox_3(double x_min, double y_min, double z_min, double x_max, double y_max,
         double z_max) {
    _min[0] = x_min;
    _min[1] = y_min;
    _min[2] = z_min;
...
inline std::ostream& operator<<(std::ostream& os, const Bbox_3& b) {
  return os << b.xmin() << ' ' << b.ymin() << ' ' << b.zmin() << ' ' << b.xmax()
            << ' ' << b.ymax() << ' ' << b.zmax();
}

inline std::istream& operator>>(std::istream& is, Bbox_3& b) {
...
inline bool do_overlap_strict(const Bbox_3& bb1, const Bbox_3& bb2) {
  // check for emptiness ??
  if (bb1.xmax() <= bb2.xmin() || bb2.xmax() <= bb1.xmin()) return false;
  if (bb1.ymax() <= bb2.ymin() || bb2.ymax() <= bb1.ymin()) return false;
  if (bb1.zmax() <= bb2.zmin() || bb2.zmax() <= bb1.zmin()) return false;
  return true;
}
