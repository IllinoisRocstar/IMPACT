...
/*!
  ...
  nodal coordinates, element connectivity, and dataitems of a pane.
...
class RFC_Pane_base {
  ...
  */
  struct ParentEdge_ID {
    //! A dummy constructor.
    ParentEdge_ID() {}
    //! Constructor from two integers.
    /*!
      \param f local face id.
      \param e edge id within a face
    */
    ParentEdge_ID(int f, int e) : face_id(f), edge_id(e) {}

    int face_id;  //!< local face id.
    int edge_id;  //!< edge id within the face.

    //! Less-than operator.
    bool operator<(const ParentEdge_ID &v) const {
  ...
  ParentEdge_ID get_edge_id(const int face_lid, const int vertex_lid) const;
  ...
  //! Get the address of a given dataitem with id i.
  Real *pointer(int i) { return (Real *)_base->dataitem(i)->pointer(); }
  //! \overload
  const Real *pointer(int i) const {
    return (Real *)_base->dataitem(i)->pointer();
  }
  ...
  const Point_3 &get_point(int id) const {
    ...
    return *reinterpret_cast<const Point_3 *>(_base->coordinates() +
                                              3 * (id - 1));
  }
    ...
    if (e == 3) {
      switch (idx) {
        case 0:
          return;
        case 1:
          nc = Point(1. - nc[0] - nc[1], nc[0]);
          return;
        case 2:
          nc = Point(nc[1], 1. - nc[0] - nc[1]);
          return;
        default:
          RFC_assertion(false);
      }
      ...
      switch (idx) {
        case 0:
          return;
        case 1:
          nc = Point(1 - nc[1], nc[0]);
          return;
        case 2:
          nc = Point(1 - nc[0], 1 - nc[1]);
          return;
        case 3:
          nc = Point(nc[1], 1 - nc[0]);
          return;
        default:
          RFC_assertion(false);
      }
  ...
  void get_host_element_of_subnode(int i, Element_node_enumerator &ene,
                                   Point_2 &nc) const {
  ...
  void get_nat_coor_in_element(const int eid, const int lid,
                               Point_2 &nc) const {
  ...
  //! within the element.
  void get_nat_coor_in_element(const int sn_id, Element_node_enumerator &ene,
                               Point_2 &nc) const;
 ...
 protected:
  ...
  class COM_Pane_friend : protected COM::Pane {
   ...
   public:
    ...
    using COM::Pane::dataitem;
    using COM::Pane::reinit_conn;
    using COM::Pane::set_size;
  };

  //=========== IO Functions
  void write_tec_ij(std::ostream &, const COM::DataItem *a = 0) const;
  void write_tec_tri(std::ostream &, const COM::Connectivity &,
                     const COM::DataItem *a = 0) const;
  void write_tec_mixed(std::ostream &,
                       const std::vector<const COM::Connectivity *> &,
                       const COM::DataItem *a = 0) const;
  void write_tec_sub(std::ostream &) const;
  void write_tec_ascii(std::ostream &os, const COM::DataItem *attr = 0) const;

  // Write in native binary format
  void write_binary(std::ostream &os) const;

  // Write in OFF binary format
  void write_off(std::FILE *fhandle) const;

  // Read in native binary format.
  void read_binary(std::istream &is, std::vector<int> *b2v_all = NULL,
                   COM::Pane *p = NULL);

  // Register the dataitems for the pane for output using Rocout.
  void register_sdv_dataitems(const std::string &wname) const;
  ...
  void read_rocin(const std::string &sdv_wname,
                  const std::string &parent_wname = "", COM::Pane *p = NULL);
 ...
 protected:
  ...
  std::vector<ParentEdge_ID> _subnode_parents;
  //!< Edge ids of parents.
...
class RFC_Window_base {
 public:
  ...
  //! Retrieve an dataitem object from the base using the dataitem name.
  const DataItem *dataitem(const char *f) const { return _base->dataitem(f); }
  ...
  //! New dataitems
  void new_sdv_dataitems(const std::string &wname) const;
...
class RFC_Window_derived : public RFC_Window_base {
 ...
 public:
  ...
  //! Get a handle to the primary copy of a node contained in the pane \p p with
  //! local id \p vid.
...
template <class _Pane>
void RFC_Window_derived<_Pane>::init() {
  std::vector<COM::Pane *> ps;
  _base->panes(ps);

  for (std::vector<COM::Pane *>::const_iterator it = ps.begin(); it != ps.end();
       ++it) {
...
template <class _Pane>
const _Pane &RFC_Window_derived<_Pane>::pane(const int pid) const {
  Pane_set::const_iterator pit = _pane_set.find(pid);
  RFC_assertion(pit != _pane_set.end());
...
template <class _Pane>
_Pane &RFC_Window_derived<_Pane>::pane(const int pid) {
  Pane_set::iterator pit = _pane_set.find(pid);
  RFC_assertion(pit != _pane_set.end());
...
template <class _Pane>
void RFC_Window_derived<_Pane>::panes(std::vector<Pane *> &ps) {
  const_cast<Self *>(this)->panes((std::vector<const Pane *> &)ps);
}

template <class _Pane>
void RFC_Window_derived<_Pane>::panes(std::vector<const Pane *> &ps) const {
  ps.resize(0);
...
template <class _Pane>
std::pair<_Pane *, int> RFC_Window_derived<_Pane>::get_primary(Pane *p,
                                                               int vid) {
...
template <class _Pane>
std::pair<const _Pane *, int> RFC_Window_derived<_Pane>::get_primary(
    const Pane *p, int vid) const {
