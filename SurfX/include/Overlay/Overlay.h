...
protected:
  ...
  void get_green_parent( const Node &v,
			 const HEdge &b,
			 HEdge *o,
			 Parent_type  *t,
			 Point_2 *nc);
  ...
  void
  project_adjacent_green_vertices( const INode *, const HEdge &);
  ...
  bool contains( const HEdge &e1, const Parent_type t1,
...
private: // Data and functions for numering the subnodes and subfaces.
  ...
  void convert_nat_coordinates( const INode *i, HEdge h, int color, 
				int pane_id, int &lid, 
...
private:
  ...
  void insert_edge_into_queue( const HEdge &h, INode *v, 
			       std::queue<HEdge>   &q, 
			       std::queue<HEdge>   &q_rdg, 
			       std::queue<HEdge>   &q_crn);
  
  bool is_queue_empty(  std::queue<HEdge>   &q, 
			std::queue<HEdge>   &q_rdg, 
			std::queue<HEdge>   &q_crn);
