...
class RFC_Pane_overlay : public RFC_Pane_base,
                         protected SURF::Pane_manifold_2 {
public:
  ...
  int get_index( const Edge_ID &id) const { 
    if (id.is_border()) return size_of_faces()*4+id.eid()-1;
    else return (id.eid()-1)*4+id.lid(); 
  }
  ...
  { RFC_assertion(i>0); return Node(this,i); }
  ...
  const Node get_primary( const Node &v) const { 
    if ( !v.is_border_l()) return v;
    else return _primnodes[get_border_index(v.halfedge_l())];
  }
  ...
  HEdge get_counterpart( const HEdge &h) {
    if ( !h.id().is_border()) return h;
    else return get_counterpart( h.id());
  }
  ...
  { return get_normal( h, const_cast<RFC_Pane_overlay*>(this)->get_node_from_id(i)); }
  ...
  { assert(v.is_primary()); return _is_on_f[ get_index(v)]; }
  bool is_feature_0( const Node &v) const 
  { assert(v.is_primary()); return _is_f_0[ get_index(v)]; }
...
protected:
  ...
  { _fd_1.clear(); int n = 4*size_of_faces() + size_of_border_edges();
    _fd_1.resize( n, HUGE_VALF); }

  const float& get_angle_defect( const Node &v) const 
  { return _ad_0[ get_index( v)]; }
  float& get_angle_defect( const Node &v)
  { return _ad_0[ get_index( v)]; }
  void init_angle_defect() 
  { _ad_0.clear(); _ad_0.resize( size_of_nodes(), HUGE_VALF); }

  const float& get_cos_edge_angle( const Node &v) const 
  { return _ea_0[ get_index( v)]; }
  float& get_cos_edge_angle( const Node &v) 
  { return _ea_0[ get_index( v)]; }
  void reset_cos_edge_angle( const Node &v) 
  { _ea_0[ get_index( v)] = HUGE_VALF; }
  void init_edge_angle() 
  { _ea_0.clear(); _ea_0.resize( size_of_nodes(), HUGE_VALF); }
  ...
  { assert(h.id().is_border()); return h.id().eid()-1; }
...
private:
  ...
  { assert(v.is_primary()); v.pane()->set_feature_0( v); }
  void set_on_feature( const Node &v) const 
  { assert(v.is_primary()); v.pane()->set_on_feature( v); }

  bool is_feature_0( const Node &v) const 
  { assert(v.is_primary()); return v.pane()->is_feature_0(v); }
  bool is_on_feature( const Node &v) const 
  { assert(v.is_primary()); return v.pane()->is_on_feature(v); }
  bool is_feature_1( const HEdge &h) const 
  { return h.pane()->is_feature_1(h); }
  void unset_strong_edge( const HEdge &h) 
...
const Point_3 &Node::point() const 
{ return _pm->get_point(_vID); }
...
HEdge HEdge::opposite_g() const
{ return _pm->get_opposite_edge( _eID); }

HEdge HEdge::prev_l() const 
{ return HEdge(_pm, _pm->get_prev_real_edge( _eID)); }

HEdge HEdge::prev_g() const 
{ return _pm->get_prev_edge( _eID); }

HEdge HEdge::next_l() const 
{ return HEdge( _pm, _pm->get_next_real_edge( _eID)); }

HEdge HEdge::next_g() const 
{ return _pm->get_next_edge( _eID); }

Node  HEdge::origin_g() const
{ Node n( _pm, _pm->get_origin( _eID)); return _pm->get_primary( n); }

Node  HEdge::origin_l() const
{ return Node( _pm, _pm->get_origin( _eID)); }

Node  HEdge::destination_g() const
{ Node n( _pm, _pm->get_destination( _eID)); return _pm->get_primary(n); }
...
{ 
  if (_eID.is_border()) { return Face(); }
  else { return Face(_pm, _eID.eid()); }
}

bool HEdge::is_border_l() const 
{ return _eID.is_border(); }

bool HEdge::is_border_g() const 
