...
class Transfer_base {
 public:
  ...
  typedef std::vector<const RFC_Pane_transfer *>::const_iterator
      Pane_const_iterator;
 ...
 public:
   ...
   */
  template <class _SDF>
  void transfer_2f(const _SDF &sDF, Facial_data &tDF, const Real alpha, int doa,
                   bool verb);
 ...
 protected:
  ...
  void integrate_subface(const RFC_Pane_transfer *p_src,
                         RFC_Pane_transfer *p_trg, const _SDF &sDF,
                         ENE &ene_src, ENE &ene_trg, int sfid_src, int sfid_trg,
                         const Real alpha, Facial_data &tDF, Facial_data &tBF,
                         int doa);
   ...
   */
  template <class _SDF>
  void init_load_vector(const _SDF &vS, const Real alpha, Nodal_data &ld,
                        Nodal_data &diag, int doa, bool lump);

  // This is a matrix-free solver that solves the equation M*x=ld,
  // where M is the mass matrix computed on the fly.
  int pcg(Nodal_data &x, Nodal_data &b, Nodal_data &p, Nodal_data &q,
          Nodal_data &r, Nodal_data &s, Nodal_data &z, Nodal_data &di,
  ...
  void precondition_Jacobi(const Nodal_data_const &rhs,
                           const Nodal_data_const &diag, Nodal_data &x);
  ...
  void saxpy(const Real &a, const Nodal_data_const &x, const Real &b,
             Nodal_data &y);
  ...
  template <class _SDF>
  void compute_load_vector_wra(
      const RFC_Pane_transfer *p_src,  //< Source pane
  ...
  void element_load_vector(
      const _Data &data_s,         //< Data field
  ...
  void compute_load_prime(const Generic_element &e_s, const _Data &data_s,
                          const Vector_3 *grads_s, Vector_3 *load_prime,
                          Tag_nodal) {
 ...
 public:
  // Useful utilities
  void minmax(const RFC_Window_transfer &win, const Facial_data_const &sDF,
              Array_n &min_v, Array_n &max_v);

  void minmax(const RFC_Window_transfer &win, const Nodal_data_const &sDF,
              Array_n &min_v, Array_n &max_v);

  void integrate(const RFC_Window_transfer &win, const Nodal_data_const &sDF,
                 Array_n &intergral, const int doa);

  void integrate(const RFC_Window_transfer &win, const Facial_data_const &sDF,
                 Array_n &intergral, const int doa);
 ...
 private:
  ...
  void interpolate(const Generic_element &e, const Element_var_const values,
                   const Generic_element::Nat_coor &nc, _Value &v) {
    e.interpolate(values, nc, &v);
  }

  template <class _Value>
  void interpolate(const Generic_element &e, const Array_n_const value,
                   const Generic_element::Nat_coor &nc, _Value &v) {
    v = value;
  }
 ...
 private:
  ...
  const Array_n_const make_field(const Facial_data_const &d,
                                 const RFC_Pane_transfer *pn, int i) {
