...
class Transfer_n2n : public Transfer_base {
 ...
 public:
  //! Constructor.
  Transfer_n2n(RFC_Window_transfer *s, RFC_Window_transfer *t) : Base(s, t) {}
   ...
   */
  void transfer(const Nodal_data_const &sf, Nodal_data &tf, const Real alpha,
                Real *t, int *iter, int doa, bool ver);
   ...
   *  \param verb  Verbose level
   */
  void comp_loads(const Nodal_data_const &sf, Nodal_data &tf, const Real alpha,
                  const int doa, bool verb);
};
...
class Transfer_f2n : public Transfer_base {
 ...
 public:
  //! Constructor.
  Transfer_f2n(RFC_Window_transfer *s, RFC_Window_transfer *t) : Base(s, t) {}

  /** The main entry to the data transfer algorithm.
   *  \see Transfer_n2n::transfer
   */
  void transfer(const Facial_data_const &sf, Nodal_data &tf, const Real alpha,
                Real *tol, int *iter, int doa, bool verb);

  /** Compute the nodal load vector
   *  \see Transfer_n2n::comp_loads
   */
  void comp_loads(const Facial_data_const &sf, Nodal_data &tf, const Real alpha,
...
class Interpolator : public Transfer_base {
 ...
 public:
  //! Constructor.
  Interpolator(RFC_Window_transfer *s, RFC_Window_transfer *t) : Base(s, t) {}
