...
//    extra dataitem for storing buffers for data transfer.
...
class RFC_Window_transfer : public RFC_Window_derived<RFC_Pane_transfer> {
 public:
  ...
  RFC_Window_transfer(COM::Window *b, int color, MPI_Comm com,
                      const char *pre = NULL, const char *format = NULL);
  ...
  void set_tags(const COM::DataItem *tag);
  ...
  void wait_any(int n, MPI_Request *requests, int *index,
                MPI_Status *stat = NULL);

  void allreduce(Array_n &arr, MPI_Op op) const {
    allreduce(arr.begin(), arr.end() - arr.begin(), op);
  }
  void allreduce(Real *x, MPI_Op op) const { allreduce(x, 1, op); }
...
class RFC_Data_const {
 public:
  ...
  RFC_Data_const(const RFC_Window_transfer &win, const char *aname) {
    const COM::DataItem *a = win.dataitem(aname);
    _id = a->id();
    _dim = a->size_of_components();
  }
  RFC_Data_const(const COM::DataItem *a)
      : _id(a->id()), _dim(a->size_of_components()) {}
  RFC_Data_const(int i, int d) : _id(i), _dim(d) {}

  int id() const { return _id; }
  Size dimension() const { return _dim; }
  bool is_valid(const RFC_Pane_transfer *p, int i) const { return true; }
...
class RFC_Data : public RFC_Data_const<_Tag> {
 public:
  ...
  RFC_Data(COM::DataItem *a) : Base(a) {}
