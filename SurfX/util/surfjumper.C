...
void read_file(const char *fname, const string &wname, double alpha,
  ...
  if (with_ghost)
    ...
    COM_clone_dataitem((wname + ".all").c_str(), (bufwin + ".all").c_str(), 0);
...
int main(int argc, char *argv[]) {
  ...
  if (!rank) {
    if (argc < 3) {
      std::cerr << "Usage: " << argv[0]
                << " <src Rocin control> <targ Rocin control> [<out_prefix>] "
                   "[<RocfaceControlFile>]\n\n"
                << "\t<src Rocin control>  specifies the Rocin control file "
                   "for the old window\n"
                << "\t<targ Rocin control> specifies the Rocin control file "
                   "for the new window\n"
                ...
                << "\t<RocfaceControlFile> specifies a file name for Rocface "
                   "control parameters. \n"
                << "\nExample:\t" << argv[0]
                << " Rocflo/Rocin/\"ifluid*.txt\" "
                   "Rocfrac/Rocin/\"isolid*.txt\" Rocman/RocfloRocfrac/"
                << "\n\t\t" << std::endl;
      exit(-1);
    }
  }
  ...
  if (argc > 4) {
    if (!rank) std::cout << "Reading Rocface control file..." << std::endl;
  ...
  for (int k = 0; k < 3; ++k) {
    ...
    if (n0 == std::string::npos)
      ...
      fname = fnames[k].substr(n0 + 1, fnames[k].size());
  ...
  int srcmesh = COM_get_dataitem_handle((wnames[0] + ".mesh").c_str());
  int trgmesh = COM_get_dataitem_handle((wnames[1] + ".mesh").c_str());
  std::vector<int> pane_id;

  if (!rank) std::cout << "Reading mesh overlay..." << std::endl;
  ...
  for (int i = 0; i < num_dataitems; i++) {
    ...
    if ((loc == 'e' || loc == 'n') && comtype == COM_DOUBLE) {
      ...
      COM_new_dataitem((trgwin + "." + aname).c_str(), (char)loc, COM_DOUBLE,
                       (int)ncomp, unit.c_str());
      COM_new_dataitem((r_trgwin + "." + aname).c_str(), (char)loc, COM_DOUBLE,
                       (int)ncomp, unit.c_str());
      ...
      for (int p = 0; p < npanes; p++) {
        ...
        int trg_std = 0;
        int trg_cap = 0;
        COM_get_array((trgwin + "." + aname).c_str(), pane_id[p], &src_ptr,
                      &src_std, &src_cap);
        //	COM_assertion_msg((src_ptr && (src_std*src_cap >= 0)),"Source
        //array missing or of 0 size.");
        COM_get_array((r_trgwin + "." + aname).c_str(), pane_id[p], &trg_ptr,
                      &trg_std, &trg_cap);
        //	COM_assertion_msg((trg_ptr && (trg_std*trg_cap >= 0)),"Target
        //array missing or of 0 size.");
  ...
  Ouf << "@Proc: " << rank << endl
      << "@Files: "
      << "new_surf_" << setw(5) << setfill('0') << rank + 1 << ".hdf" << endl;
  Ouf.clear();
  Ouf << "@Panes: ";
