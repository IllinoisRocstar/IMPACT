...
void Rocblas::calcDot(void *yout, const DataItem *x, const DataItem *z,
                      const MPI_Comm *comm, const DataItem *mults) {
  COM_assertion_msg(!x->is_windowed() && !z->is_windowed(),
                    "Unsupported dataitem type");
  ...
  if (ytype == BLAS_VOID) {
    ...
    *yval = data_type(0);
    ...
    y = reinterpret_cast<DataItem *>(yout);
  ...
  // Initialize y to 0 for BLAS_VOID or window dataitem
  if (ytype == BLAS_VOID) {
    *yval = data_type(0);
  } else {
    if (y->is_windowed()) {
  ...
  for (zit = zpanes.begin(), zend = zpanes.end(), xit = xpanes.begin();
    ...
    const DataItem *pz = (*zit)->dataitem(z->id());
    ...
    if (ytype != BLAS_VOID && !y->is_windowed()) {
      ...
      if (py->stride() == ynum_comp) {
        ...
        for (int i = 0; i < ynum_comp; ++i) {
          DataItem *py_i =
              ynum_comp > 1 ? (*yit)->dataitem(y->id() + i + 1) : py;
    ...
    if (mults != NULL) {
      const DataItem *pm = (*mit)->dataitem(mults->id());
...
void Rocblas::dot_MPI(const DataItem *x, const DataItem *y, DataItem *z,
                      const MPI_Comm *comm, const DataItem *mults) {
  COM_Type att_type = x->data_type();
...
void Rocblas::dot_scalar_MPI(const DataItem *x, const DataItem *y, void *z,
                             const MPI_Comm *comm, const DataItem *mults) {
  COM_Type att_type = x->data_type();
...
/// Wrapper for 2-norm.
void Rocblas::dot(const DataItem *x, const DataItem *y, DataItem *z,
                  const DataItem *mults) {
  dot_MPI(x, y, z, NULL, mults);
}

void Rocblas::dot_scalar(const DataItem *x, const DataItem *y, void *z,
                         const DataItem *mults) {
  dot_scalar_MPI(x, y, z, NULL, mults);
}

/// Wrapper for 2-norm.
void Rocblas::nrm2(const DataItem *x, DataItem *y, const DataItem *mults) {
  dot_MPI(x, x, y, NULL, mults);
}

/// Wrapper for 2-norm with y as a scalar pointer.
void Rocblas::nrm2_scalar(const DataItem *x, void *y, const DataItem *mults) {
  dot_scalar_MPI(x, x, y, NULL, mults);
}

/// Wrapper for 2-norm with MPI.
void Rocblas::nrm2_MPI(const DataItem *x, DataItem *y, const MPI_Comm *comm,
                       const DataItem *mults) {
  dot_MPI(x, x, y, comm, mults);
}

/// Wrapper for 2-norm with y as a scalar pointer with MPI.
void Rocblas::nrm2_scalar_MPI(const DataItem *x, void *y,  //
                              const MPI_Comm *comm, const DataItem *mults) {
