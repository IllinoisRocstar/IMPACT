...
void Rocblas::axpy_gen( const void *ain, const DataItem *x,
			const DataItem *y, DataItem *z) {
  ...
  for(zit=zpanes.begin(),zend=zpanes.end(),xit=xpanes.begin(),yit=ypanes.begin(); 
    ...
    const DataItem *py = (*yit)->dataitem(y->id());
    ...
    const DataItem *pa = (atype!=BLAS_VOID&&ait)?(*ait)->dataitem(a->id()):a;
    ...
    if ( !xstg && !zstg && !ystg && !astg && atype != BLAS_VEC 
	 && (atype != BLAS_SCNE || num_dims==1)) {
      ...
      // Get address for a if a is not window dataitem
	...
	*zval = getref<data_type,atype,0>(aval,i,0,1)* *xval+ *yval;
	...
	const DataItem *py_i = num_dims==1?py:(*yit)->dataitem(y->id()+i+1);
	const data_type *yval = (const data_type *)py_i->pointer();
	ystrd=get_stride<BLAS_VEC2D>(py_i);
	  
	if ( atype != BLAS_VOID && ait) {
	  const DataItem *pa_i=anum_dims==1?pa:(*ait)->dataitem(a->id()+i+1);
...
void Rocblas::axpy( const DataItem *a, const DataItem *x, const DataItem *y,
		   DataItem *z) {
...
void Rocblas::axpy_scalar( const void *a, const DataItem *x, const DataItem *y,
			   DataItem *z) {
  COM_Type att_type = z->data_type();
