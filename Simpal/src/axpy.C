...
void Rocblas::axpy_gen(const void *ain, const DataItem *x, const DataItem *y,
                       DataItem *z) {
  ...
  for (zit = zpanes.begin(), zend = zpanes.end(), xit = xpanes.begin(),
    ...
    const DataItem *pa =
        (atype != BLAS_VOID && ait) ? (*ait)->dataitem(a->id()) : a;
