...
public:
  ...
  static
  void limit1(const DataItem *x, const DataItem *y, DataItem *z);
  ...
  static
  void add_scalar(const DataItem *x, const void *y, DataItem *z, int swap = 0);

  /// Operation wrapper for subtraction with y as a scalar pointer.
  static
  void sub_scalar(const DataItem *x, const void *y, DataItem *z, int swap = 0);

  /// Operation wrapper for multiplication with y as a scalar pointer.
  static
  void mul_scalar(const DataItem *x, const void *y, DataItem *z, int swap = 0);

  /// Operation wrapper for division with y as a scalar pointer.
  static
  void div_scalar(const DataItem *x, const void *y, DataItem *z, int swap = 0);

  /// Operation wrapper for addition with y as a scalar pointer.
  static
  void maxof_scalar(const DataItem *x, const void *y, DataItem *z, int swap = 0);

  /// Wrapper for dot product.
  static
  void dot(const DataItem *x, const DataItem *y, DataItem *z,
	   const DataItem *mults=NULL);

  /// Wrapper for 2-norm with z as a scalar pointer.
  static
  void dot_scalar(const DataItem *x, const DataItem *y, void *z,
		  const DataItem *mults=NULL);

  /// Wrapper for dot product.
  static
  void dot_MPI(const DataItem *x, const DataItem *y, DataItem *z,
	       const MPI_Comm *comm=NULL, const DataItem *mults=NULL);

  /// Wrapper for 2-norm with z as a scalar pointer.
  static
  void dot_scalar_MPI(const DataItem *x, const DataItem *y, void *z,
		      const MPI_Comm *comm=NULL, const DataItem *mults=NULL);

  /// Wrapper for 2-norm.
  static
  void nrm2(const DataItem *x, DataItem *y, const DataItem*mults=NULL);

  /// Wrapper for 2-norm with y as a scalar pointer.
  static
  void nrm2_scalar(const DataItem *x, void *y, const DataItem*mults=NULL);

  /// Wrapper for 2-norm with MPI.
  static
  void nrm2_MPI(const DataItem *x, DataItem *y,
		const MPI_Comm *comm=NULL, const DataItem*mults=NULL);

  /// Wrapper for 2-norm with y as a scalar pointer with MPI.
  static
  void nrm2_scalar_MPI(const DataItem *x, void *y,
		       const MPI_Comm *comm, const DataItem *mults=NULL);
  ...
  static
  void rand(const DataItem *a, DataItem *z);
  ...
  /// Generate a random number between 0 and $a$ for each entry in z
  static
  void rand_scalar(const void *a, DataItem *z);

  /// Wrapper for neg (y=-x).
  static
  void neg(const DataItem *x, DataItem *y);

  /// Wrapper for sqrt (y=sqrt(x)).
  static
  void sqrt(const DataItem *x, DataItem *y);

  /// Wrapper for acos (y=acos(x)).
  static
  void acos(const DataItem *x, DataItem *y);

  /// Wrapper for max.
  static
  void max_MPI(const DataItem *x, DataItem *y, const MPI_Comm *comm=NULL);

  /// Operation wrapper for max (y is a scalar pointer).
  static
  void max_scalar_MPI(const DataItem *x, void *y, const MPI_Comm *comm=NULL);

  /// Wrapper for min.
  static
  void min_MPI(const DataItem *x, DataItem *y, const MPI_Comm *comm=NULL);

  /// Operation wrapper for min (y is a scalar pointer).
  static
  void min_scalar_MPI(const DataItem *x, void *y, const MPI_Comm *comm=NULL);

  /// Wrapper for sum.
  static
  void sum_MPI(const DataItem *x, DataItem *y, const MPI_Comm *comm=NULL);

  /// Operation wrapper for sum (y is a scalar pointer).
  static
  void sum_scalar_MPI(const DataItem *x, void *y, const MPI_Comm *comm=NULL);

  /// Operation wrapper for z = a * x + y.
  static
  void axpy(const DataItem *a, const DataItem *x, const DataItem *y,
	    DataItem *z);

  /// Operation wrapper for z = a * x + y (a is a scalar pointer).
  static
  void axpy_scalar(const void *a, const DataItem *x, const DataItem *y,
		   DataItem *z);
...
protected:
  ...
  static
  void calc(DataItem *z, const DataItem *x, const void *yin,
	    FuncType opp, bool swap=false);

  ///  Performs the operation:  z = <x, y>
  template <class data_type, int ztype>
  static
  void calcDot( void *zout, const DataItem *x, const DataItem *y,
		const MPI_Comm *comm=NULL, const DataItem *mults=NULL);

  ///  Performs the operation  opp(x, y)
  template <class FuncType, int ytype>
  static
  void gen2arg( DataItem *z, void *yin, FuncType opp);

  ///  Performs the operation:  z = a*x + y
  template <class data_type, int atype>
  static
  void axpy_gen( const void *a, const DataItem *x, const DataItem *y,
		 DataItem *z);

  /// Chooses which calc function to call based on type of y.
  template <class FuncType>
  static
  void calcChoose(const DataItem *x, const DataItem *y, DataItem *z,
		  FuncType opp);


  template < class Op> static
  void copy_helper( const DataItem *x, DataItem *z);
  ...
  inline static int
  get_stride( const DataItem *attr);
  ...
  inline static
  void reduce_MPI(const DataItem *x, DataItem *z,
		  const MPI_Comm *comm, int, double);

  template <class OPint, class OPdbl, int OPMPI>
  inline static
  void reduce_scalar_MPI(const DataItem *x, void *y,
			 const MPI_Comm* comm, int, double);
