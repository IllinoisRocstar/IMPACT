...
/**
 *  @brief The PaneConnectivity object holds the inter-zone connectivity of each
 * grid.
...
class PaneConnectivity {
 private:
  std::map<int, std::vector<int>>
      pconn; /*!< @brief Holds a mapping of remote pane to a list of local ids
                that are shared.
              */
  std::map<int, std::vector<int>>
      point2remotePaneList; /*!< @brief Holds a mapping of localpoint ids to
                               remote pane ids
                             */
  std::map<std::pair<int, int>, int>
      rmtpanePointPair2Index; /*!< @brief Holds a mapping of a
                                 (remotePane,localPoint) pair to the index in
                                 the shared node list */

  /**
   * @brief Returns the index to the shared node list of the local point that is
   * shared with the given remote pane.
...
#ifdef ASSERT_ON
    assert(this->rmtpanePointPair2Index.find(keyPair) !=
           this->rmtpanePointPair2Index.end());
#endif
  ...
  /**
   * @brief A convenience method which provides safe access to the pconn array
   * by doing bounds checking and memory corruption checking. After, the access
   * transaction is complete, the index is also automatically incremented.
  ...
  /**
   * @brief Builds the pane connectivity data-structured from a flat integer
   * array returned from RocMap.
    ...
    for (int connection = 0; connection < numberOfConnections; ++connection) {
      ...
      for (int node = 0; node < numnodes; ++node) {
        sharedNodeList[node] =
            ...
            1; /* Note, pconn is 1-based, not 0-based so 1 is subtracted */
        std::pair<int, int> keyPair =
            std::make_pair(rmtpane, sharedNodeList[node]);
 ...
 public:
  ...
  /**
   * @brief This method returns the point connection pairs for the given local
   * point.
   * @param localPointId the point id in query.
   * @param connections the list of connection pairs.
   * @note A connection pair consists of the remotePaneId and sharedNodeIndex to
   * the
   */
  void getPointConnectionPairs(const int localPointId,
    ...
    for (int i = 0; i < rmtpanes.size(); ++i) {
      //					std::cout << rmtpanes[ i ] << "
      //" << localPointId << std::endl;

      std::pair<int, int> valuePair = std::make_pair(
          rmtpanes[i], this->getSharedNodeIndex(rmtpanes[i], localPointId));

      //					std::cout << valuePair.first <<
  ...
  /**
   * @brief Constructs a PaneConnectivity object from the pcon array returned
   * from Rocmap.
   * @param pconnarray the pcon connectivity array.
   * @param size the size of the pconnarray.
   * @pre pconnarray != NULL
   ...
   */
  inline bool hasPane(const int paneId) const {
    return (this->pconn.find(paneId) != this->pconn.end());
  }
  ...
  inline bool hasLocalPoint(const int localPointId) const {
    ...
    if (this->point2remotePaneList.find(localPointId) !=
        this->point2remotePaneList.end())
      return true;
    else
      return false;
  }
  ...
  /**
   * @brief This method returns the list of remote panes that share the point
   * associated with the provided localPointId.
   * @param localPointId the local point id.
   ...
   * @post this ->hasPane( rmtpanes[ i ] ) == true, for 0 <= i <=
   * rmtpances.size( ).
   */
  ...
  /**
   * @brief This method returns the local shared point Id of the given pane at
   * the given index.
   ...
   * @post localID >= 0 && localID < N where N is the number of nodes in the
   * mesh at the given pane.
   */
   ...
   * @param remotePane the id of the remote pane that has shared nodes with this
   * instance.
   * @return NodeList a reference to list of shared nodes.
   * @note NodeList cannot be changed by the client of this method.
   * @pre this ->hasPane( remotePane ) == true.
   * @post NodeList.size( ) == this ->getNumberOfNodesSharedWithRemotePane(
   * remotePane ).
   */
