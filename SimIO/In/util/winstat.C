...
int main(int argc, char *argv[]) {
   ...
   if (winTopoData) {
	 ...
	 for (int i=0; i<numConn; ++i) {
	    ...
	    COM_get_dataitem(fullConnName, &dataItemLoc, &dataItemType,
   ...
   if (meshData){
	 ...
	 COM_get_size((wname+".nc").c_str(), paneId, &nNde);
         ...
         for (int iNde=0; iNde<nNde; iNde++){
            ...
            std::cout << std::left << std::setw(numWidth) << ndeCrd[iNde+2*nNde];
            std::cout << std::endl;
         }
   ...
   if (spcData){
      ...
      COM_get_dataitem((wname+"."+spcDataItmName).c_str(), &dataItemLoc, &dataItemType,
                            &dataItemNComp, &dataItemUnits);
      COM_get_size((wname+"."+spcDataItmName).c_str(),paneList[0], &dataItemSize);
