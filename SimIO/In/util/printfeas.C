...
void COM_print_window(const std::string &wName, const std::string &timeStr,
  ...
  // Eliminate window and pane dataitems, note element dataitems.
  // EDIT: pane dataitems with one item are used as element data.
  ...
  for (i = 0; i < nPanes; ++i) {
    ...
    if (COM_get_dataitem_handle((wName + ".ridges").c_str()) > 0) {
      ++zone_count;
...
int main(int argc, char *argv[]) {
  ...
  int IN_all = COM_get_dataitem_handle(win_all.c_str());

  std::cerr << "Obtaining data dataitems for window  " << win_in << std::endl;
