...
/**
 * @brief Utilizes Roctrail to convert the panes to an unstructured grid format
 * @param wName the window name.
...
std::string convertWindow(const std::string &wName) {
  ...
  TRAIL_Window2UnstructuredMesh(wName, Program.meshes, Program.smdv,
  ...
  for (int i = 0; i < Program.meshes.size(); ++i) {
    TRAIL_UnstructuredMesh2Pane(newWindow, Program.paneIds[i],
                                Program.meshes[i], Program.smdv[i],
                                Program.solution[i], 0);
  }
...
std::string getWindow() {
  ...
  // Taken from from Roctrail::TRAIL_HDF2Window
