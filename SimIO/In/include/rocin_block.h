...
struct VarInfo_HDF4 {
  ...
  VarInfo_HDF4(const std::string &name, char pos, COM_Type dType,
               const std::string &units, int nc, int32 i, int nitems, int ng,
               bool is_null)
      : m_name(name),
        m_position(pos),
        m_dataType(dType),
...
struct GridInfo_HDF4 {
  ...
  inline GridInfo_HDF4(int32 *size, int ng)
      : m_name(":st0:"),
        m_numElements(size[2] - 1 * std::max(int32(1), (size[1] - 1)) *
...
struct VarInfo_CGNS {
  ...
  VarInfo_CGNS(const std::string &name, char pos, COM_Type dType,
               const std::string &units, int nc, int i, int nitems, int ng,
               bool is_null)
      : m_name(name),
        m_position(pos),
        m_dataType(dType),
...
struct GridInfo_CGNS {
  ...
  inline GridInfo_CGNS(int *size, int cellDim, int ng)
      : m_name(":st0:"), m_numElements(size[cellDim]), m_numGhostElements(ng) {
    m_name[3] += cellDim;
    m_size[0] = size[0];
    m_size[1] = m_size[2] = 1;
    if (cellDim > 1) {
      m_size[1] = size[1];
      m_numElements *= size[cellDim + 1];
      if (cellDim > 2) {
        m_size[2] = size[2];
        m_numElements *= size[5];
      }
    }
  }
  ...
  int m_numElements;       ///< Number of elements in a mesh.
  int m_numGhostElements;  ///< Number of ghost elements in a mesh.
                           // We don't really need to save the Element index,
                           // since we use all of them in the Zone.
};
