 ...
 */
void writeVtkFiles(const std::string &outputfile,
                   const std::vector<int> &paneIds,
  ...
  for (int m = 0; m < meshes.size(); ++m) {
    ...
    ofs << "POINT_DATA " << masks[m].size() << std::endl;
    ofs << "SCALARS SharedNodes double\nLOOKUP_TABLE default\n";
    for (int i = 0; i < masks[m].size(); ++i) {
      if (masks[m][i])
        ofs << "1\n";
      else
        ofs << "0\n";
 ...
 */
void writeVtkData(NodalCoordinates &nc, Connectivity &con,
  ...
  for (int i = 0; i < nelem; ++i) {
    ...
    if (maxnodes < esize) maxnodes = esize;
  }
  ...
  for (int i = 0; i < nelem; ++i) {
    ofs << con.Esize(i) << " ";
    for (int j = 0; j < con.Esize(i); ++j) ofs << con[i][j] - 1 << " ";
    ofs << std::endl;
  }
