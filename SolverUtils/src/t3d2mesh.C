...
class T3DGeomEnt : public std::pair<unsigned int, unsigned int> {
 public:
  ...
  bool operator<(const T3DGeomEnt &ge) const {
    return (first < ge.first ? true
                             : first > ge.first ? false : second < ge.second);
  };
  bool operator==(const T3DGeomEnt &ge) const {
    return (first == ge.first ? second == ge.second : false);
  };
};
...
int main(int argc, char *argv[]) {
  ...
  switch (mesh_type) {
    ...
    case 7:
      ...
      break;
    default:
      std::cerr << "Fatal Error: Unsupported mesh type, " << mesh_type << "."
                << std::endl;
      return (1);
      break;
  }
  ...
  if (nedges > 0) {
    ...
    for (unsigned int i = 0; i < nedges; i++) std::getline(std::cin, line);
    std::getline(std::cin, line);
  }
  ...
  if (mesh_type != 3 && nquads > 0) {
    for (unsigned int i = 0; i < nquads; i++) {
      ...
      std::cout << std::endl;
      int property = 0;
      Istr >> elem_geoms[i + egoffset].first >>
          elem_geoms[i + egoffset].second >> property;
      elemental_properties.push_back(property);
    }
    std::getline(std::cin, line);
  }
  ...
  while (gi != nodal_geoms.end()) {
    std::list<T3DGeomEnt>::iterator gli =
        std::find(geomlist.begin(), geomlist.end(), *gi);
    if (gli == geomlist.end()) {
  ...
  while (gi != elem_geoms.end()) {
    std::list<T3DGeomEnt>::iterator gli =
        std::find(geomlist.begin(), geomlist.end(), *gi);
    if (gli == geomlist.end()) geomlist.push_back(*gi);
    gi++;
  }
  geomlist.sort();
  ...
  while (gli != geomlist.end()) {
    std::cerr << "Dimension: " << gli->first << ", Id = " << gli->second
              << std::endl;
    gli++;
  }
             ...
             : gli->first == 2
                   ...
                   : gli->first == 3
                         ...
                         : gli->first == 4
                               ...
                               : gli->first == 5
                                     ? 2
                                     : gli->first == 6 ? 2 : gli->first);
    if (dim > 3) {
      std::cerr << "Error: Cannot deal with interface mesh type" << std::endl;
      return (1);
    }
    std::ostringstream Ostr;
    if (dim == 0) Ostr << "Vertex-" << ++nvertex;
    if (dim == 1) Ostr << "Curve-" << ++ncurve;
    if (dim == 2) Ostr << "Surface-" << ++nsurf;
    if (dim == 3) Ostr << "Region-" << ++nregion;
  ...
  while (gi != nodal_geoms.end()) {
    Mesh::IndexType gindex =
        std::find(t3d_geovec.begin(), t3d_geovec.end(), *gi++) -
        t3d_geovec.begin();
