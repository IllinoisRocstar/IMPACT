...
std::ostream &operator<<(std::ostream &Out, const ComLineObject &cl) {
  ...
  if (!cl._args.empty()) {
    Out << "<arguments>\n";
    std::vector<std::pair<std::string, int> >::const_iterator ai =
        cl._args.begin();
    while (ai != cl._args.end()) {
      std::map<std::string, std::string>::const_iterator ahi =
          cl._arghelp.find(ai->first);
...
void ComLineObject::Copy(ComLineObject &incom) {
  ...
  while (ici != incom._error_messages.end()) _error_messages.push_back(*ici++);
  ici = incom._nonops.begin();
  while (ici != incom._nonops.end()) _nonops.push_back(*ici++);
  ...
  std::vector<std::pair<std::string, int> >::iterator icai =
      incom._args.begin();
  while (icai != incom._args.end()) _args.push_back(*icai++);
  ...
  while (cli != incom.end()) this->push_back(*cli++);
}

std::string ComLineObject::LongUsage() {
  std::ostringstream Ostr;
  if (!_description.empty()) Ostr << _description << std::endl << std::endl;
  Ostr << "Usage: " << std::endl
       << std::endl
       << ShortUsage() << std::endl
       << std::endl;
  ...
  while (ti != this->end()) {
    Ostr << "\t"
         ...
         << "\t\t" << _help[ti->first];
    ti++;
    if (ti != this->end()) Ostr << std::endl << std::endl;
  }
  ...
  while (ai != _args.end()) {
    Ostr << "\t" << (ai->second ? "<" : "[") << ai->first
         ...
         << "\t\t" << _arghelp[ai->first];
    ai++;
    if (ai != _args.end()) Ostr << std::endl << std::endl;
  }
  if (!_notes.empty()) Ostr << std::endl << _notes;
  return (Ostr.str());
}
...
std::string ComLineObject::GetLong(const char &s) {
  ...
  while (ti != this->end()) {
    if (ti->first == s) return (ti->second);
    ti++;
  }
  return ("");
}
...
char ComLineObject::GetShort(const std::string &l) {
  std::vector<std::pair<char, std::string> >::iterator ti = this->begin();
  while (ti != this->end()) {
    if (ti->second == l) return (ti->first);
    ti++;
  }
  return ('\0');
}
...
void ComLineObject::AddOption(char s, const std::string &l, int atype) {
  ...
  _type[s] = atype;
  if (atype > 0) _argname[s] = "arg";
  _help[s] = std::string("");
}

void ComLineObject::AddOption(char s, const std::string &l, int type,
...
int ComLineObject::ProcessOptions() {
  ...
  while (ai != args.end()) {
    ...
    if (ai != args.end()) next_argument = *ai--;
    std::string::iterator si = this_argument.begin();
    if (*si == '-' && !end_of_ops) {
      si++;
      if (si == this_argument.end()) end_of_ops = true;
      if (*si != '-') {  // then we are processing a short option
        while (si != this_argument.end()) {
          char flag_char = *si++;
          std::vector<std::pair<char, std::string> >::iterator oi =
              this->begin();
          bool found = false;
          while ((oi != this->end()) && !found) {
            if (flag_char == oi->first) {
              found = true;
              if (si ==
                  this_argument.end()) {  // means it's not a string of flags
                if (_type[flag_char] != 0) {
                  if (!next_argument.empty() && next_argument[0] != '-') {
                    _options[flag_char] = next_argument;
                    ai++;
                  } else if (_type[flag_char] >
                             1) {  // next arg wasn't valid, err if reqd
                    errs++;
                    std::ostringstream Ostr;
                ...
                if (_type[flag_char] >
                    1) {  // it requires an argument, can't be in a flag string
                  errs++;
                  std::ostringstream Ostr;
                  Ostr << "Option -" << flag_char << " requires an argument.";
                  _error_messages.push_back(Ostr.str());
...
void ComLineObject::SetRawComLine(const std::string &incl) {
  ...
  std::string token;
  Istr >> _line;
  while (Istr >> token) _line += (std::string(" ") + token);
}
