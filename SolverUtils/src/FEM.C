...
//  }
				      ...
				      const std::vector<FieldData> &fields) // input
  {
	...
	if(fields[*efi-1].Order() > 0)
	  enfields[jj].push_back(*efi);
	efi++;
      }
    }
    // Step 1
	...
	if(!nfields[node_index].empty()){
	  if(!processed_node[node_index]){
	    ...
	    std::vector<Mesh::IndexType>::iterator nfi = nfields[node_index].begin();
	    ...
	    std::vector<Mesh::IndexType>::iterator ndi = NodalDofs[node_index].begin();
				     ...
				     int rank) 
  {
	...
	NumberElementDofsLocal(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,
			       info,processed_node,processed_borderel,border_element_list,false,rank);
	nprocessed++;
      }
	...
	if(!processed_element[nbrindex]){
	  ...
	  nprocessed++;
	  NumberElementDofsLocal(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,nbrindex,
				 info,processed_node,processed_borderel,border_element_list,false,rank);
	}
      }
    }
    ...
    while(bei != border_element_list.end()){
      ...
      if(!rank)
	std::cout << "border element " << elid << std::endl;
      NumberElementDofsLocal(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,
  ...
  bool IsBorderElement(std::vector<Mesh::IndexType> &ec,
		       Mesh::PartInfo &info)
  {
  ...
  void NumberElementDofsLocalII(Mesh::Connectivity &ec,
				...
				std::vector<bool> &is_border_element,
				bool remote,
				int rank)
  {
    if(!rank)
      std::cout << "Processing element " << el_index+1 << std::endl;
	...
	if(!processed_node[node_index]){
	  ...
	  while(nfi != nfields[node_index].end()){
		...
		if(node_id <= info.nlocal){
		  if(!rank)
		    std::cout << "Numbering dofs for node " << node_id << std::endl;
		  processed_node[node_index] = true;
		  NodalDofs[node_index].push_back(dofnum);
		  if(!border_element){
		    //		    ElementDofs[el_index].push_back(dofnum);
		  ...
		  //		  else if(!rank)
		  //		    std::cout << "Skipping numbering dofs of border element." << std::endl;
		  dofnum++;
		}
		else {
		  if(!rank)
	      ...
	      //	      else if(node_id > info.nlocal){ // remote is enabled and this is a border element
	      //		if(!rank)
	      //		  std::cout << "Numbering dofs for remote node " << node_id << std::endl;
	      //		NodalDofs[node_index].push_back(dofnum);
	      //		ElementDofs[el_index].push_back(dofnum++);
	      //		processed_node[node_index] = true;
	      //	      }
	...
	//	      if(!rank)
	//		std::cout << "Adding dofs from existing node " << node_index+1 << std::endl;
	//	      //	      ElementDofs[el_index].push_back(*ndi);
    ...
    if(!remote){
      if(!rank)
	std::cout << "Numbering dofs for the element." << std::endl;
      std::vector<Mesh::IndexType>::iterator efi = efields[el_index].begin();
      while(efi != efields[el_index].end()){
	Mesh::IndexType field_id = *efi++;
	Mesh::IndexType field_index = field_id - 1;
	...
	std::cout << "Recovering dof numbers from border element." << std::endl;
      ...
      while(efi != redofs.end())
	ElementDofs[el_index].push_back(*efi++);
    }
  };
				     ...
				     int rank) 
  {
	...
	queue.pop_front();
      ...
      if(!rank)
	std::cout << "Considering element " << el_index+1 << std::endl;
      if(!processed_element[el_index]){
	...
	if(!rank)
	  std::cout << "Not a border and not processed, processing " << el_index+1 << std::endl;
	processed_element[el_index] = true;
	NumberElementDofsLocalII(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,
				 info,processed_node,is_border_element,false,rank);
	nprocessed++;
      }
      // Loop thru this element's neighbors
      std::vector<Mesh::IndexType>::iterator eni = nbr[el_index].begin();
	...
	if(is_border_element[nbrindex]){
	  ...
	  nprocessed++;
	  processed_element[nbrindex] = true;
	  NumberElementDofsLocalII(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,nbrindex,
				 info,processed_node,is_border_element,false,rank);
	}
      }
    }
    ...
    while(bei != border_element_list.end()){
      ...
      Mesh::IndexType el_index = elid-1;
      if(!rank)
	std::cout << "numbering border element " << elid << std::endl;
      //      NumberElementDofsLocal(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,
			     //			     info,processed_node,processed_borderel,border_element_list,true,rank);
      NumberElementDofsLocalII(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,
			       info,processed_node,is_border_element,false,rank);
				  ...
				  const std::vector<FieldData> &fields) // input
  {
	...
	queue.pop_front();
      ...
      if(!processed_element[el_index]){
	processed_element[el_index] = true;
	NumberElementDofs(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,el_index,processed_node);
	nprocessed++;
      }
      // Loop thru this element's neighbors
      std::vector<Mesh::IndexType>::iterator eni = nbr[el_index].begin();
	...
	if(!processed_element[nbrindex]){
	  ...
	  processed_element[nbrindex] = true;
	  nprocessed++;
	  NumberElementDofs(ec,ElementDofs,NodalDofs,efields,nfields,fields,dofnum,nbrindex,processed_node);
	}
      }
    }
    //    assert(queue.empty());
  ...
  // each node/element/mesh entity.  Need to match each dof with it's field
  // and component and mesh entity.
  Mesh::IndexType AssignUniformFields(Mesh::Connectivity &ec,
			   Mesh::Connectivity &dc,
			   Mesh::Connectivity &efields,
			   Mesh::Connectivity &nfields,
			   Mesh::Connectivity &NodalDofs,
			  ...
			  std::vector<Mesh::IndexType> &dofs)
  {
    ...
    while(eni != econ[elindex].end()){
      ...
      while(ndi != NodalDofs[*eni-1].end())
	dofs[vindex++] = *ndi++;
      eni++;
    }
    eni = ElementDofs[elindex].begin();
    while(eni != ElementDofs[elindex].end())
      dofs[vindex++] = *eni++;
    return(vindex);
  }
...
//     MyErrFile << "info.doffset = " << info.doffset << std::endl;
    ...
    while(eIt != element_queue.end()){
      ...
      elindex = *eIt++ - 1; 
      //    ndofe = (INTEGRATE ELEMENT); // integrates element and populates dofdat (add physics here)
      ndofe = NDofE[elindex]; // Number of element centered dofs
      std::vector<Mesh::IndexType>::iterator eni = econ[elindex].begin();
      while(eni != econ[elindex].end()){  // Loop through the nodes of the element
	...
	col   = NodalDofs[*enIt-1][0]; // the first dof id of the first node (should be sorted i suppose)
	datindbase = rowcount*ndofe;  // rowcount wrt element times number dofs in element
	colcount   = 0;
...
// 	  if(CurPos == Send){
// 	    searcherr++;
// 	    MyErrFile << "nn(Rrow,Rnrow,Sbegin,Send):(col,ncol,colcount):(datindbase,nsearches) = (" << Rrow << "," 
// 		      << Rnrow << "," << *Sbegin << "," << *Send << "):(" << col
// 		      << "," << ncol << "," << colcount << "):(" << datindbase 
// 		      << "," << nsearches << ")" << std::endl; 
...
// 	  if(CurPos == Send){
// 	    searcherr++;
// 	    MyErrFile << "ne(Rrow,Rnrow):(col,ncol,colcount):(datindbase,nsearches) = (" << Rrow << "," 
// 		      << Rnrow << "):(" << col
// 		      << "," << ncol << "," << colcount << "):(" << datindbase 
// 		      << "," << nsearches << ")" << std::endl; 
// 	    Util::DumpContents(MyErrFile,(*(k._dofs))[Rrow-1]);
...
// 	  if(CurPos == Send){
// 	    searcherr++;
// 	    MyErrFile << "en(Rrow,Rnrow):(col,ncol,colcount):(datindbase,nsearches) = (" << Rrow << "," 
// 		      << Rnrow << "):(" << col
// 		      << "," << ncol << "," << colcount << "):(" << datindbase 
// 		      << "," << nsearches << ")" << std::endl; 
// 	    Util::DumpContents(MyErrFile,(*(k._dofs))[Rrow-1]);
...
// 	  if(CurPos == Send){
// 	    searcherr++;
// 	    MyErrFile << "ee(Rrow,Rnrow):(col,ncol,colcount):(datindbase,nsearches) = (" << Rrow << "," 
// 		      << Rnrow << "):(" << col
// 		      << "," << ncol << "," << colcount << "):(" << datindbase 
// 		      << "," << nsearches << ")" << std::endl; 
// 	    Util::DumpContents(MyErrFile,(*(k._dofs))[Rrow-1]);
// 	  }
    ...
    //    MyErrFile << "NSearchErrors/Nsearches: " << searcherr << "/" << nsearches << std::endl;
    //    MyErrFile.close();
    return(nsearches);
  }
  ...
  Mesh::IndexType AssembleLocalElement(std::vector<Mesh::IndexType> &con,
	...
	if(nedofs > 0){
	  nsearches++;
	  Mesh::IndexType col = edofs[datind]+doffset; // all of these are local
  ...
  Mesh::IndexType AssembleLocalElementII(std::vector<Mesh::IndexType> &con,
	...
	if(nedofs > 0){
	  nsearches++;
	  //      Mesh::IndexType bcol = edofs[datind]+info.doffset; // all of these are local
  ...
  Mesh::IndexType AssembleBorderElementII(std::vector<Mesh::IndexType> &con,
	...
	while(ieni != con.end()){
	  ...
	  if(node_id <= info.nlocal){
	    ...
	    AssembleToCommBuffer(brow,bcol,dofdata,borders[color-1].data,datind,dskip,nrow,ncol);
	  }
	  //      datind+=ncol;
	}
	if(nedofs > 0){
	  ...
	  if(node_id <= info.nlocal){
	    ...
	    AssembleToCommBuffer(brow,bcol,dofdata,borders[color-1].data,datind,dskip,nrow,nedofs);
	  }
	}
      }
    }
	...
	if(inode_id > info.nlocal){
	  ...
	  if(ncol > 0)
	    bcol = RemoteNodalDofs[inode_id-info.nlocal-1][0];
	}
	else{
	  ncol = NodalDofs.Esize(inode_id);
	  if(ncol > 0)
			    ...
			    unsigned int N,unsigned int M)
  {
    ...
    iIt  += data.SendAp[i-1];
    iEnd += data.SendAp[i];
    //  std::vector<Mesh::IndexType>::iterator iFind = std::lower_bound(iIt,iEnd,j);
    //  assert(iFind != iEnd);
    //  assert(*iFind == j);
    Mesh::IndexType index = std::lower_bound(iIt,iEnd,j) - iIt + data.SendAp[i-1]; 
    //  assert(index >= 0);
		      ...
		      std::vector<double> &dofdat)
  {
    ...
    while(rni != border.nrecv.end()){
      ...
      if(nrow > 0){
	brow = NodalDofs[node_id-1][0];
	Mesh::IndexType ncol_total = border.data.RecvAp[bnodeid] - 
	  border.data.RecvAp[bnodeid-1];
	Mesh::IndexType datind = 0;
	if(ncol_total > 0){
	  Mesh::IndexType curind = border.data.RecvAp[bnodeid-1];
	  std::vector<Mesh::IndexType>::iterator AiIt = border.data.RecvAi.begin();
	  AiIt += curind;
	  while(datind < ncol_total && AiIt != border.data.RecvAi.end()){
	    ...
	    FEM::AssembleIJMN(brow,bcol,dofdat,k,0,dskip,nrow,ncol_this_write);
