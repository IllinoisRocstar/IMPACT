...
int Partition::GetBorderElements(std::vector<Mesh::IndexType> &be) const {
  ...
  for (Mesh::IndexType nn = nvolume_nodes; nn < number_of_nodes; nn++) {
    std::vector<Mesh::IndexType>::const_iterator ei = dc[nn].begin();
    while (ei != dc[nn].end()) {
  ...
  while (bei != be.end()) *bei++ = *beli++;
  return (0);
}

int Partition::Read(const std::string &MeshName,
                    IRAD::Comm::CommunicatorObject &comm, bool allow_n2m,
                    std::ostream &ErrOut) {
  unsigned int nproc = comm.Size();
  unsigned int rank = comm.Rank();
  unsigned int id = rank + 1;
  std::cout << "Partition::Read " << MeshName << "(" << nproc << "," << rank
            << "," << id << ")" << std::endl;
  ...
  if (nproc > 1) {
    ...
    if (comm.Check()) {
      ErrOut << "Partition::Read Could not find partition info for "
             << MeshName;
      return (1);
    }
    InfoInf >> _info.npart >> _info.part >> _info.nelem >> _info.nnodes >>
        _info.nborder >> _info.nshared >> _info.nown;
    std::cout << _info.npart << _info.part << _info.nelem << _info.nnodes
              << _info.nborder << _info.nshared << _info.nown;
    if (_info.npart != nproc && !allow_n2m) {
      comm.SetErr(1);
    }
    if (comm.Check()) {
      ErrOut << "Partition::Read " << MeshName
             << " has partition/processor mismatch: (" << _info.npart << "/"
             << nproc << ").\n";
      return (1);
    }
...
int Partition::Read(const std::string &MeshName, int id) {
  // rank = id - 1
  std::ifstream Inf;
  std::ostringstream FNOstr;
  if (id > 0) {
    ...
    if (!Inf) return (2);
    Inf >> _info.npart >> _info.part >> _info.nelem >> _info.nnodes >>
        _info.nborder >> _info.nshared >> _info.nown;
    Inf.close();
    _info.nlocal = _info.nnodes - _info.nshared + _info.nown;
    FNOstr.str("");
