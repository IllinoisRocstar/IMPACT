 ...
 *    + For these, there are 3 numbers per line, with a space in between
 * numbers.
 ...
 *   To run the program, you need to specify where the input text file is in the
 * command line. Example: ./meshgen2d "path to input file" Replace "path to
 * input file" with a path to the text file you made. The default output is to
 * write the VTK to stdout, but you can easily redirect this to a .vtk file By
 * doing > blah.vtk
 */
