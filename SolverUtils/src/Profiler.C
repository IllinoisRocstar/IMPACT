...
/// faster timeval math
///
inline struct timeval operator-(const struct timeval &t1,
...
/// faster timeval math
///
inline struct timeval &operator-=(struct timeval &t1,
                                  const struct timeval &t2) {
...
/// Event math
///
bool operator<(const std::pair<unsigned int, std::list<Event> > &p1,
...
std::ostream &operator<<(std::ostream &ost, const Event &e) {
  ost << e._id << " " << e._timestamp << " " << e._inclusive << " "
      << e._exclusive;
  return (ost);
}
...
int ProfilerObj::Init(const std::string &name, int id) {
  if (_initd) {
    std::cerr << "ProfilerObj::Init: Error: already initialized. Tried to "
                 "reinit with "
              << name << "." << std::endl;
    return (1);
...
int ProfilerObj::FunctionExit(const std::string &name) {
  ...
  if (id == 0)
    std::cerr << "Mismatched(" << profiler_rank << "):" << name << ", expected "
...
int ProfilerObj::ReadConfig(const std::string &cfname) {
  if (cfname.empty()) {
    if (Err) *Err << "ProfilerObj::Error: Config file name empty." << std::endl;
    return (1);
  }
  ...
  if (!conf_file) {
    if (Err)
      *Err << "ProfilerObj::Error: Could not open config file, " << cfname
           << "." << std::endl;
    return (1);
  }
...
void ProfilerObj::SummarizeSerialExecution(std::ostream &Ostr) {
  ...
  if (cmi != configmap.end()) application_name = cmi->second;
  ei = event_list.begin();
  ...
  while (si != statmap.end()) {
    ...
    Ostr << std::setprecision(5) << std::setw(12) << si->second.incl << " "
         << std::setw(12) << si->second.excl << " " << std::setw(12)
         << si->second.ncalls << " " << std::setw(12) << imean << " "
         << std::setw(12)
         << (si->second.ncalls == 1 ? 0
                                    : sqrt(std::fabs(imean2 - (imean * imean))))
         << " " << std::setw(12) << emean << " " << std::setw(12)
         << (si->second.ncalls == 1 ? 0
...
void ProfilerObj::WriteEventFile() {
  ...
  if (!(profiler_rank / 10000)) Ostr << "0";
  if (!(profiler_rank / 1000)) Ostr << "0";
  if (!(profiler_rank / 100)) Ostr << "0";
  if (!(profiler_rank / 10)) Ostr << "0";
  Ostr << profiler_rank;
...
int ProfilerObj::ReadEventsFromFile(const std::string &filename) {
  ...
  while (datafile >> e) event_list.push_back(e);
  datafile.close();
  return (0);
}

int ProfilerObj::ReadParallelEventFiles(const std::vector<std::string> &ifiles,
  ...
  while (ifi != ifiles.end()) {
    ...
    Inf >> profiler_rank;
    Event e;
    while (Inf >> e) event_list.push_back(e);
    Inf.close();
    event_list.sort();
...
int ProfilerObj::SummarizeParallelExecution(std::ostream &Ostr,
  ...
  if (parallel_event_list.empty()) return (1);
  std::string application_name = "Application";
  std::map<unsigned int, std::string>::iterator cmi = configmap.find(0);
  if (cmi != configmap.end()) application_name = cmi->second;
  ...
  Ostr << "#Statistics for " << application_name << " (" << number_of_processors
       << " procs):" << std::endl
       << std::endl
  ...
  while (si != pstat_map.end()) {
    ...
    double imean2 = (si->second.incl_min == si->second.incl_max
                         ? 0
                         : (sqrt(fabs((si->second.incl_stdev /
                                       (double)number_of_processors) -
                                      (imean * imean)))));
    Ostr << std::setw(32) << routine_name << " " << std::setw(12)
         << si->second.incl_min << " " << std::setw(5)
         << si->second.incl_minrank << " " << std::setw(12)
         << si->second.incl_max << " " << std::setw(5)
         << si->second.incl_maxrank << " " << std::setw(12) << imean << " "
         << std::setw(12) << imean2 << " " << std::endl;
    Ouf << si->first << " " << si->second.incl_min << " "
        << si->second.incl_minrank << " " << si->second.incl_max << " "
        << si->second.incl_maxrank << " " << imean << " " << imean2
        << std::endl;
    si++;
  }
  si = pstat_map.begin();
  ...
  while (si != pstat_map.end()) {
    ...
    double emean2 = (si->second.excl_max == si->second.excl_min
                         ? 0
                         : (sqrt(fabs((si->second.excl_stdev /
                                       (double)number_of_processors) -
                                      (emean * emean)))));
    Ostr << std::setw(32) << routine_name << " " << std::setw(12)
         << si->second.excl_min << " " << std::setw(5)
         << si->second.excl_minrank << " " << std::setw(12)
         << si->second.excl_max << " " << std::setw(5)
         << si->second.excl_maxrank << " " << std::setw(12) << emean << " "
         << std::setw(12) << emean2 << " " << std::endl;
    Ouf << si->first << " " << si->second.excl_min << " "
        << si->second.excl_minrank << " " << si->second.excl_max << " "
        << si->second.excl_maxrank << " " << emean << " " << emean2
        << std::endl;
    si++;
  }
  return (0);
...
int ProfilerObj::ReadSummaryFiles(const std::vector<std::string> &input_files,
                                  ScalaMap &scala_map) {
...
int ProfilerObj::PopulateScalaMap(ScalaMap &scala_map,
                                  ScalaStatMap &scala_statmap, bool is_scaled) {
  if (scala_map.empty() || (scala_map.size() == 1)) {
    if (Err)
      *Err << "ProfilerObj::Error: Invalid number of runs for scalability "
              "analysis."
           << std::endl;
    return (1);
  }
  ...
  while (scalamap_i != scala_map.end()) {
    ...
    while (psm_i != scalamap_i->second.end()) {
      ...
      if (ssmi == scala_statmap.end()) {  // Not found
        ...
        ss.sstats[IMIN_E].push_back(
            ((ss.sstats[IMIN_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[IMIN_T][cpos] * ss.nprocs[cpos]) / probsize));
        ss.sstats[IMAX_E].push_back(
            ((ss.sstats[IMAX_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[IMAX_T][cpos] * ss.nprocs[cpos]) / probsize));
        ss.sstats[IMEAN_E].push_back(
            ((ss.sstats[IMEAN_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[IMEAN_T][cpos] * ss.nprocs[cpos]) / probsize));
        ...
        ss.sstats[EMIN_E].push_back(
            ((ss.sstats[EMIN_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[EMIN_T][cpos] * ss.nprocs[cpos]) / probsize));
        ss.sstats[EMAX_E].push_back(
            ((ss.sstats[EMAX_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[EMAX_T][cpos] * ss.nprocs[cpos]) / probsize));
        ss.sstats[EMEAN_E].push_back(
            ((ss.sstats[EMEAN_T][0] * ss.nprocs[0]) / probsize0) /
            ((ss.sstats[EMEAN_T][cpos] * ss.nprocs[cpos]) / probsize));
      ...
      // runsize.  Add to it's scalastats entries for this problem size.
        ...
        ssmi->second.sstats[IMIN_E].push_back(
            ((ssmi->second.sstats[IMIN_T][0] * ssmi->second.nprocs[0]) /
             probsize0) /
            ((ssmi->second.sstats[IMIN_T][cpos] * ssmi->second.nprocs[cpos]) /
             probsize));
        ssmi->second.sstats[IMAX_E].push_back(
            ((ssmi->second.sstats[IMAX_T][0] * ssmi->second.nprocs[0]) /
             probsize0) /
        ...
        ssmi->second.sstats[IMEAN_E].push_back(
            ((ssmi->second.sstats[IMEAN_T][0] * ssmi->second.nprocs[0]) /
             probsize0) /
            ((ssmi->second.sstats[IMEAN_T][cpos] * ssmi->second.nprocs[cpos]) /
             probsize));
  ...
  while (ssm_i != scala_statmap.end()) {
    ...
    for (unsigned int a = 0; a < nps; a++) {
      Out << std::resetiosflags(std::ios::floatfield)
          ...
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[IMAX_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[IMAX_E][a] << " "
          << std::setw(7) << std::setprecision(1) << ss.sstats[IMAX_S][a]
          << "   "

          << std::setprecision(3) << std::showpoint << std::fixed
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[IMEAN_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[IMEAN_E][a]
          << " " << std::setw(7) << std::setprecision(1)
          << ss.sstats[IMEAN_S][a] << "   "

          << std::setprecision(3) << std::showpoint << std::fixed
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[IMIN_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[IMIN_E][a] << " "
          << std::setw(7) << std::setprecision(1) << ss.sstats[IMIN_S][a]
          << "   "

          << std::setprecision(3) << std::showpoint << std::fixed
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[EMAX_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[EMAX_E][a] << " "
          << std::setw(7) << std::setprecision(1) << ss.sstats[EMAX_S][a]
          << "   "

          << std::setprecision(3) << std::showpoint << std::fixed
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[EMEAN_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[EMEAN_E][a]
          << " " << std::setw(7) << std::setprecision(1)
          << ss.sstats[EMEAN_S][a] << "   "

          << std::setprecision(3) << std::showpoint << std::fixed
          << std::setw(9) << std::setiosflags(std::ios::right)
          << ss.sstats[EMIN_T][a] << " " << std::setiosflags(std::ios::left)
          << std::setprecision(2) << std::setw(4) << ss.sstats[EMIN_E][a] << " "
