...
Mesh::NodalCoordinates::NodalCoordinates(Mesh::IndexType n) {
  if (n > 0) {
...
Mesh::NodalCoordinates::NodalCoordinates(Mesh::IndexType n, double *data) {
  if ((n > 0) && (data != NULL)) {
    mydata = false;
    nnodes = n;
    ncdata = data;
...
Mesh::NodalCoordinates::NodalCoordinates(Mesh::IndexType n, double *xdata,
                                         double *ydata, double *zdata) {
  if ((n > 0) && xdata && ydata && zdata)
    init_copy(n, xdata, ydata, zdata);
  else {
    ncdata = NULL;
    nnodes = 0;
    mydata = false;
...
void Mesh::NodalCoordinates::init(Mesh::IndexType n) {
  destroy();
  if (n > 0) {
    ncdata = new double[3 * n];
    nnodes = n;
...
void Mesh::NodalCoordinates::init(Mesh::IndexType n, double *data) {
  destroy();
  if (n > 0) {
    ncdata = data;
    nnodes = n;
...
void Mesh::NodalCoordinates::init_node(Mesh::IndexType n,
...
const GeoPrim::CPoint NodalCoordinates::closest_point(
    const GeoPrim::CPoint &p) const {
  double dist = 1000000;
...
Mesh::IndexType NodalCoordinates::closest_node(const GeoPrim::CPoint &p,
                                               double *dist_ptr) const {
  double dist = 1000000;
...
void Mesh::NodalCoordinates::init_copy(Mesh::IndexType n, double *data) {
  destroy();
  if (n > 0 && data) {
    ncdata = new double[3 * n];
    nnodes = n;
...
void Mesh::NodalCoordinates::init_copy(Mesh::IndexType n, double *data,
  ...
  if (n > 0 && data) {
    ...
    if (stride == 1)
      ...
      for (Mesh::IndexType i = 0; i < n; i++)
        for (Mesh::IndexType j = 0; j < 2; j++)
          std::memcpy((ncdata + (i * 3)) + j, ((data + i) + (j * stride)),
                      sizeof(double));
    }
  }
}

void Mesh::NodalCoordinates::init_copy(Mesh::IndexType n, double *xdata,
                                       double *ydata, double *zdata) {
  destroy();
  if (n > 0 && xdata && ydata && zdata) {
    ncdata = new double[3 * n];
    ...
    mydata = true;
    for (Mesh::IndexType i = 0; i < n; i++) {
      std::memcpy(ncdata + (i * 3), xdata + i, sizeof(double));
      std::memcpy(ncdata + (i * 3) + 1, ydata + i, sizeof(double));
      std::memcpy(ncdata + (i * 3) + 2, zdata + i, sizeof(double));
    }
  }
}

void Mesh::NodalCoordinates::cleanup(double tol) {
  for (unsigned int i = 0; i < 3 * nnodes; i++) {
    if (std::abs(ncdata[i]) < tol) ncdata[i] = 0.0;
  }
}

void DisplaceNodalCoordinates(Mesh::NodalCoordinates &nc,
                              std::vector<double> &displacements) {
  unsigned int nnodes = nc.NNodes();
  if (displacements.size() != 3 * nnodes) {
    std::cerr << "SolverUtils::Mesh::DisplaceNodalCoordinates:Error: size of "
                 "displacements array not consistent with number of nodes."
              << std::endl;
    return;
  }
  std::vector<double>::iterator dispit = displacements.begin();
...
int CollideCellsWithBox(Mesh::NodalCoordinates &nc, Mesh::Connectivity &conn,
                        GeoPrim::CBox &box,
  ...
  while (ci != candidates.end()) {
    ...
    for (Mesh::IndexType node = 1; node <= esize; node++) {
    ...
    if (!cbox.empty()) cells.push_back(cell_id);
  }
  return (0);
}

int CollideMeshWithBox(Mesh::NodalCoordinates &nc, Mesh::Connectivity &conn,
                       GeoPrim::CBox &box,
                       std::vector<Mesh::IndexType> &cells) {
  ...
  while (n <= nelem) {
    ...
    // collide the ebox with the box
    GeoPrim::CBox cbox = box.intersect(ebox);
    if (!cbox.empty()) cells.push_back(n);
    n++;
  }
  return (0);
}

int GetMeshCentroids(Mesh::NodalCoordinates &nc, Mesh::Connectivity &conn,
...
void GetCoordinateBounds(NodalCoordinates &nc, std::vector<double> &bnds) {
  ...
  bnds[1] = bnds[3] = bnds[5] = -1.0 * std::numeric_limits<double>::max();
  Mesh::IndexType number_of_nodes = nc.Size();
  for (Mesh::IndexType nn = 1; nn <= number_of_nodes; nn++) {
    if (nc.x(nn) < bnds[0]) bnds[0] = nc.x(nn);
    if (nc.x(nn) > bnds[1]) bnds[1] = nc.x(nn);
    if (nc.y(nn) < bnds[2]) bnds[2] = nc.y(nn);
    if (nc.y(nn) > bnds[3]) bnds[3] = nc.y(nn);
    if (nc.z(nn) < bnds[4]) bnds[4] = nc.z(nn);
    if (nc.z(nn) > bnds[5]) bnds[5] = nc.z(nn);
  }
}
...
Mesh::Connectivity::Connectivity()
    : std::vector<std::vector<Mesh::IndexType> >() {
  this->_nelem = 0;
  this->resize(0);
  std::vector<std::vector<Mesh::IndexType> >(*this).swap(*this);
}

Mesh::Connectivity::Connectivity(unsigned int N)
    : std::vector<std::vector<Mesh::IndexType> >(N) {
  std::vector<std::vector<Mesh::IndexType> >(*this).swap(*this);
}
void Mesh::Connectivity::Resize(unsigned int N) { this->resize(N); }
Mesh::Connectivity::~Connectivity() { destroy(); }
void Mesh::Connectivity::AddElement(const std::vector<Mesh::IndexType> &elem) {
  _nelem++;
  this->push_back(elem);
}
void Mesh::Connectivity::AddElements(Mesh::IndexType nielem,
                                     Mesh::IndexType nnpe,
                                     const std::vector<Mesh::IndexType> &elem) {
  std::vector<Mesh::IndexType>::const_iterator ei = elem.begin();
  for (int i = 0; i < nielem; i++) {
    std::vector<Mesh::IndexType> inelem(nnpe, 0);
    for (int j = 0; j < nnpe; j++) inelem[j] = *ei++;
    this->AddElement(inelem);
  }
}
...
void Mesh::Connectivity::AddElement(Mesh::IndexType a, Mesh::IndexType b,
                                    Mesh::IndexType c) {
  _nelem++;
  std::vector<Mesh::IndexType> elem;
  elem.push_back(a);
  elem.push_back(b);
  elem.push_back(c);
  this->push_back(elem);
...
void Mesh::Connectivity::AddElement(Mesh::IndexType a, Mesh::IndexType b,
                                    Mesh::IndexType c, Mesh::IndexType d) {
  _nelem++;
  std::vector<Mesh::IndexType> elem;
  elem.push_back(a);
  elem.push_back(b);
...
void Mesh::Connectivity::AddElement(Mesh::IndexType a, Mesh::IndexType b,
                                    Mesh::IndexType c, Mesh::IndexType d,
                                    Mesh::IndexType e, Mesh::IndexType f,
                                    Mesh::IndexType g, Mesh::IndexType h) {
  _nelem++;
  std::vector<Mesh::IndexType> elem;
  elem.push_back(a);
  elem.push_back(b);
...
void Mesh::Connectivity::destroy() {
  ...
  while (ti != this->end()) {
    ti->resize(0);
    std::vector<Mesh::IndexType> tmp(*ti);
    ti->swap(tmp);
    ti++;
  }
  this->resize(0);
...
void Mesh::Connectivity::SyncSizes() {
  ...
  for (Mesh::IndexType i = 0; i < _nelem; i++) {
    _sizes[i] = (*this)[i].size();
    //	std::vector<Mesh::IndexType> tmp((*this)[i]);
    //	((*this)[i].swap(tmp));
  }
}
...
// is silently garbage.
void Mesh::Connectivity::InverseDegenerate(Connectivity &rc,
                                           Mesh::IndexType nnodes) const {
  if (nnodes <= 0)
    nnodes = MaxNodeId<Connectivity, std::vector<Mesh::IndexType> >(*this);
  rc.Resize(nnodes);
  rc.Sync();
  for (Mesh::IndexType i = 0; i < _nelem; i++) {
    if ((*this)[i].size() > 1) {
      std::vector<Mesh::IndexType>::const_iterator ii = (*this)[i].begin();
      while (ii != (*this)[i].end()) {
...
// warning, if called with unsync'd Connectivity table, result
// is silently garbage.
void Connectivity::Inverse(Connectivity &rc, Mesh::IndexType nnodes) const {
  if (nnodes <= 0)
    nnodes = MaxNodeId<Connectivity, std::vector<Mesh::IndexType> >(*this);
  //      Connectivity::const_iterator ci = this->begin();
  //      while(ci != this->end()){
  //	std::vector<Mesh::IndexType>::const_iterator ii = ci->begin();
  //	while(ii != ci->end()){
...
int Skin(Mesh::UnstructuredMesh &inmesh, Mesh::UnstructuredMesh &outmesh) {
  Mesh::Connectivity F_N;              // for every face, the nodes
  Mesh::Connectivity E_F;              // for every element, the faces
  Mesh::Connectivity F_E;              // for every face, the elements
  std::vector<Mesh::SymbolicFace> sf;  // symbolic faces
  Mesh::Connectivity dc;
  inmesh.con.SyncSizes();
  inmesh.con.Inverse(dc, inmesh.nc.Size());
  inmesh.con.BuildFaceConnectivity(F_N, E_F, sf, dc);
  E_F.ShrinkWrap();
  E_F.SyncSizes();
  ...
  F_E.SyncSizes();
  F_N.SyncSizes();
  Mesh::Connectivity &BoundaryConn = outmesh.con;
  Mesh::Connectivity::iterator fei = F_E.begin();
  while (fei != F_E.end()) {
    Mesh::IndexType faceid = (fei++ - F_E.begin()) + 1;
    if (F_E.Esize(faceid) == 1) BoundaryConn.AddElement(F_N.Element(faceid));
  }
  // Now iterate through the boundary connectivity and
  // identify the unique list of nodes
  std::list<Mesh::IndexType> boundary_nodes;
  fei = BoundaryConn.begin();
  while (fei != BoundaryConn.end()) {
    std::vector<Mesh::IndexType>::iterator fii = fei->begin();
    while (fii != fei->end()) boundary_nodes.push_back(*fii++);
    fei++;
  }
  boundary_nodes.sort();
  boundary_nodes.unique();
  // Create a map to update the face connectivity table
  std::map<Mesh::IndexType, Mesh::IndexType> remap;
  Mesh::IndexType node_id = 1;
  std::list<Mesh::IndexType>::iterator bni = boundary_nodes.begin();
  std::vector<double> boundary_coordinates(boundary_nodes.size() * 3, 0.0);
  while (bni != boundary_nodes.end()) {
    Mesh::IndexType node_index = node_id - 1;
    remap[*bni++] = node_id;
    boundary_coordinates[node_index * 3] = inmesh.nc.x(node_id);
    boundary_coordinates[(node_index * 3) + 1] = inmesh.nc.y(node_id);
    boundary_coordinates[(node_index * 3) + 2] = inmesh.nc.z(node_id);
    node_id++;
  }
  fei = BoundaryConn.begin();
  while (fei != BoundaryConn.end()) {
  ...
  outmesh.nc.init_copy(boundary_nodes.size(), &boundary_coordinates[0]);
  return (0);
}

void Connectivity::BuildFaceConnectivity(Connectivity &fcon, Connectivity &ef,
                                         std::vector<SymbolicFace> &sf,
                                         Connectivity &dc) const {
  //    this->Sync();
  //    this->SyncSizes();
  ...
  for (Mesh::IndexType element_being_processed = 1;
    ...
    for (Mesh::IndexType face_being_processed = 1;
         face_being_processed <= nfaces; face_being_processed++) {
      Mesh::IndexType findex = face_being_processed - 1;
      if (!ef[index][findex]) {  // 0 if face hasn't yet been processed
        fcon.push_back(all_face_conn[index][findex]);  // add the new face to
                                                       // the F[N] array
        ef[index][findex] =
            number_of_faces + 1;  // add the face id to the C[F] array
        Mesh::SubEntityId seid1;
        Mesh::SubEntityId seid2;
        ...
        // Now look at each cell containing each node of the current face and
        // determine which one (if any) have the same face.   This will be the
        // face neighbor of the element_being_processed for the face identified
        // by (number_of_faces + 1).
        //	    std::list<Mesh::IndexType> element_nbrlist;
        bool found = false;
        std::vector<Mesh::IndexType>::iterator fni =
            fcon[number_of_faces].begin();
        while (fni != fcon[number_of_faces].end() &&
               !found) {  // for every node in this face
          Mesh::IndexType face_node_index = *fni++ - 1;
          std::vector<Mesh::IndexType>::iterator enbri =
              dc[face_node_index].begin();
          while (enbri != dc[face_node_index].end() &&
                 !found) {  // look thru all the node's cells
            Mesh::IndexType enbr_index = *enbri++ - 1;
            if (enbr_index + 1 > element_being_processed) {
              Mesh::IndexType nbr_nfaces = ef.Esize(enbr_index + 1);
              for (Mesh::IndexType nbrface = 1; nbrface <= nbr_nfaces && !found;
                   nbrface++) {
                Mesh::IndexType nbr_face_index = nbrface - 1;
                if (!ef[enbr_index][nbr_face_index]) {
                  if (IRAD::Util::HaveOppositeOrientation(
                          all_face_conn[index][findex],
...
void Connectivity::graph_mode(Mesh::IndexType offset) {
...
//
// Converts from graph mode, where the "self" is not contained
// in the row, to matrix mode where it is.
void Connectivity::matrix_mode(Mesh::IndexType offset) {
  Connectivity::iterator ci = this->begin();
  Mesh::IndexType n = 1;
  while (ci != this->end()) {
    ci->insert(std::lower_bound(ci->begin(), ci->end(), offset + n), 1,
               offset + n);
    n++;
    ci++;
  }
...
void Connectivity::GetNeighborhood(NeighborHood &rl, const Connectivity &dc) {
  rl.resize(_nelem);
  //    rl._nelem = _nelem;
  for (Mesh::IndexType i = 0; i < _nelem; i++) {
    std::vector<Mesh::IndexType>::const_iterator ni = (*this)[i].begin();
    while (ni != (*this)[i].end()) {
      Mesh::IndexType index = *ni - 1;
      std::vector<Mesh::IndexType>::const_iterator dci = dc[index].begin();
      while (dci != dc[index].end()) rl[i].insert(*dci++);
      ni++;
    }
    rl[i].erase(i + 1);
  }
...
// input is dual connectivity (i.e. for every node, which elements)
void Connectivity::GetNeighborhood(Connectivity &rl, Connectivity &dc,
                                   bool exclude_self, bool sortit) {
  rl.Resize(_nelem);
  rl._nelem = _nelem;
  std::vector<bool> added(_nelem, false);
  for (Mesh::IndexType i = 0; i < _nelem; i++) {
    Mesh::IndexType current_element = i + 1;
    std::vector<Mesh::IndexType>::iterator ni = (*this)[i].begin();
    std::list<Mesh::IndexType> nbrlist;
    while (ni != (*this)[i].end()) {
      Mesh::IndexType index = *ni - 1;
      std::vector<Mesh::IndexType>::iterator dci = dc[index].begin();
      while (dci != dc[index].end()) {
        Mesh::IndexType ai = *dci - 1;
        if (!added[ai]) {
    ...
    Mesh::IndexType j = 0;
    std::list<Mesh::IndexType>::iterator si = nbrlist.begin();
...
// input is dual connectivity (i.e. for every node, which elements)
void Connectivity::GetAdjacent(Connectivity &rl, Connectivity &dc,
                               Mesh::IndexType n, bool sortit) {
  rl.Resize(_nelem);
  rl._nelem = _nelem;
  Mesh::IndexType nadj = n;
  if (n == 0) {
    std::cout << "GETADJACENT: Determining MaxNodeId" << std::endl;
    nadj = MaxNodeId<Connectivity, std::vector<Mesh::IndexType> >(dc);
  }
...
void Connectivity::ElementsOn(std::vector<Mesh::IndexType> &nodes,
  ...
  while (ni != nodes.end()) {
    ...
    while (dci != dc[node_index].end()) alist.push_back(*dci++);
  }
  alist.sort();
  alist.unique();
...
void Connectivity::BreadthFirstRenumber(std::vector<Mesh::IndexType> &remap) {
  ...
  Mesh::IndexType renumber = 1;
  for (Mesh::IndexType i = 0; i < _nelem && renumber <= _nelem; i++) {
    //    while(renumber <= _nelem){
    if (remap[i] == 0) {
      remap[i] = renumber++;
      std::list<Mesh::IndexType> processing_queue;
      std::vector<Mesh::IndexType>::iterator ni = (*this)[i].begin();
      while (ni != (*this)[i].end()) {
        Mesh::IndexType index = *ni++ - 1;
        if (remap[index] == 0) {
      ...
      std::list<Mesh::IndexType>::iterator pqi = processing_queue.begin();
      while (pqi != processing_queue.end()) {
        Mesh::IndexType index = *pqi;
        std::vector<Mesh::IndexType>::iterator ini = (*this)[index].begin();
        while (ini != (*this)[index].end()) {
          Mesh::IndexType iindex = *ini++ - 1;
          if (remap[iindex] == 0) {
...
GeoPrim::C3Point GenericCell_2::Centroid(std::vector<Mesh::IndexType> &ec,
                                         NodalCoordinates &nc) const {
  GeoPrim::C3Point centroid(0, 0, 0);
  Mesh::IndexType i = 0;
  Mesh::IndexType esize = ec.size();
  while (i < esize) centroid += GeoPrim::C3Point(nc[ec[i++]]);
...
void GenericCell_2::shape_func(const GeoPrim::CVector &natc,
                               std::vector<double> &SF) const {
  switch (_size) {
    case 3: {
...
void GenericCell_2::dshape_func(const GeoPrim::CVector &natc,
                                double Np[][2]) const {
  switch (_size) {
    case 3: {
      Np[0][0] = -1;
...
void GenericCell_2::jacobian(const std::vector<GeoPrim::CPoint> &p,
  ...
  switch (_size) {
    ...
    case 6: {
      ...
      J[0] = ((p[1] - p[0]) *= 4. * xi - 1.) +=
          ((p[3] - p[0]) *= 4. * zeta - 4 * xi) += ((p[4] - p[5]) *= 4. * eta);
      J[1] = ((p[2] - p[0]) *= 4. * eta - 1.) +=
          ((p[5] - p[0]) *= 4 * zeta - 4 * eta) += ((p[4] - p[3]) *= 4. * xi);
      return;
    }
    case 8: {
      const double xi = nc[0];
      ...
      J[0] = ((p[1] - p[0]) *= eta_minus) += ((p[2] - p[3]) *= eta) -=
          ((((p[0] - p[4]) += (p[1] - p[4])) *=
            2. * eta_minus * (xi_minus - xi)) +=
           (((p[2] - p[6]) += (p[3] - p[6])) *= 2. * eta * (xi_minus - xi)) +=
           (((p[1] - p[5]) += (p[2] - p[5]) -=
             ((p[0] - p[7]) += (p[3] - p[7]))) *= 2. * eta * eta_minus));

      ((J[1] = p[3] - p[0]) *= xi_minus) += ((p[2] - p[1]) *= xi) -=
          ((((p[1] - p[5]) += (p[2] - p[5])) *= 2. * xi * (eta_minus - eta)) +=
           (((p[0] - p[7]) += (p[3] - p[7])) *=
            2. * xi_minus * (eta_minus - eta)) +=
           (((p[2] - p[6]) += (p[1] - p[4]) -=
             ((p[3] - p[6]) += (p[0] - p[4]))) *= 2. * xi * xi_minus));
      return;
    }
    default:
...
double GenericCell_2::Area(Mesh::IndexType elnum, const Connectivity &ec,
  ...
  if (_size == 3) {
    ...
    GeoPrim::CPoint p2(nc[ec.Node(elnum, 3)]);
    return (.5 * (GeoPrim::CVector(p1 - p0) % GeoPrim::CVector(p2 - p0)).mag());
  }
  if (_size == 4) {
    GeoPrim::CPoint p0(nc[ec.Node(elnum, 1)]);
    GeoPrim::CPoint p1(nc[ec.Node(elnum, 2)]);
    GeoPrim::CPoint p2(nc[ec.Node(elnum, 3)]);
    GeoPrim::CPoint p3(nc[ec.Node(elnum, 4)]);
    return (.5 * (GeoPrim::CVector(p2 - p0) % GeoPrim::CVector(p1 - p3)).mag());
  }
  return (0);
}

void GenericCell_2::GetNormalSet(GeoPrim::CVector ns[], Mesh::IndexType elnum,
                                 const Connectivity &ec,
                                 const NodalCoordinates &nc) {
  GeoPrim::CPoint P[3];
  for (Mesh::IndexType i = 0; i < _size; i++) {
    Mesh::IndexType i_minus = (i == 0 ? _size - 1 : i - 1);
    P[i_minus] = nc[ec.Node(elnum, i_minus + 1)];
    P[i] = nc[ec.Node(elnum, i + 1)];
    Mesh::IndexType i_plus = (i == (_size - 1) ? 0 : i + 1);
    P[i_plus] = nc[ec.Node(elnum, i_plus + 1)];
    ns[i] = GeoPrim::CVector(P[i] - P[i_minus]) %
            GeoPrim::CVector(P[i_plus] - P[i]);
  }
}

void GenericCell_2::GetPointSet(GeoPrim::CPoint P[], Mesh::IndexType elnum,
                                const Connectivity &ec,
                                const NodalCoordinates &nc) {
  for (Mesh::IndexType i = 0; i < _size; i++)
...
void GenericCell_2::shapef_jacobian_at(
    const GeoPrim::CPoint &p, GeoPrim::CVector &natc, Mesh::IndexType elnum,
    const Connectivity &ec, const NodalCoordinates &nc, GeoPrim::CVector &fvec,
    GeoPrim::CVector jac[]) const  // fjac is 3x2
{
  // GeoPrim::CPoint P[(Mesh::IndexType)_size];
  std::vector<GeoPrim::CPoint> P;
  P.reserve((Mesh::IndexType)_size);
  // double SF[(Mesh::IndexType)_size];
  std::vector<double> SF;
  SF.reserve((Mesh::IndexType)_size);
...
void GenericCell_2::interpolate(const GeoPrim::CVector f[],
  ...
  switch (_size) {
    ...
    case 4: {
      v += ((f[1] - f[0]) *= (xi * (1. - eta))) += ((f[3] - f[0]) *= eta) +=
          ((f[2] - f[3]) *= xi * eta);
      return;
    }
    case 6: {
    ...
    case 8: {
      const double xi_minus = 1. - xi;
      const double eta_minus = 1. - eta;
      v += ((f[1] - f[0]) *= eta * (2. * eta - 1.)) += ((f[3] - f[0]) *= eta) +=
          ((f[2] - f[3]) *= xi * eta) -=
          ((((f[0] - f[4]) += (f[1] - f[4])) *=
            2. * xi * xi_minus * eta_minus) +=
           (((f[2] - f[6]) += (f[3] - f[6])) *= 2. * xi * xi_minus * eta) +=
           (((f[1] - f[5]) += (f[2] - f[5])) *= 2. * xi * eta * eta_minus) +=
           (((f[0] - f[7]) += (f[3] - f[7])) *=
            2. * xi_minus * eta * eta_minus));
      return;
    }
    default:
      assert(false);
...
void GenericCell_2::ReOrient(std::vector<Mesh::IndexType> &ec) {
  switch (_size) {
    case 3: {
...
// Evaluates the shape function and jacobian at a given point
void GenericElement::shapef_jacobian_at(
    const GeoPrim::CPoint &p, GeoPrim::CVector &natc, Mesh::IndexType elnum,
    const Connectivity &ec, const NodalCoordinates &nc, GeoPrim::CVector &fvec,
  ...
  for (Mesh::IndexType i = 0; i < _size; i++) {
    //      std::cout << "debugging: " <<
    //      GeoPrim::CPoint(nc[ec.Node(elnum,i+1)]) << std::endl;
    P[i].init(nc[ec.Node(elnum, i + 1)]);
    fvec += SF[i] * P[i];
  }
...
// Populate SF with the shape function of the element at the nat.c.
void GenericElement::shape_func(const GeoPrim::CVector &nc,
                                std::vector<double> &SF) const {
  switch (_size) {
...
// at nat.c.
void GenericElement::dshape_func(const GeoPrim::CVector &nc,
                                 double dSF[][3]) const {
  switch (_size) {
    case 4: {
      dSF[0][0] = -1;
    ...
    case 8: {
      ...
      const double zeta = nc.z();
      const double zeta_minus = 1. - zeta;
      dSF[0][0] = -1. * eta_minus * zeta_minus;
      dSF[0][1] = -1. * xi_minus * zeta_minus;
      dSF[0][2] = -1. * xi_minus * eta_minus;
      dSF[1][0] = eta_minus * zeta_minus;
      dSF[1][1] = -1. * xi * zeta_minus;
      dSF[1][2] = -1. * xi * eta_minus;
      dSF[2][0] = eta * zeta_minus;
      dSF[2][1] = xi * zeta_minus;
      dSF[2][2] = -1. * xi * eta;
      dSF[3][0] = -1. * eta * zeta_minus;
      dSF[3][1] = xi_minus * zeta_minus;
      dSF[3][2] = -1. * xi_minus * eta;
      dSF[4][0] = -1. * eta_minus * zeta;
      dSF[4][1] = -1. * xi_minus * zeta;
      dSF[4][2] = xi_minus * eta_minus;
      dSF[5][0] = eta_minus * zeta;
      dSF[5][1] = -1. * xi * zeta;
      dSF[5][2] = xi * eta_minus;
      dSF[6][0] = eta * zeta;
      dSF[6][1] = xi * zeta;
      dSF[6][2] = xi * eta;
      dSF[7][0] = -1. * eta * zeta;
      dSF[7][1] = xi_minus * zeta;
      dSF[7][2] = xi_minus * eta;
      break;
    }
    default:
...
void GenericElement::jacobian(const std::vector<GeoPrim::CPoint> &p,
                              const GeoPrim::CVector &nc,
                              GeoPrim::CVector J[]) const {
  switch (_size) {
    case 4: {
    ...
    case 8: {
      ...
      const double zeta = nc.z();
      const double zeta_minus = 1. - zeta;
      J[0] = ((p[6] - p[7]) *= eta * zeta) +=
          ((p[5] - p[4]) *= eta_minus * zeta) +=
          ((p[2] - p[3]) *= eta * zeta_minus) +=
          ((p[1] - p[0]) *= eta_minus * zeta_minus);
      J[1] = ((p[7] - p[4]) *= xi_minus * zeta) +=
          ((p[6] - p[5]) *= xi * zeta) +=
          ((p[3] - p[0]) *= xi_minus * zeta_minus) +=
          ((p[2] - p[1]) *= xi * zeta_minus);
      J[2] = ((p[7] - p[3]) *= xi_minus * eta) += ((p[6] - p[2]) *= xi * eta) +=
          ((p[5] - p[1]) *= xi * eta_minus) +=
          ((p[4] - p[0]) *= xi_minus * eta_minus);
      break;
    }
    default:
...
// Interpolate a field value from the nodes of the element to the point
// having natural coordinates nc.
void GenericElement::interpolate(const GeoPrim::CVector f[],
                                 const GeoPrim::CVector &nc,
                                 GeoPrim::CVector &v) const {
  const double xi = nc.x();
  const double eta = nc.y();
  ...
  switch (_size) {
    ...
    case 10: {
      ...
      v = (alpha * (1. - 2. * (xi + eta + zeta)) * f[0] +
           xi * (2. * xi - 1.) * f[1] + eta * (2. * eta - 1.) * f[2] +
           zeta * (2. * zeta - 1.) * f[3] + 4. * xi * alpha * f[4] +
           4. * eta * alpha * f[5] + 4. * zeta * alpha * f[6] +
           4. * xi * eta * f[7] + 4. * eta * zeta * f[8] +
           4. * zeta * xi * f[9]);
      break;
    }
    case 8: {
      ...
      v = (xi_minus * eta_minus * zeta_minus * f[0] +
           ...
           xi_minus * eta_minus * zeta * f[4] + xi * eta_minus * zeta * f[5] +
           xi * eta * zeta * f[6] + xi_minus * eta * zeta * f[7]);
      break;
    }
    default:
...
GeoPrim::C3Point GenericElement::Centroid(std::vector<Mesh::IndexType> &ec,
                                          NodalCoordinates &nc) const {
  GeoPrim::C3Point centroid(0, 0, 0);
  Mesh::IndexType i = 0;
  Mesh::IndexType esize = ec.size();
...
void GenericElement::Centroid(std::vector<Mesh::IndexType> &ec,
                              NodalCoordinates &nc,
                              GeoPrim::C3Point &centroid) const {
  centroid.Init();
  Mesh::IndexType i = 0;
  Mesh::IndexType esize = ec.size();
  while (i < esize) centroid += GeoPrim::C3Point(nc[ec[i++]]);
...
bool GenericElement::Inverted(std::vector<Mesh::IndexType> &ec,
                              NodalCoordinates &nc) const {
  ...
  if (_size == 4 || _size == 10) {  // Tet Faces:      132 241 342 143
    ...
    return ((avec * aface.Normal()) < 0);
             ...
             _size == 13) {  // Pyr Faces:      1432 251 352 453 154
    GeoPrim::C3Point p1(nc[ec[0]]);
    ...
    return ((bvec * bface.Normal()) < 0);
             ...
             _size == 15) {  // Prism Faces:    2541 3652 1463 132 456
    ...
    return ((cvec * cface.Normal()) < 0);
             ...
             _size == 20) {  // Hex Faces:      1432 2651 3762 4873 1584 5678
    GeoPrim::C3Point p1(nc[ec[0]]);
    GeoPrim::C3Point p2(nc[ec[3]]);
    GeoPrim::C3Point p3(nc[ec[2]]);
    GeoPrim::C3Point p4(nc[ec[1]]);
...
bool TRIANGLE_OK(const GeoPrim::C3Point &p1, const GeoPrim::C3Point &p2,
                 const GeoPrim::C3Point &p3) {
  GeoPrim::C3Line line(p1, p2);
  if (line.has_point(p3)) return (false);
  return (true);
}
bool QUAD_OK(const GeoPrim::C3Point &p1, const GeoPrim::C3Point &p2,
             const GeoPrim::C3Point &p3, const GeoPrim::C3Point &p4) {
  // Make sure 3 points make a triangle
  if (!TRIANGLE_OK(p1, p2, p3)) return (false);
  if (!TRIANGLE_OK(p1, p3, p4)) return (false);
  if (!TRIANGLE_OK(p1, p2, p4)) return (false);
  if (!TRIANGLE_OK(p2, p3, p4)) return (false);
  GeoPrim::C3Plane plane1(p1, p2, p3);
  if (!plane1.contains_point(p4)) return (false);
  return (true);
}
bool GenericElement::ShapeOK(std::vector<Mesh::IndexType> &ec,
                             NodalCoordinates &nc) const {
  ...
  //   make sure 5th point not co-planar with quad base
    ...
    GeoPrim::C3Point p3(nc[ec[2]]);
    GeoPrim::C3Point p4(nc[ec[3]]);
    if (!QUAD_OK(p1, p2, p3, p4)) return (false);
    GeoPrim::C3Point p5(nc[ec[4]]);
    GeoPrim::C3Plane plane1(p1, p2, p3);
    if (plane1.contains_point(p5)) return (false);
    return (true);
  }
  // ensure element is not poorly shaped
  ...
  //   make sure quad faces have positive area
    ...
    if (!TRIANGLE_OK(p4, p5, p6)) return (false);
    if (!QUAD_OK(p2, p5, p4, p1)) return (false);
    if (!QUAD_OK(p3, p6, p5, p2)) return (false);
    if (!QUAD_OK(p1, p4, p6, p3)) return (false);
    return (true);
  }
  // ensure element is not poorly shaped
  ...
  //   make sure each face has positive area
    ...
    if (!QUAD_OK(p1, p4, p3, p2)) return (false);
    if (!QUAD_OK(p2, p6, p5, p1)) return (false);
    if (!QUAD_OK(p3, p7, p6, p2)) return (false);
    if (!QUAD_OK(p4, p8, p7, p4)) return (false);
    if (!QUAD_OK(p1, p5, p8, p4)) return (false);
    if (!QUAD_OK(p5, p6, p7, p8)) return (false);
    return (true);
  } else
    return (true);

  // Never gets here - but eliminates compile warning
  return (true);
}

void GenericElement::ReOrient(std::vector<Mesh::IndexType> &ec) {
  Mesh::IndexType temp;
  switch (_size) {
...
void GetMeshBoxes(const NodalCoordinates &nc, const Connectivity &ec,
                  GeoPrim::CBox &mesh_box, GeoPrim::CBox &small_box,
                  GeoPrim::CBox &large_box) {
  mesh_box.init(nc[1], nc.size());
  small_box = mesh_box;
  Mesh::IndexType nelem = ec.Nelem();
  Mesh::IndexType n = 1;
  while (n <= nelem) {
...
/// list<Mesh::IndexType> with all the elements that lie within.
/// \n
/// Inputs: CBox object (see GeoPrimitives.H)
///         NodalCoordinates object
///         ElementConnectivity object
///         DualConnectivity object
///         list<Mesh::IndexType> stores results
///
///
void FindElementsInBox(const GeoPrim::CBox &box, const NodalCoordinates &nc,
  ...
  for (int i = 1; i <= nnodes; i++) {
    ...
    if (box.contains(p)) {
      std::vector<Mesh::IndexType>::const_iterator dci = dc[i - 1].begin();
...
///
Mesh::IndexType FindPointInCells(
    const GeoPrim::CPoint &p,                      // Target point
    ...
    GeoPrim::CVector &natc)                        // Returns Targ nat
{
  ...
  std::vector<Mesh::IndexType>::const_iterator ei = elements.begin();
  while (ei != elements.end()) {
    ...
    if (!NewtonRaphson(natc, *ei, GenericElement(esize), ec, nc, p)) {
      //	double dist = 0.0;
      //	Mesh::Mesh::IndexType closest_node = nc.closest_node(p,&dist);
      //	std::cout << "Mesh::FindPointInMesh: Error: Closest approach: "
      //<< dist
      //		  << " at node " << closest_node << "." << std::endl
      //		  << "Mesh::FindPointInMesh: Element(" << *ei << ") =
      //("; 	DumpContents(std::cout,ec[*ei-1],","); 	std::cout << ")" <<
      //std::endl; 	std::cout << "Point: " << p << std::endl
      //	          << "Nodes: " << std::endl;
      //	std::vector<Mesh::IndexType>::const_iterator ni =
      //ec[*ei-1].begin(); 	while(ni != ec[*ei-1].end()){ 	  std::cout << ni -
      //ec[*ei-1].begin() << ": " << GeoPrim::C3Point(nc[*ni]) << std::endl;
      //	  ni++;
      //	}
      //	assert(false);
      return (0);
    }
    if (natc[0] >= LTOL && natc[0] <= HTOL && natc[1] >= LTOL &&
        natc[1] <= HTOL && natc[2] >= LTOL && natc[2] <= HTOL) {
      if (esize == 4 || esize == 10) {
        if ((natc[0] + natc[1] + natc[2]) <= HTOL) return (*ei);
      } else if (esize == 8 || esize == 20)
        return (*ei);
      else {
...
///          the containing element is returned in natc
///
Mesh::IndexType FindPointInMesh(
    const GeoPrim::CPoint &p,    // Target Mesh point
    const NodalCoordinates &nc,  // Source
    const Connectivity &ec,      // Source connectivity
    const Connectivity &dc,      // Source dual connectivity
    const GeoPrim::CBox
        &box,  // neigborhood (typically defined by some source character)
    GeoPrim::CVector &natc)  // Returns Targ nat
{
  GeoPrim::CBox bounds(box.around(p));
  std::list<Mesh::IndexType> elements;
  FindElementsInBox(bounds, nc, dc, elements);
  std::list<Mesh::IndexType>::iterator ei = elements.begin();
  while (ei != elements.end()) {
    GeoPrim::CVector guess;
    unsigned int esize = ec.Esize(*ei);
    if (esize == 4 || esize == 10)
    ...
    if (!NewtonRaphson(natc, *ei, GenericElement(esize), ec, nc, p)) {
      ...
      std::cout << ")" << std::endl;
      std::vector<Mesh::IndexType>::const_iterator ei2 =
          dc[closest_node - 1].begin();
      ...
      while (ei2 != dc[closest_node - 1].end()) {
        ...
        while (ni != ec[*ei2 - 1].end()) new_bounds.AddPoint(nc[*ni++]);
        ei2++;
      }
...
///          the containing element is returned in natc
///
Mesh::IndexType FindPointInMesh_2(
    const GeoPrim::CPoint &p,    // Target Mesh point
    const NodalCoordinates &nc,  // Source
    const Connectivity &ec,      // Source connectivity
    const Connectivity &dc,      // Source dual connectivity
    ...
    GeoPrim::CVector &natc)      // Returns Targ nat
{
  ...
  while (ei != elements.end()) {
    ...
    if (natc[0] >= LTOL && natc[0] <= HTOL && natc[1] >= LTOL &&
        natc[1] <= HTOL) {
      if (esize == 3 || esize == 6) {
        if (((natc[0] + natc[1]) <= HTOL)) return (*ei);
      } else if (esize == 4 || esize == 8)
        return (*ei);
      else {
        std::cerr << "Mesh::FindPointInMesh: Error: Cannot handle"
...
Mesh::IndexType GlobalFindPointInMesh(
    const GeoPrim::CPoint &p,    // Target Mesh point
    const NodalCoordinates &nc,  // Source
...
void GenericElement::get_face_connectivities(
    Connectivity &rv, const std::vector<Mesh::IndexType> &e) const {
  //    std::vector<std::vector<Mesh::IndexType> > rv;
  switch (_size) {
    case 4:  // Tet Faces:      132 241 342 143
    case 10:
      rv.Resize(4);
      ...
      rv[0].resize(3);
      //      std::vector<Mesh::Mesh::IndexType>(rv[0]).swap(rv[0]);
      rv[0][0] = e[0];  // 1
      rv[0][1] = e[2];  // 3
      rv[0][2] = e[1];  // 2
...
/// Mesh::IndexType elnum = index of the mesh element to use
...
///
bool NewtonRaphson(GeoPrim::CVector &natc, Mesh::IndexType elnum,
                   const GenericElement &el, const Connectivity &ec,
                   const NodalCoordinates &nc, const GeoPrim::CPoint &point) {
  int k, i;
  ...
  for (k = 0; k < ntrial; k++) {
    ...
    //	<< "err: "  << errf    << std::endl;
    if (!LUDcmp(fjac, indx)) {
      //	std::cerr << "NewtonRaphson::error: LUDcmp failed." <<
      //std::endl; 	std::cout << "Jac: " << fjac[0] << std::endl
      //		  << "Jac: " << fjac[1] << std::endl
      //		  << "Jac: " << fjac[2] << std::endl;
      return (false);
    }
    LUBksb(fjac, indx, p);
    errx = 0.0;
    for (i = 0; i < 3; i++) errx += fabs(p[i]);
    natc += p;
    if (errx <= errtol) return (true);
  }
  //    std::cerr << "NewtonRaphson::warning: reached maximum iterations" <<
  //    std::endl;
  return (false);
}
///
...
/// const CPoint &point = the point at which we wish a solution
///
bool NewtonRaphson_2(GeoPrim::CVector &natc, Mesh::IndexType elnum,
                     const GenericCell_2 &el, const Connectivity &ec,
                     const NodalCoordinates &nc, const GeoPrim::CPoint &point) {
  int k, i;
  ...
  for (k = 0; k < ntrial; k++) {
    el.shapef_jacobian_at(point, natc, elnum, ec, nc, fvec, fjac);
    errf = 0.0;
    for (i = 0; i < 3; i++) errf += fabs(fvec[i]);
    if (errf <= TOL) return (true);
    p = -1.0 * fvec;
    if (!LUDcmp(fjac, indx)) {
      //	std::cerr << "NewtonRaphson::error: LUDcmp failed." <<
      //std::endl;
...
std::istream &operator>>(std::istream &iSt, Mesh::Connectivity &ec) {
  ...
  while (line.empty()) std::getline(iSt, line);
  std::istringstream Istr(line);
  Mesh::IndexType nelem = 0;
  Istr >> nelem;
  if (nelem > 0) {
    ec.Resize(nelem);
    ec._nelem = nelem;
    for (Mesh::IndexType i = 0; i < nelem; i++) {
      std::getline(iSt, line);
      std::istringstream Istr2(line);
      Mesh::IndexType node;
      ec[i].reserve(8);
      while (Istr2 >> node) ec[i].push_back(node);
...
std::istream &operator>>(std::istream &iSt, Mesh::NodalCoordinates &nc) {
  ...
  if (nnodes > 0) {
    nc.init(nnodes);
    for (Mesh::IndexType n = 1; n <= nc.size(); n++) {
      double *inPtr = nc[n];
      std::getline(iSt, line);
      std::istringstream Istr2(line);
...
std::ostream &operator<<(std::ostream &oSt, const Mesh::NodalCoordinates &nc) {
  ...
  if (nc.size() > 0) oSt << "\n";
  for (Mesh::IndexType n = 1; n <= nc.size(); n++) {
    oSt << std::setw(16) << nc[n][0] << "\t" << std::setw(16) << nc[n][1]
        << "\t" << std::setw(16) << nc[n][2];
    if (n != nc.size()) oSt << std::endl;
  }
  return (oSt);
}
...
std::ostream &operator<<(std::ostream &oSt, const Mesh::Connectivity &ec) {
  ...
  while (ci != ec.end()) {
    std::vector<Mesh::IndexType>::const_iterator ni = ci->begin();
    while (ni != ci->end()) {
      oSt << *ni++;
      if (ni != ci->end()) oSt << "\t";
    }
...
int WriteVTKToStream(const std::string &name, Mesh::UnstructuredMesh &mesh,
                     std::ostream &Ostr) {
  Mesh::NodalCoordinates &nc = mesh.nc;
  Mesh::Connectivity &con = mesh.con;
  Mesh::IndexType nnodes = nc.NNodes();
  Mesh::IndexType totalsize = 0;
  Mesh::IndexType nelem = con.Nelem();
  Mesh::IndexType i = 1;

  con.SyncSizes();
  while (i <= nelem) totalsize += con.Esize(i++);
  ...
  i = 1;
  while (i <= nnodes) Ostr << GeoPrim::C3Point(nc[i++]) << std::endl;

  Ostr << "CELLS " << nelem << " " << nelem + totalsize << std::endl;
  i = 1;
  while (i <= nelem) {
    Ostr << con.Esize(i) << " ";
    std::vector<Mesh::IndexType>::iterator ni = con[i - 1].begin();
    while (ni != con[i - 1].end()) Ostr << *ni++ - 1 << " ";
    Ostr << std::endl;
    i++;
  }

  Ostr << "CELL_TYPES " << nelem << std::endl;
  Mesh::GenericElement ge(4);
  i = 1;
  while (i <= nelem) {
    unsigned int esize = con.Esize(i);
    Ostr << ((esize == 2)
                 ? "3"
                 : (esize == 3)
                       ? "5"
                       : (esize == 4)
                             ? (ge.ShapeOK(con[i - 1], nc) ? "10" : "9")
                             : (esize == 5)
                                   ? "14"
                                   : (esize == 6) ? "13"
                                                  : (esize == 8) ? "12" : "-1")
...
int ReadWRLFromStream(Mesh::UnstructuredMesh &mesh, std::istream &Istr) {
  std::string line;
  Mesh::NodalCoordinates &nc = mesh.nc;
  Mesh::Connectivity &ec = mesh.con;
  std::vector<double> coords;

  std::getline(Istr, line);
  std::string::size_type x = line.find("point [");
  ...
  nc.init_copy(number_of_nodes, &(coords[0]));
  getline(Istr, line);
  x = line.find("coordIndex [");
  while (x == std::string::npos && !Istr.eof()) {
    getline(Istr, line);
    x = line.find("coordIndex [");
  }
  if (Istr.eof()) {
  ...
  while (x == std::string::npos && !Istr.eof()) {
    ...
    std::string delim;
    int dummy;
    std::vector<Mesh::IndexType> element;
    while (Istr2 >> dummy) {
      if (dummy >= 0) {
        element.push_back(dummy);
        Istr2 >> delim;
      }
    }
    std::vector<Mesh::IndexType>::iterator ei = element.begin();
    while (ei != element.end()) (*ei++)++;
    ec.AddElement(element);
    std::getline(Istr, line);
    x = line.find("]");
  }
  if (Istr.eof()) return (-4);
  return (0);
}
}  // namespace Mesh
}  // namespace SolverUtils
