...
namespace SolverUtils { 
  namespace Mesh {
    ...
    Mesh::NodalCoordinates::NodalCoordinates(Mesh::IndexType n,double *data,int stride) {
    ...
    Mesh::NodalCoordinates::NodalCoordinates(Mesh::IndexType n,double *xdata,double *ydata,double *zdata){
    ...
    void Mesh::NodalCoordinates::init_node(Mesh::IndexType n,const GeoPrim::CPoint &point)
    {
      this->x(n) = point.x();
      this->y(n) = point.y();
      this->z(n) = point.z();
    ...
    void Mesh::NodalCoordinates::init_copy(Mesh::IndexType n, double *data,int stride)
    {
      destroy();
      if(n > 0 && data){
        ncdata = new double [3*n];
        ...
        if(stride == 1)
          ...
          for(Mesh::IndexType i = 0;i < n;i++)
            for(Mesh::IndexType j = 0;j < 2;j++)
              std::memcpy((ncdata+(i*3))+j,((data+i)+(j*stride)),sizeof(double));
        }
      }
    };

    void Mesh::NodalCoordinates::init_copy(Mesh::IndexType n, double *xdata,double *ydata,double *zdata)
    {
      destroy();
      if(n > 0 && xdata && ydata && zdata){
        ncdata = new double [3*n];
    ...
    void DisplaceNodalCoordinates(Mesh::NodalCoordinates &nc,std::vector<double> &displacements)
    {
      ...
      if(displacements.size() != 3*nnodes){
        std::cerr << "SolverUtils::Mesh::DisplaceNodalCoordinates:Error: size of displacements array not consistent with number of nodes."
                  << std::endl;
        return;
      }
    ...
    Mesh::Connectivity::Connectivity() : std::vector<std::vector<Mesh::IndexType> >()
    {
      this ->_nelem = 0;
      this->resize(0);
    ...
    Mesh::Connectivity::Connectivity(unsigned int N) : std::vector<std::vector<Mesh::IndexType> >(N)
    {
      std::vector<std::vector<Mesh::IndexType> >(*this).swap(*this);
    };
    ...
    void Mesh::Connectivity::AddElements(Mesh::IndexType nielem,Mesh::IndexType nnpe,
    ...
    void Mesh::Connectivity::AddElement(Mesh::IndexType a,Mesh::IndexType b,Mesh::IndexType c)
    {
      _nelem++;
    ...
    void Mesh::Connectivity::AddElement(Mesh::IndexType a,Mesh::IndexType b,Mesh::IndexType c,Mesh::IndexType d)
    {
      _nelem++;
      std::vector<Mesh::IndexType> elem;
      elem.push_back(a);
    ...
    // is silently garbage.
    void Mesh::Connectivity::InverseDegenerate(Connectivity &rc,Mesh::IndexType nnodes) const
    {
      if(nnodes <= 0)
    ...
    GeoPrim::C3Point
    GenericCell_2::Centroid(std::vector<Mesh::IndexType> &ec,NodalCoordinates &nc) const
    {
    ...
    double GenericCell_2::Area(Mesh::IndexType elnum,
    ...
    GeoPrim::C3Point
    GenericElement::Centroid(std::vector<Mesh::IndexType> &ec,NodalCoordinates &nc) const
    {
      GeoPrim::C3Point centroid(0,0,0);
      Mesh::IndexType i = 0;
      Mesh::IndexType esize = ec.size();
    ...
    void
    GenericElement::Centroid(std::vector<Mesh::IndexType> &ec,NodalCoordinates &nc,GeoPrim::C3Point &centroid) const
    {
      centroid.Init();
      Mesh::IndexType i = 0;
      Mesh::IndexType esize = ec.size();    
    ...
    bool
    GenericElement::Inverted(std::vector<Mesh::IndexType> &ec,NodalCoordinates &nc) const
    {
    ...
    bool TRIANGLE_OK(const GeoPrim::C3Point &p1,const GeoPrim::C3Point &p2, const GeoPrim::C3Point &p3)
    {
      GeoPrim::C3Line line(p1,p2);
      if(line.has_point(p3))
        return(false);
      return(true);
    }
    bool QUAD_OK(const GeoPrim::C3Point &p1, const GeoPrim::C3Point &p2, const GeoPrim::C3Point &p3, const GeoPrim::C3Point &p4)
    {
      // Make sure 3 points make a triangle
      if(!TRIANGLE_OK(p1,p2,p3))
        return(false);
      if(!TRIANGLE_OK(p1,p3,p4))
        return(false);
      if(!TRIANGLE_OK(p1,p2,p4))
        return(false);
      if(!TRIANGLE_OK(p2,p3,p4))
        return(false);
      GeoPrim::C3Plane plane1(p1,p2,p3);
      if(!plane1.contains_point(p4))
        return(false);
      return(true);
    }
    bool
    GenericElement::ShapeOK(std::vector<Mesh::IndexType> &ec,NodalCoordinates &nc) const
    {
      ...
      //   make sure 5th point not co-planar with quad base
        ...
        GeoPrim::C3Point p3(nc[ec[2]]);
        GeoPrim::C3Point p4(nc[ec[3]]);
        if(!QUAD_OK(p1,p2,p3,p4))
          return(false);
        GeoPrim::C3Point p5(nc[ec[4]]);
        GeoPrim::C3Plane plane1(p1,p2,p3);
        if(plane1.contains_point(p5))
          return(false);
        return(true);
      }
      // ensure element is not poorly shaped
      ...
      //   make sure quad faces have positive area
        ...
        if(!TRIANGLE_OK(p4,p5,p6))
          return(false);
        if(!QUAD_OK(p2,p5,p4,p1))
          return(false);
        if(!QUAD_OK(p3,p6,p5,p2))
          return(false);
        if(!QUAD_OK(p1,p4,p6,p3))
          return(false);
        return(true);
      }
      // ensure element is not poorly shaped
      ...
      //   make sure each face has positive area
        ...
        if(!QUAD_OK(p1,p4,p3,p2))
          return(false);
        if(!QUAD_OK(p2,p6,p5,p1))
          return(false);
        if(!QUAD_OK(p3,p7,p6,p2))
          return(false);
        if(!QUAD_OK(p4,p8,p7,p4))
          return(false);
        if(!QUAD_OK(p1,p5,p8,p4))
          return(false);
        if(!QUAD_OK(p5,p6,p7,p8))
          return(false);
        return(true);
      }
      else
    ...
    void
    GenericElement::ReOrient(std::vector<Mesh::IndexType> &ec)
    {
      Mesh::IndexType temp;
      switch(_size){
    ...
    GenericElement::get_face_connectivities(Connectivity &rv,
                                            const std::vector<Mesh::IndexType> &e) const
    {
...
#undef LUDTINY
    ...
    int WriteVTKToStream(const std::string &name,Mesh::UnstructuredMesh &mesh,std::ostream &Ostr)
    {
      ...
      i = 1;
      while(i <= nnodes)
        Ostr << GeoPrim::C3Point(nc[i++]) << std::endl;
      ...
      while(i <= nelem){
        ...
        Ostr << ((esize == 2) ? "3" :
                 (esize == 3) ? "5" :
                 (esize == 4) ? (ge.ShapeOK(con[i-1],nc) ? "10" : "9") :
                 (esize == 5) ? "14" :
                 (esize == 6) ? "13" :
                 (esize == 8) ? "12" : "-1") << std::endl;
        i++;
      }      
      return(0);
    };
