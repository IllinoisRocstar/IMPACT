...
int main(int argc, char *argv[]) {
  ...
  while (pi != points.end()) {
    ...
    Mesh::IndexType element_id =
        FindPointInMesh_2(*pi++, nc, con, dual_con, box, natc);
    if (!element_id)
      std::cout << "Could not find point " << point_number << " in the mesh"
                << std::endl;
    else {
      ...
      std::cout << "The interpolated coordinates are: " << point_coords
                << std::endl;
    }
  }
  return (0);
