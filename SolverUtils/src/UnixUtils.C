...
int Remove(const std::string &fname) {
  ...
  if (ISDIR(fname)) {
    ...
    if (RemoveDir.size() != 0) {
      for (std::vector<std::string>::iterator it = RemoveDir.begin();
           it != RemoveDir.end(); ++it) {
        if (Remove(fname + "/" + *it) < 0) return -1;
      }
    }
...
bool ISDIR(const std::string &fname) {
  struct stat fstat;
  if (stat(fname.c_str(), &fstat)) return false;
...
const std::string ResolveLink(const std::string &path) {
  ...
  if (x != std::string::npos) retVal.erase(x);
  return (retVal);
}
...
void TokenizePath(std::vector<std::string> rv, const std::string &path) {
  ...
  while (std::getline(Istr, tok, '/'))
    if (!tok.empty()) rv.push_back(tok);
}

Environment::Environment() { this->init(); }

void Environment::init() {
  this->clear();
  std::vector<std::string> raw_env;
  unsigned int i = 0;
  while (environ[i]) raw_env.push_back(environ[i++]);
...
int Environment::SetEnv(const std::string &var, const std::string &val,
                        bool ow) {
...
void Environment::UnSetEnv(const std::string &var) {
  unsetenv(var.c_str());
  this->init();
}
...
const std::string Environment::GetEnv(const std::string &var) const {
  Environment::const_iterator ei = this->begin();
  while (ei != this->end()) {
    if ((*ei).first == var) return ((*ei).second);
    ei++;
  }
  return (std::string(""));
}

std::string &Environment::GetEnv(const std::string &var) {
...
int Environment::PutEnv(char *envs) {
  int retVal = putenv(envs);
  this->init();
  return (retVal);
}

void Environment::Refresh() { this->init(); }

char **Environment::GetRawEnv() { return (environ); }

std::ostream &operator<<(std::ostream &output, const Environment &env) {
  std::vector<std::pair<std::string, std::string> >::const_iterator ei =
      env.begin();
  while (ei != env.end()) {
