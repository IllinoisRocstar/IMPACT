...
void BuildBorderStiffness(Mesh::Connectivity &ec,std::vector<Mesh::Border> &borders,
			  ...
			  Mesh::Connectivity &RemoteNodalDofs,
			  Mesh::PartInfo &info,IRAD::Comm::CommunicatorObject &comm,
			  std::ostream *Out,bool verbose)
{
  unsigned int nborders = borders.size();
...
int RegisterCommBuffers(std::vector<Mesh::Border> &borders,IRAD::Comm::CommunicatorObject &comm)
{
  unsigned int nborders = borders.size();
  for(unsigned int i = 0;i < nborders;i++)
    {
...
//       }
  ...
  if(do_mesh_testing){
	...
	*StdOut << "Lists did " << (same_orientation ? "" : "not ")
    ...
    if(do_search){
      ...
      if(!IPFile){
	std::cerr << "Points file not found (" << PointsFileName << ") " << std::endl;
	exit(1);
      }
      int npoints;
      IPFile >> npoints;
      ...
      int pindex = 0;
      while(pindex < npoints)
	IPFile >> search_points[pindex++];
      IPFile.close();
	...
	uint cell_id = Mesh::FindPointInMesh(the_point,nc,econ,dc,max_box,natc);
	global.FunctionExit("FindPointInMesh");
	if(cell_id <= 0){
	  std::cout << "Cell not found for point " << (tpi - search_points.begin()) 
		    << ", (" << the_point.x() << "," << the_point.y() << ","
		    << the_point.z() << ")" << std::endl;
	  npoints_not_found++;
	}
      }
      std::cout << "Number of points not found: " << npoints_not_found << std::endl;
    }
	...
	if(*nei > 0){
	  ...
	  nestr << " " << soe << "-elements ";
	  *StdOut << "Number of" << ((soe == 4 || soe == 10) ? " tetrahedra " :
				     (soe == 5 || soe == 13) ? " pyramids " :
				     (soe == 6 || soe == 15) ? " prisms " :
				     (soe == 8 || soe == 20) ? " hexahedra " :
				     nestr.str().c_str()) << *nei << std::endl;
	}
	nei++;
      }
    ...
    Mesh::CreateAdjacentNodeList<Mesh::Connectivity,std::vector<Mesh::IndexType> >
  ...
  if(do_assembly){
	...
	*LogOut << std::endl;
    ...
    if(true){ // create an element queue to loop thru for numbering, and assembly
	...
	if(qi==queue.end()) { // strangely not done, get next unprocessed element
	  element_index = 0;
	  for(Mesh::IndexType i = 0;(i < number_of_elements && neighbors_processed[i]);i++)
	    element_index++;
	}
	...
	if(!neighbors_processed[element_index]){
	  ...
	  nprocessed_this_time++;
	  std::vector<Mesh::IndexType>::iterator ni = nl2[element_index].begin();
	...
	*LogOut << std::endl;
    ...
    if(LogOut && debug){
      *LogOut << "Total NodalDof storage(bytes): "
	      << (IRAD::Util::GetTotalSize(NodalDofs)+(number_of_nodes*2))*datasize
	      << std::endl
	      << "Total ElementDof storage(bytes): "
	      << (IRAD::Util::GetTotalSize(ElementDofs)+(number_of_elements*2))*datasize
	      << std::endl;
    }
    comm.Barrier();
	...
	*LogOut << std::endl;
    ...
    if(nglobdof != nglobal_dofs){
      if(ErrOut) *ErrOut << "Fatal Error: Final DOF count mismatch. ("
			 << nglobdof << "/" << nglobal_dofs << ")" << std::endl;
  ...
  if(do_partitioning){
	...
	*StdOut << "Partition " << iii+1 << ": "
    ...
    IRAD::Util::Flatten<Mesh::Connectivity,std::vector<Mesh::IndexType>,std::list<Mesh::IndexType> >
      (bordercon,bnodes);
    ...
    Mesh::IndexVec psize;
    IRAD::Primitive::MultiContainer<Mesh::IndexType,Mesh::IndexType>::VecMap pglob2loc;
...
void FiniteDifferenceGraph(Mesh::Connectivity graphcon,int nd,int *gridsize,int stencil_type, int stencil_size)
{
  Mesh::IndexType number_of_nodes = 1;
  for(int i = 0;i < nd;i++)
    number_of_nodes *= gridsize[i];
