...
///
class MeshTestComLine : public IRAD::Util::ComLineObject {
 public:
  MeshTestComLine(const char *args[]) : ComLineObject(args){};
  void Initialize() {
    AddOption('h', "help");
    ...
    AddOption('i', "skin");
    AddArgument("input", 1);
    AddHelp("skin", "Skins the mesh and writes the surface to the output.");
    AddHelp("network", "Sockets client/server test.");
    ...
    AddHelp("checking",
            "Paranoid and insanely verbose dumping of all important arrays to "
            "Log.");
    AddHelp(
        "partition",
    ...
    AddHelp("search", "Searches for specified points.");
...
void GetStatistics(std::ostream &Ostr, Mesh::Connectivity &con) {
  ...
  for (Mesh::IndexType iii = 0; iii < con.Nelem(); iii++) {
    ...
    if (max < con.Esize(iii + 1)) max = con.Esize(iii + 1);
    if (min > con.Esize(iii + 1)) min = con.Esize(iii + 1);
    for (Mesh::IndexType jjj = 0; jjj < con.Esize(iii + 1); jjj++) {
      if (minvalue > con[iii][jjj]) minvalue = con[iii][jjj];
      if (maxvalue < con[iii][jjj]) maxvalue = con[iii][jjj];
      meanvalue += con[iii][jjj];
      nvalues++;
    }
  }
  Ostr << "Esizes(min/max/mean): (" << min << "/" << max << "/"
       << mean / con.Nelem() << ")" << std::endl
       << "Flattened Size = " << nvalues << std::endl
       << "Values(min/max/mean): (" << minvalue << "/" << maxvalue << "/"
       << meanvalue / nvalues << ")" << std::endl;
}

// *** Warning ***
// This is called *after* the local dof id's have been mapped to the new dof
// id's and the info.doffset has been reset to an appropriate value.  The
// NodalDofs array should be the new global numbers with the doffset applied. We
// need local dof id's (i.e. 1 - ndof_local) in order to do the stiffness matrix
// properly.  So we need to determine the new "rank ordering" (i.e. determined
// the doffset).
void GlobalDofReMapExchange(
    Mesh::Connectivity &ec, Mesh::Connectivity &NodalDofs,
    Mesh::Connectivity &ElementDofs, Mesh::Connectivity &RemoteNodalDofs,
    std::vector<Mesh::Border> &borders, Mesh::PartInfo &info,
    std::vector<Mesh::IndexType> &order, IRAD::Comm::CommunicatorObject &comm,
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    if (Out && verbose)
      *Out << "Receive posted for " << RcvBuf[i].size() << " dofs from "
           << borders[i].rpart << std::endl;
  }
  comm.Barrier();
  if (Out && verbose)
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    if (Out && verbose) {
      *Out << "Posting send of " << SndBuf[i].size() << " nodal dofs to "
           << borders[i].rpart << ":" << std::endl;
      IRAD::Util::DumpContents(*Out, SndBuf[i], " ");
      *Out << std::endl;
    }
    //    borders[i].recvsize = SndBuf[i].size();
    comm.ASend<Mesh::IndexType>(SndBuf[i],
                                borders[i].rpart - 1);  // asynchronous send
  }
  comm.WaitAll();
  comm.ClearRequests();
  comm.Barrier();
  if (Out && verbose) *Out << "All procs send posted 2." << std::endl;
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    if (Out && verbose) {
      *Out << "Unpacking recv'd dofs:" << std::endl;
      IRAD::Util::DumpContents(*Out, RcvBuf[i], " ");
      *Out << std::endl;
    }
    while (sni != borders[i].nsend.end()) {
      Mesh::IndexType rnindex = *sni - info.nlocal - 1;
      std::vector<Mesh::IndexType>::iterator ndi =
          RemoteNodalDofs[rnindex].begin();
      while (ndi != RemoteNodalDofs[rnindex].end()) *ndi++ = *rdi++;
      sni++;
    }
  }
  comm.Barrier();
  //
  // All Nodal Dofs have been updated.  Now need to update the communication
  // buffers
  ...
  if (Out && verbose)
    *Out << "Forming dual connectivity for " << number_of_nodes << " nodes."
         << std::endl;
  ec.Inverse(dc, number_of_nodes);
  if (Out && verbose) *Out << "dual done." << std::endl;
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    while (sni != borders[i].nsend.end()) {
      ...
      while (
          ei !=
          dc[*sni - 1].end()) {  // for every element touching the remote node
        ...
        while (eni != ec[*ei - 1].end()) {  // loop thru the element's nodes
          ...
          if (*eni > info.nlocal) {  // then it's a remote node
            ...
            while (ndi != ndend) doflist.push_back(*ndi++);
          } else {
            //	    offset = info.doffset;
      ...
      while (dli != doflist.end()) borders[i].data.SendAi.push_back(*dli++);
      sni++;
    }
    if (Out && verbose) {
      *Out << "SendAp for color " << borders[i].rpart << ":" << std::endl;
      IRAD::Util::DumpContents(*Out, borders[i].data.SendAp, " ");
      *Out << std::endl
           << "SendAi for color " << borders[i].rpart << ":" << std::endl;
      IRAD::Util::DumpContents(*Out, borders[i].data.SendAi, " ");
      *Out << std::endl;
    }
  }
  comm.Barrier();
  if (Out && verbose) *Out << "All procs have remapped SendAi." << std::endl;
  // By now, the remote processor has sent us an array of sizes for the nz
  // entries relevant to each local *node*.  To get the total size of the
  // stiffness block that will be sent, we need to multiply by the number of
  // actual dofs living on the local node.  Right now, we are just interested in
  // the dof id's that we will be receiving/node.
  for (unsigned int i = 0; i < nborders; i++) {
    //    borders[i].data.RecvAp.resize(RcvBuf[i].size()+1);
    //    std::vector<Mesh::IndexType>::iterator rbi = RcvBuf[i].begin();
    //    std::vector<Mesh::IndexType>::iterator rai =
    //    borders[i].data.RecvAp.begin(); *rai = 0; while(rbi !=
    //    RcvBuf[i].end()){
    ...
    if (Out && verbose) {
      *Out << "RecvAP from partition " << borders[i].rpart << ":" << std::endl;
      IRAD::Util::DumpContents(*Out, borders[i].data.RecvAp, " ");
      *Out << std::endl;
    }
    // We can go ahead and post receives for the Ai
    std::vector<Mesh::IndexType>::reverse_iterator rsi =
        borders[i].data.RecvAp.rbegin();
    ...
    comm.ARecv<Mesh::IndexType>(borders[i].data.RecvAi,
                                borders[i].rpart - 1);  // async recv
    // Note how we've already conveniently filled the SendAi, send it out now
    comm.ASend<Mesh::IndexType>(borders[i].data.SendAi,
                                borders[i].rpart - 1);  // async recv
    if (Out && verbose) {
  ...
  if (Out && verbose) {
    for (unsigned int i = 0; i < nborders; i++) {
      *Out << "Recieved RecvAi (" << borders[i].data.RecvAi.size()
           << ") from partition" << borders[i].rpart << ":" << std::endl;
      IRAD::Util::DumpContents(*Out, borders[i].data.RecvAi, " ");
      *Out << std::endl;
    }
  }
}
...
void TestMPI(std::vector<Mesh::Border> &borders, Mesh::Connectivity &NodalDofs,
             ...
             IRAD::Comm::CommunicatorObject &comm, std::ostream *Out,
             bool verbose) {
  ...
  for (unsigned int ntrial = 0; ntrial < 100; ntrial++) {
    ...
    RcvBuf.resize(nborders);
    for (unsigned int i = 0; i < nborders; i++) {
      // For each remotely owned node, we will receive one integer indicating
      // the number of dofs on that remotely owned node
      RcvBuf[i].resize(borders[i].nsend.size());
      if (Out && verbose)
        *Out << "Posting receive for " << borders[i].nsend.size()
             << " nodes from " << borders[i].rpart << std::endl
             << std::flush;
      comm.ARecv<Mesh::IndexType>(RcvBuf[i],
                                  borders[i].rpart - 1);  // async recv
    }
    comm.Barrier();
    if (Out && verbose) *Out << "All procs receives posted." << std::endl;
    ...
    for (unsigned int i = 0; i < nborders; i++) {
      ...
      if (Out && verbose) {
        *Out << "Posting send of " << borders[i].nrecv.size()
             << " node's dof sizes to " << borders[i].rpart << std::endl;
        *Out << "Sending: ";
        IRAD::Util::DumpContents(*Out, SndBuf[i], " ");
...
void GlobalDofExchange(std::vector<Mesh::Border> &borders,
                       Mesh::Connectivity &NodalDofs,
                       Mesh::Connectivity &RemoteNodalDofs,
                       Mesh::PartInfo &info,
                       IRAD::Comm::CommunicatorObject &comm, std::ostream *Out,
                       bool verbose) {
  if (Out && verbose) *Out << "Inside Global Dof Exchange" << std::endl;
  // First just exchange the sizes (in number of dofs) for each node
  //
  // Build the receive buffers and post the receives
  Mesh::IndexType doffset = info.doffset;
  ...
  if (Out && verbose) *Out << "All procs sends posted." << std::endl;
  ...
  comm.Barrier();
  if (Out && verbose) *Out << "All procs node sizes received." << std::endl;
  ...
  comm.Barrier();
  if (Out && verbose) *Out << "All procs receive posted 2." << std::endl;
...
void BuildBorderStiffness(
    Mesh::Connectivity &ec, std::vector<Mesh::Border> &borders,
    Mesh::Connectivity &NodalDofs, Mesh::Connectivity &ElementDofs,
    Mesh::Connectivity &RemoteNodalDofs, Mesh::PartInfo &info,
    IRAD::Comm::CommunicatorObject &comm, std::ostream *Out, bool verbose) {
  unsigned int nborders = borders.size();
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    if (Out && verbose)
      *Out << "Posting receives for " << rsizes[i]
           << " sizes of stiffness entries "
           << "from " << borders[i].rpart << "." << std::endl;
    comm.ARecv<Mesh::IndexType>(RcvBuf[i], borders[i].rpart - 1);  // async recv
  }
  comm.Barrier();
  if (Out && verbose) *Out << "All procs have posted recvs." << std::endl;
    ...
    while (sni != borders[i].nsend.end()) {
      ...
      while (
        ...
        while (eni != ec[*ei - 1].end()) {  // loop thru the element's nodes
          ...
          while (ndi != ndend) {  // Loop thru the node's dofs and record them
                                  // into the doflist
      ...
      borders[i].data.SendAp.push_back(doflist.size() +
                                       *(borders[i].data.SendAp.rbegin()));
      std::list<Mesh::IndexType>::iterator dli = doflist.begin();
      while (dli != doflist.end()) borders[i].data.SendAi.push_back(*dli++);
      sni++;
    }
  ...
  comm.ClearRequests();
  comm.Barrier();
  if (Out && verbose) *Out << "All procs have posted sends." << std::endl;
  // By now, the remote processor has sent us an array of sizes for the nz
  // entries relevant to each local *node*.  To get the total size of the
  // stiffness block that will be sent, we need to multiply by the number of
  // actual dofs living on the local node.  Right now, we are just interested in
...
unsigned int AllocateCommBuffers(std::vector<Mesh::Border> &borders,
  ...
  for (unsigned int i = 0; i < nborders; i++) {
    ...
    if (Out && verbose)
      *Out << "Allocating buffer(" << nval_recv
           << ") for receiving stiffness blocks"
           << " from " << borders[i].rpart << "." << std::endl;
    ...
    if (Out && verbose)
      *Out << "Allocating buffer(" << nval_send
           << ") for sending stiffness blocks"
           << " to " << borders[i].rpart << "." << std::endl;
...
int RegisterCommBuffers(std::vector<Mesh::Border> &borders,
...
void CountDofs(Mesh::Connectivity &ElementDofs, Mesh::Connectivity &NodalDofs,
               Mesh::PartInfo &info, Mesh::IndexType &nglobaldof,
               Mesh::IndexType &nremotedof) {
...
int main(int argc, char *argv[]) {
  ...
  IRAD::Global::GlobalObj<std::string, Mesh::IndexType,
                          IRAD::Profiler::ProfilerObj>
      global;
  ...
  bool do_search = !comline.GetOption("search").empty();
  ...
  bool do_network_test = !comline.GetOption("network").empty();
  bool do_skin = !comline.GetOption("skin").empty();
  //  bool do_metis_test   = !comline.GetOption("metis").empty();
  bool do_renumbering = !comline.GetOption("renumber").empty();
  IRAD::Comm::DataTypes IndexDT =
      (sizeof(Mesh::IndexType) == sizeof(size_t) ? IRAD::Comm::DTSIZET
                                                 : IRAD::Comm::DTUINT);
  if (!comline.GetOption("help").empty()) {
    if (StdOut) *StdOut << comline.LongUsage() << std::endl;
    comm.SetExit(1);
  }
  if (comm.Check()) return (0);
  ...
  if (npartitions <= 0) npartitions = 2;
  if (nproc > 1) {
    ...
    LogOut = &LogFile;
    ErrOut = &LogFile;
    if (verblevel > 1 && rank > 0) StdOut = &LogFile;
  } else {
  ...
  if (debug) {
    global.SetDebugLevel(10);
  }
  if (debug && rank > 0) StdOut = LogOut;
  ...
  if (do_generate) {
    std::string MeshName;
    if (!infiles.empty()) MeshName = infiles[0];
    unsigned int N = 10;
    unsigned int D = 3;
    if (infiles.size() > 1) {
    ...
    if (infiles.size() > 2) {
      std::istringstream Instr(infiles[2]);
      Instr >> D;
    }
    if (N < 2) N = 2;
    if (D < 1) D = 3;
    Mesh::IndexType number_of_nodes = N;
    Mesh::IndexType number_of_elements = (N - 1);
    for (unsigned int nd = 1; nd < D; nd++) {
      number_of_nodes *= N;
      number_of_elements *= (N - 1);
    }
    ...
    if (LogOut && debug) *LogOut << "NC.size() = " << nc.Size() << std::endl;
    ...
    if (D == 3) {
      ...
      for (unsigned int j = 0; j < N; j++) {
        ...
        for (unsigned int i = 0; i < N; i++) {
          ...
          nc.z(node_id++) = z;
          if (j < (N - 1) && i < (N - 1)) {
            Mesh::IndexType ulnode = j * N + i % N + 1;
            ec[element_id - 1].push_back(ulnode);
            ec[element_id - 1].push_back(ulnode + N);
            ec[element_id - 1].push_back(ulnode + N + 1);
            ec[element_id - 1].push_back(ulnode + 1);
  ...
  if (do_network_test) {
    ...
    if (!infiles.empty()) mode = infiles[0];
    std::ifstream Inf;
    std::string MeshName;
    if (mode == "server") {
      if (infiles.size() >= 2) {
      ...
      std::ostringstream Ostr;
      Ostr << MeshName;
      if (nproc > 1) Ostr << "." << rank + 1 << ".pmesh";
      Inf.open(Ostr.str().c_str());
      if (!Inf) {
        if (ErrOut) *ErrOut << "Failed to open " << Ostr.str() << std::endl;
        comm.SetExit(1);
      }
      if (comm.Check()) return (1);
      ...
      if (StdOut && verblevel)
        *StdOut << "Number of elements: " << number_of_elements << std::endl;
      IRAD::Sys::Net::Server server;
      server.SimpleInit();
      ...
      bool do_input = false;
      bool coords_send = false;
      bool coords_recvd = false;
      bool elems_sent = false;
      bool elems_recvd = false;
      IRAD::Sys::fdistream clientInStr(client_descriptor);
      IRAD::Sys::fdostream clientOutStr(client_descriptor);
      global.FunctionEntry("Coord Send");
      clientOutStr << nc << std::endl;
      global.FunctionExit("Coord Send");
      global.FunctionEntry("Elem Send");
      clientOutStr << econ << std::endl;
      global.FunctionExit("Elem Send");
      std::string line;
      global.FunctionEntry("NetRead Line");
      std::getline(clientInStr, line);
      global.FunctionExit("NetRead Line");
      std::cout << "got " << line << " from client." << std::endl;
      std::ofstream OutF;
      OutF.open("testmesh_server.txt");
      OutF << nc << std::endl << econ << std::endl;
      OutF.close();
      //       while(!done){
      // 	int nio = setman.Select(4.0); // select timeout is 2 seconds
      // 	//      std::cout << nio << " descriptors are ready for io." <<
      // std::endl; 	if(setman.ReadyForInput(client_descriptor)){ 	  std::cout <<
      // "Client has input to process." << std::endl; 	  std::cout << "Receiving
      // message number " << ++nrecv
      ...
      // 	    done = true;
      // 	  else if(clientstring.find("coords") != std::string::npos)
      // 	    coords_recvd = true;
      // 	  else if(clientstring.find("elems") != std::string::npos)
      // 	    elems_recvd = true;
      // 	  if(!done && !coords_send){
      // 	    coords_send = true;
      // 	    std::cout << "Sending coords to client." << std::endl;
      // 	    global.FunctionEntry("Coordinate Send");
      // 	    clientOutStr << nc << std::endl;
      // 	    global.FunctionExit("Coordinate Send");
      // 	  }
      // 	  else if(!done && !elems_sent && coords_recvd){
      // 	    elems_sent = true;
      // 	    std::cout << "Sending elems to client." << std::endl;
      // 	    global.FunctionEntry("Elem Send");
      // 	    clientOutStr << econ << std::endl;
      // 	    global.FunctionExit("Elem Send");
      // 	  }
      // 	  else
      // 	    clientOutStr << "Nothing to send." << std::endl;
      // 	}
      //       }

    } else {
      std::string host(mode);
      IRAD::Sys::Net::Client client;
      ...
      IRAD::Sys::fdostream fdOut(client.Descriptor());
      IRAD::Sys::fdistream fdIn(client.Descriptor());
      //      fdOut << "I am ready for the mesh please." << std::endl;
      //      sleep(10);
      int nsend = 0;
      int nrecv = 0;
      int nlines = 0;
      ...
      global.FunctionEntry("NetRead Coords");
      fdIn >> nc;
      global.FunctionExit("NetRead Coords");
      global.FunctionEntry("NetRead Elems");
      fdIn >> econ;
      global.FunctionExit("NetRead Elems");
      fdOut << "All done receiving" << std::endl;
      //       while(!done){
      // 	setman.Select(2.0);
      // 	bool recv_time = (setman.ReadyForInput(client.Descriptor()) >
      // 0); 	bool send_time = (setman.ReadyForOutput(client.Descriptor()) > 0);
      // 	if(recv_time){
      // 	  std::cout << "Ready to receive message " << ++nrecv
      // 		    << " from server." << std::endl;
      // 	  if(!coords_read){
      // 	    coords_read = true;
      // 	    global.FunctionEntry("Network Read");
      // 	    fdIn >> nc;
      // 	    global.FunctionExit("Network Read");
      std::cout << "Received these coords: " << std::endl;
      ...
      // 	    fdOut << "coords" << std::endl;
      // 	    //	    sleep(10);
      // 	  }
      // 	  else if(coords_read && !elems_read){
      // 	    elems_read = true;
      // 	    global.FunctionEntry("Elem Read");
      // 	    fdIn >> econ;
      // 	    global.FunctionExit("Elem Read");
      Mesh::IndexType number_of_elements = econ.Nelem();
      if (StdOut && verblevel)
        *StdOut << "Number of elements: " << number_of_elements << std::endl;
      std::ofstream OutF;
      OutF.open("testmesh_client.txt");
      OutF << nc << std::endl << econ << std::endl;
      OutF.close();
      // 	    fdOut << "elems" << std::endl;
      // 	    //	    sleep(10);
      // 	  }
      // 	  else{
      // 	    std::string line;
      ...
      // 	    if(!line.empty())
      // 	      std::cout << "line(" << ++nlines << ") from server: " <<
      // line << std::endl; 	    else 	      std::cout << "No line received." << std::endl;
      // 	  }
      // 	  if(!done_sending){
      // 	    std::cout << "Sending message " << ++nsend << " to server."
      // << std::endl; 	    fdOut << "I am the client, and I am sending message
      // number " << nsend
      // 		  << std::endl;
      // 	    if(nsend == 2)
      // 	      done_sending = true;
  ...
  if (do_mesh_testing) {
    ...
    if (do_search == true) {
      if (infiles.size() < 2) {
        std::cerr << "No points specified." << std::endl;
        exit(1);
      }
      PointsFileName = infiles[1];
    }
    std::ostringstream Ostr;
    Ostr << MeshName;
    if (nproc > 1) Ostr << "." << rank + 1 << ".pmesh";
    ...
    if (false) {
      ...
      Mesh::IndexType el = 1;
      while (ci != econ.end()) {
        if (StdOut && verblevel) *StdOut << "Element " << el++ << ": (";
    ...
    if (do_neighborhood) {
      ...
      if (false) {
        ...
        //     while(ei != nl.end()){
        //       if(StdOut && verblevel) *StdOut << "Element " << el++ << "
        //       neighbors: "; std::set<Mesh::IndexType>::iterator ni =
        //       ei->begin(); while(ni != ei->end()){
        // 	if(StdOut && verblevel) *StdOut << *ni++;
        // 	if(ni != ei->end())
        // 	  if(StdOut && verblevel) *StdOut << ",";
        //       }
        ...
        //       if(StdOut && verblevel) *StdOut << "Element " << el++ << "
        //       neighbors: "; std::list<Mesh::IndexType>::iterator ni =
        //       eni->begin(); while(ni != eni->end()){
        // 	if(StdOut && verblevel) *StdOut << *ni++;
        // 	if(ni != eni->end())
        // 	  if(StdOut && verblevel) *StdOut << ",";
        //       }
    ...
    if (false) {
      ...
      bool same_orientation = IRAD::Util::HaveSameOrientation(a, b);
      global.FunctionExit("Same Orientation");
      global.FunctionEntry("Opp Orientation");
      bool opp_orientation = IRAD::Util::HaveOppositeOrientation(a, b);
    ...
    Mesh::IndexType badshapecount = 0;
    std::vector<Mesh::IndexType> nelements(21, 0);
    for (Mesh::IndexType element_being_processed = 1;
      ...
      nelements[size_of_element]++;
      Mesh::GenericElement ge(size_of_element);
      if (ge.Inverted(econ[index], nc)) {
        invertedcount++;
        ge.ReOrient(econ[index]);
      }
      global.FunctionEntry("ProfOverHead");
      global.FunctionExit("ProfOverHead");
      global.FunctionEntry("ShapeOK");
      if (!ge.ShapeOK(econ[index], nc)) badshapecount++;
      global.FunctionExit("ShapeOK");
    }
    global.FunctionExit("Orient elements");
    std::vector<GeoPrim::CPoint> search_points;
    if (do_search) {
      std::ifstream IPFile;
      IPFile.open(PointsFileName.c_str());
      if (!IPFile) {
        std::cerr << "Points file not found (" << PointsFileName << ") "
                  << std::endl;
        exit(1);
      }
      int npoints;
      IPFile >> npoints;
      search_points.resize(npoints);
      int pindex = 0;
      while (pindex < npoints) IPFile >> search_points[pindex++];
      IPFile.close();
      std::vector<GeoPrim::CPoint>::iterator tpi = search_points.begin();
      GeoPrim::CVector natc;
      GeoPrim::CBox min_box;
      GeoPrim::CBox max_box;
      GeoPrim::CBox full_box;
      global.FunctionEntry("GetMeshBoxes");
      Mesh::GetMeshBoxes(nc, econ, full_box, min_box, max_box);
      global.FunctionExit("GetMeshBoxes");
      uint npoints_not_found = 0;
      while (tpi != search_points.end()) {
        GeoPrim::CPoint the_point(*tpi++);
        global.FunctionEntry("FindPointInMesh");
        uint cell_id =
            Mesh::FindPointInMesh(the_point, nc, econ, dc, max_box, natc);
        global.FunctionExit("FindPointInMesh");
        if (cell_id <= 0) {
          std::cout << "Cell not found for point "
                    << (tpi - search_points.begin()) << ", (" << the_point.x()
                    << "," << the_point.y() << "," << the_point.z() << ")"
                    << std::endl;
          npoints_not_found++;
        }
      }
      std::cout << "Number of points not found: " << npoints_not_found
                << std::endl;
    }
    // Don't destroy the coordinates yet.
    //    nc.destroy();
    if (StdOut && verblevel) {
      std::vector<Mesh::IndexType>::iterator nei = nelements.begin();
      while (nei != nelements.end()) {
        if (*nei > 0) {
          Mesh::IndexType soe = (nei - nelements.begin());
          std::ostringstream nestr;
          nestr << " " << soe << "-elements ";
          *StdOut << "Number of"
                  << ((soe == 4 || soe == 10)
                          ? " tetrahedra "
      ...
      *StdOut << "Number of elements reoriented: " << invertedcount
              << std::endl;
      *StdOut << "Number of elements with bad shape: " << badshapecount
              << std::endl;
    }

    global.FunctionEntry("All Face Processing");
    Mesh::IndexType nface_estimate =
        static_cast<Mesh::IndexType>(2.2 * number_of_elements);
    ...
    if (StdOut && verblevel)
      *StdOut << "Estimated number of faces: " << nface_estimate << std::endl
              ...
              << canned_bndry_quad << "," << canned_bndry_tri << ")"
              << std::endl;
    global.FunctionExit("All Face Processing");
    if (do_skin) {
      Mesh::Connectivity BoundaryConn;
      fei = F_E.begin();
      while (fei != F_E.end()) {
        Mesh::IndexType faceid = (fei++ - F_E.begin()) + 1;
        if (F_E.Esize(faceid) == 1)
          BoundaryConn.AddElement(F_N.Element(faceid));
      }
    }
    ...
    Mesh::CreateAdjacentNodeList<Mesh::Connectivity,
                                 std::vector<Mesh::IndexType> >(
        anodelist, F_N, number_of_nodes);
    //    Mesh::CreateAdjacentNodeList<std::vector<std::vector<Mesh::IndexType>
    //    >,
    //      std::vector<Mesh::IndexType> >
    //      (anodelist,F_N,number_of_nodes);
    ...
    if (false) {  // report the edges
      std::vector<std::list<Mesh::IndexType> >::iterator anli =
          anodelist.begin();
      Mesh::IndexType ii = 0;
      while (anli != anodelist.end()) {
        ...
        while (ni != anli->end()) {
          if (StdOut && array_checking) *StdOut << *ni++;
          if (ni != anli->end())
            if (StdOut && array_checking) *StdOut << ",";
        }
        if (StdOut && array_checking) *StdOut << std::endl;
        anli++;
      }
    }
  }
  ...
  if (do_assembly) {
    ...
    std::ifstream Inf;
    if (LogOut && debug) *LogOut << "DataSize = " << datasize << std::endl;
    if (true) {
      global.FunctionEntry("ReadCoord");
      std::string MeshName;
      if (!infiles.empty()) MeshName = infiles[0];
      ...
      if (nproc > 1) {
        ...
        if (!Inf) {
          if (ErrOut) *ErrOut << "Failed to open " << FNOstr.str() << std::endl;
          comm.SetExit(1);
        }
        if (comm.Check()) return (1);
        Inf >> info.npart >> info.part >> info.nelem >> info.nnodes >>
            info.nborder >> info.nshared >> info.nown;
    ...
    if (LogOut && verblevel > 1) {
      Mesh::IndexType consize = IRAD::Util::GetTotalSize(econ);
      *LogOut << "Connectivity size(bytes): "
              << (consize + (2 * number_of_elements)) * datasize << std::endl;
    }
    if (nproc == 1) {
    ...
    if (nproc > 1) {
      Inf >> info.nborder;
      if (StdOut && verblevel)
        *StdOut << "Number of neighboring partitions: " << info.nborder
                << std::endl;
      ...
      if (LogOut && verblevel > 1) {
        *LogOut << "Border description storage size(bytes): "
                << (totalrecv + totalsend) * datasize << std::endl;
      }
    }
    Inf.close();
    ...
    if (LogOut && debug) {
      *LogOut << "Dual Con size(bytes):"
              << (IRAD::Util::GetTotalSize(dc) + 2 * dc.size()) * datasize
              << std::endl;
      if (array_checking)
        *LogOut << "dual connectivity:" << std::endl << dc << std::endl;
    }
    // Neighborhood populates an array of neighbors
    // for each element.  A neighbor is any element
    // that shares a node with a given element.
    ...
    if (LogOut && array_checking)
      *LogOut << "Element Neighborhood:" << std::endl << nl2 << std::endl;
    comm.Barrier();
    if (LogOut && debug)
      *LogOut << "Neighborhood size(bytes): "
              << (IRAD::Util::GetTotalSize(nl2) + (2 * nl2.size())) * datasize
              << std::endl;
    Mesh::Connectivity NodalDofs;
    ...
    //  Mesh::IndexType ndoftotal =
    //  Mesh::AssignUniformDofs(econ,dc,ElementFields,
    //						NodalFields,NodalDofs,
    //						ElementDofs,fields);
    //  global.FunctionExit("Uniform DOF Assignment");
    comm.Barrier();
    if (LogOut && verblevel > 1)
      *LogOut << "All processors finished prepping." << std::endl
    ...
    if (LogOut && verblevel) {
      *LogOut << "Found " << border_elements.size() << " border elements."
              << std::endl;
      if (array_checking) {
        IRAD::Util::DumpContents(*LogOut, border_elements, " ");
        *LogOut << std::endl;
      }
    }
    std::list<Mesh::IndexType> queue;
    if (true) {  // create an element queue to loop thru for numbering, and
                 // assembly
      ...
      queue.resize(0);
      queue.push_back(1);
      element_processed[0] = true;
      nprocessed = 1;
      std::list<Mesh::IndexType>::iterator qi = queue.begin();
      global.FunctionEntry("Element order queue");
      Mesh::IndexType islands = 1;
      while ((nprocessed < number_of_elements)) {
        Mesh::IndexType nprocessed_this_time = 0;
        Mesh::IndexType element_index = 0;
        bool done = false;
        if (qi == queue.end()) qi = queue.begin();
        while ((qi != queue.end()) && !done) {
          element_index = *qi - 1;
          if (!neighbors_processed[element_index])
            done = true;
          else
            qi++;
        }
        if (qi ==
            queue.end()) {  // strangely not done, get next unprocessed element
          element_index = 0;
          for (Mesh::IndexType i = 0;
               (i < number_of_elements && neighbors_processed[i]); i++)
            element_index++;
        }
        if (!element_processed[element_index]) {
          element_processed[element_index] = true;
          queue.push_back(element_index + 1);
          nprocessed_this_time++;
          nprocessed++;
        }
        if (!neighbors_processed[element_index]) {
          neighbors_processed[element_index] = true;
          nprocessed_this_time++;
          std::vector<Mesh::IndexType>::iterator ni =
              nl2[element_index].begin();
          while (ni != nl2[element_index].end()) {
    ...
    if (queue.empty()) {
      Mesh::IndexType i = 1;
      while (i <= number_of_elements) queue.push_back(i++);
    } else
      assert(queue.size() == number_of_elements);
      ...
      if (nproc == 1 && false) {
        ...
        while (ei != queue.end()) {
          if (!is_border_element[*ei - 1]) {
            if (array_checking)
              *LogOut << "Assigning dofs for nonborder element " << *ei
                      << std::endl;
            std::vector<Mesh::IndexType>::iterator ni = econ[*ei - 1].begin();
            while (ni != econ[*ei - 1].end()) {
              if (!node_processed[*ni - 1]) {
                ...
                for (unsigned int i = 0; i < N; i++) {
                  if (array_checking)
                    *LogOut << "node " << *ni << " getting dof " << nvoldof
                            << std::endl;
                  NodalDofs[*ni - 1][i] = nvoldof++;
                }
              }
            ...
            for (unsigned int i = 0; i < N2; i++) {
              if (array_checking)
                *LogOut << "element " << *ei << " getting dof " << nvoldof
                        << std::endl;
              ElementDofs[*ei - 1][i] = nvoldof++;
            }
          }
        ...
        while (ei != border_elements.end()) {
          ...
          for (unsigned int i = 0; i < N2; i++) {
            if (array_checking)
              *LogOut << "border element " << *ei << " getting dof " << nvoldof
                      << std::endl;
            ElementDofs[*ei - 1][i] = nvoldof++;
          }
          ei++;
        ...
        while (ei != border_elements.end()) {
          ...
          while (ni != econ[*ei - 1].end()) {
            if (!node_processed[*ni - 1]) {
              ...
              for (unsigned int i = 0; i < N; i++) {
                if (array_checking)
                  *LogOut << "remote border node " << *ni
                          << " getting local dof " << nvoldof + nborderdof
                          << std::endl;
                NodalDofs[*ni - 1][i] = (nvoldof + nborderdof++);
              }
            }
        ...
        if (StdOut && verblevel)
          *StdOut
              << "NDof TOTAL (local + adjacent remote): " << ndoftot
              << std::endl
              ...
              << nborderdof << std::endl
              << "NDof Shared (dofs on the partition boundary that I own) "
                 "DOFS: "
              << nshareddof << std::endl;
        if (LogOut && debug) {
          ...
          *LogOut << "after sort, size = " << border_elements.size()
                  << std::endl;
        }
      }
    }
    ...
    if (StdOut && verblevel)
      *StdOut << "Number of local DOFs: " << nlocal_dofs << std::endl
              << "Number of Elements on partition boundaries: "
              << border_elements.size() << std::endl;
    if (LogOut && debug) {
      *LogOut << "Total NodalDof storage(bytes): "
              << (IRAD::Util::GetTotalSize(NodalDofs) + (number_of_nodes * 2)) *
                     datasize
              << std::endl
              << "Total ElementDof storage(bytes): "
              << (IRAD::Util::GetTotalSize(ElementDofs) +
    ...
    comm.Barrier();
    if (LogOut && debug)
      *LogOut << "All processors entering global dof counting." << std::endl;
    ...
    comm.Barrier();
    if (LogOut && debug)
      *LogOut << "All processors done counting local dofs." << std::endl
      ...
      *LogOut << "Global Dofcount verification = " << nglobal_dofs << std::endl
      ...
      if (array_checking) {
        *LogOut << "NGDofs:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, ngdofs_p);
        *LogOut << std::endl;
      }
    }
    ...
    for (unsigned int nn1 = 0; nn1 < rank; nn1++) info.doffset += ngdofs_p[nn1];
    for (unsigned int nn2 = 0; nn2 < nproc; nn2++)
      ndof_global_total += ngdofs_p[nn2];
    if (StdOut && verblevel)
      *StdOut << "Number of global dofs on this proc: " << nglobal_dofs
              << std::endl
              << "Number of remote dofs on this proc: " << nremote_dofs
              << std::endl
              << "Number of global dofs over all procs: " << ndof_global_total
              << std::endl;
    if (StdOut && verblevel)
    ...
    comm.Barrier();
    if (LogOut && debug)
      *LogOut << "All procs entering global dof exchange." << std::endl;
    ...
    GlobalDofExchange(borders, NodalDofs, RemoteNodalDofs, info, comm, LogOut,
                      array_checking);
    ...
    if (StdOut && verblevel)
      *StdOut << "After global exchange and recount: " << std::endl
              ...
              << "Number of remote dofs on this proc: " << nremotedof2
              << std::endl;

    if (nglobdof != nglobal_dofs) {
      if (ErrOut)
        *ErrOut << "Fatal Error: Final DOF count mismatch. (" << nglobdof << "/"
                << nglobal_dofs << ")" << std::endl;
      assert(nglobdof == nglobal_dofs);
    }
    //    nremote_dofs = nremotedof;
    comm.Barrier();
    // Now we need to build the send and receive stiffnesses
    // The new way is to assume that the ElementDofs actually has ONLY elemental
    // dofs, not with the nodal dofs stacked on as previously. (it only hurts
    // performance at this point)
    global.FunctionEntry("BuildBorderStiff");
    BuildBorderStiffness(econ, borders, NodalDofs, ElementDofs, RemoteNodalDofs,
                         info, comm, LogOut, array_checking);
    global.FunctionExit("BuildBorderStiff");
    // Allocate the communication buffers
    unsigned int nalloc =
        AllocateCommBuffers(borders, NodalDofs, LogOut, debug);
    if (LogOut && debug)
      *LogOut << "Allocated " << nalloc * sizeof(double)
              << " bytes of communication buffers." << std::endl;
    comm.Barrier();
    if (LogOut && verblevel)
    ...
    for (Mesh::IndexType en = 0; en < number_of_elements; en++) {
      ...
      while (eni != econ[en].end()) {  // for every node in this element
        ...
        std::vector<Mesh::IndexType>::iterator ndi =
            NodalDofs[node_id - 1].begin();
        while (ndi != NodalDofs[node_id - 1].end())
          E_LD[en].push_back(
              *ndi++);  // add it's dofs to the list of dofs for this element
      }
    ...
    //
    // Now E_LD is a list of all dofs for every element.  It is an important
    // point that LD are the LD (i.e. the local dofs), because they run from
    // 1-nlocal_dofs. Since both the indices of E (1-number_of_elements) and
    // dofs both are sequential lists, we can do some nifty array manipulations
    // to obtain the information we need. First, we invert E[LD] to obtain
    // LD[E]:
    ...
    //
    // Now, there is one slight problem with our version of LD[E], in that it
    // includes LD which are actually just local numbers for remote dofs.  We do
    // not need the stiffness rows for these.  In fact, we already have created
    // the stiffness blocks for these rows when we created the communication
    // buffers.  We know those dofs are all dofs with id's > nglobdof.  If we
    // don't take care of the problem now, we pay a hefty memory and processing
    // fee in the following steps.
    LD_E.Sync();
    assert(LD_E.Nelem() == ndoftot);
    for (Mesh::IndexType di = nglobdof; di < ndoftot; di++) LD_E[di].resize(0);
    LD_E.ShrinkWrap();
    LD_E.SyncSizes();
    ...
    if (LogOut && debug && array_checking)
      *LogOut << "All Local K:" << std::endl << symbstiff << std::endl;
    ...
    if (true) {
      ...
      for (Mesh::IndexType ni = info.nlocal; ni < number_of_nodes; ni++) {
        ...
        std::vector<Mesh::IndexType>::iterator rdi =
            RemoteNodalDofs[ni - info.nlocal].begin();
        assert(NodalDofs[ni].size() ==
               RemoteNodalDofs[ni - info.nlocal].size());
    ...
    // *Note:  This part takes the list of remote dofs (some of which are not
    // even on
    //         the border) which talk to our local dofs and updates the
    //         appropriate local rows to contain the proper global ids of the
    //         remote dofs. These values were not present before this section of
    //         code. This is *not* a mapping operation.
    //
    for (unsigned int nn = 0; nn < info.nborder; nn++) {
      std::vector<Mesh::IndexType>::iterator bni = borders[nn].nrecv.begin();
      std::vector<Mesh::IndexType>::iterator Api =
          borders[nn].data.RecvAp.begin();
      // (1) For every node that I own on this border...
      while (bni != borders[nn].nrecv.end()) {
        Mesh::IndexType begindex = *Api++;
        std::vector<Mesh::IndexType>::iterator ndi =
            NodalDofs[*bni - 1].begin();
        ...
        while (ndi != NodalDofs[*bni - 1].end()) {
          ...
          for (unsigned int i = 0; i < nvals; i++)
            symbstiff[*ndi - 1].push_back(
                borders[nn].data.RecvAi[begindex + i]);
          ndi++;
        }
        bni++;
      }
    ...
    if (LogOut && debug && array_checking)
      *LogOut << "K after update:" << std::endl << symbstiff << std::endl;
    ...
    while (ssIt != symbstiff.end()) {
      ...
      std::vector<Mesh::IndexType> tmp(ssIt->begin(),
                                       std::unique(ssIt->begin(), ssIt->end()));
      ssIt->swap(tmp);
      ssIt++;
    }
    global.FunctionExit("K sort");
    if (LogOut && debug && array_checking)
      *LogOut << "Sorted K:" << std::endl << symbstiff << std::endl;
    ...
    Mesh::IndexType nnz = IRAD::Util::GetTotalSize(symbstiff);

    k._ndof = nglobdof;
    std::vector<Mesh::IndexType> ApLoc(1, k._sizes[nglobdof]);
    std::vector<Mesh::IndexType> ApTot(1, 0);

    comm.Reduce(ApLoc, ApTot, IndexDT, IRAD::Comm::SUMOP, 0);
    if (LogOut && verblevel) {
      ...
      si++;
      while (si != k._sizes.end()) myav += (*si++ - *si2++);
    ...
    if (do_dump_sparse) {
      ...
      Ostr << infiles[0] << "_sparse_" << ZeroStr.str() << rank + 1;
      std::ofstream Ouf;
    ...
    if (do_renumbering) {
      ...
      *vtdi = 0;
      ...
      if (array_checking && LogOut) {
        *LogOut << "Graph K:" << std::endl << symbstiff << std::endl;
      }
      // this function does the 1-based to 0-based mapping
      global.FunctionEntry("Container2CSR");
      IRAD::Util::MultiContainer2CSR<std::vector<std::vector<Mesh::IndexType> >,
                                     std::vector<Mesh::IndexType>,
                                     std::vector<idxtype>, idxtype>(xadj, adj,
                                                                    symbstiff);
      symbstiff.matrix_mode(info.doffset);
      global.FunctionExit("Container2CSR");
      if (array_checking && LogOut) {
        *LogOut << "Matrix K:" << std::endl << symbstiff << std::endl;
      }
      ...
      if (array_checking) {
        *LogOut << "Vtxdist:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, vtxdist, " ");
        *LogOut << std::endl << "Xadj:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, xadj, " ");
        *LogOut << std::endl << "Adj:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, adj, " ");
      ...
      ParMETIS_V3_NodeND(&vtxdist[0], &xadj[0], &adj[0], &numflag, options,
                         &order[0], &sizes[0], &communicator);
#endif
      global.FunctionExit("Metis-NodeID");
      if (array_checking) {
        *LogOut << "Order:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, order, " ");
        *LogOut << std::endl << "Sizes:" << std::endl;
        IRAD::Util::DumpContents(*LogOut, sizes, " ");
        *LogOut << std::endl;
      }
      ...
      *LogOut << "Reordered Min: " << *sorder.begin() << std::endl
      ...
      if (debug) {
        *LogOut << "Maximum contiguous block(" << max_contig << "): ("
                << contig_min << "," << contig_max << ")" << std::endl;
      }
      int binmin = 0;
      int maxbin = 0;
      int solve_rank = 0;
      for (unsigned int iii = 0; iii < nproc; iii++) {
        if (debug)
          *LogOut << "Bin(" << binmin << "," << binmin + binsize
                  << "): " << binrank[iii].nbin << std::endl;
      ...
      *LogOut << "My Preferred Solve Rank = " << solve_rank << std::endl;
      MPI_Allreduce(binrank, binrank_p, nproc, MPI_2INT, MPI_MAXLOC,
                    MPI_COMM_WORLD);
      ...
      IRAD::Comm::CommunicatorObject solvercomm;
      commstat = comm.Split(0, solve_ranks[rank], solvercomm);
      //      int mpistat =
      //      MPI_Comm_split(MPI_COMM_WORLD,0,binrank_p[rank].rank,&solvecomm);
      if (commstat) {
      ...
      *LogOut << "My Solve Rank = " << solve_rank << std::endl;
      ...
      if (!rank_conflict.empty()) {
        ...
        *LogOut << "Conflicting for solve_rank(" << solve_rank << "): ";
        IRAD::Util::DumpContents(*LogOut, rank_conflict, " ");
        *LogOut << std::endl;
      }
      if (ncovered != nproc) {
        comm.SetExit(1);
      ...
      solvercomm.Barrier();
      int solve_doffset = 0;
      for (int iii = 0; iii < solve_rank; iii++) solve_doffset += nrows_p[iii];
      ...
      for (unsigned int iii = 0; iii < nglobdof; iii++) {
        ...
        while (findrow < row_index) findrow += nrows_p[++solve_procid];
        RemoteRows[solve_procid].push_back(row_index);
      }
      if (array_checking)
        *LogOut << "RemoteRows:" << std::endl << RemoteRows << std::endl;
      if (debug) {
      ...
      for (int iii = 0; iii < (int)nproc; iii++)
        if (iii != solve_rank) nremote_rows += RemoteRows[iii].size();
      int nrecvd_rows = mynrows - nlocal_rows;
      if (debug)
        *LogOut << "Number of Remote Rows I assemble (Sent Rows): "
                << nremote_rows << std::endl
                << "Number of my Rows assembled remotely (Recvd Rows): "
                << nrecvd_rows << std::endl;
      // Now need to communicate and discover the mapping of old remote dof
      // numbers to new.   There are two sets of dofs that we need:
      ...
      global.FunctionEntry("DofRemapXange");
      GlobalDofReMapExchange(econ, NodalDofs, ElementDofs, RemoteNodalDofs,
                             borders, info, Order, comm, LogOut,
                             array_checking);
      global.FunctionExit("DofRemapXange");
      ...
      if (true) {
        ...
        for (Mesh::IndexType en = 0; en < number_of_elements; en++) {
          ...
          while (eni != econ[en].end()) {  // for every node in this element
            ...
            std::vector<Mesh::IndexType>::iterator ndi =
                NodalDofs[node_id - 1].begin();
            while (ndi != NodalDofs[node_id - 1].end())
              E_LD[en].push_back(*ndi++);  // add it's dofs to the list of dofs
                                           // for this element
          }
          std::vector<Mesh::IndexType>::iterator edi = ElementDofs[en].begin();
          while (edi != ElementDofs[en].end())
        ...
        // point that LD are the LD (i.e. the local dofs), because they run from
        // 1-nlocal_dofs. Since both the indices of E (1-number_of_elements) and
        // dofs both are sequential lists, we can do some nifty array
        // manipulations to obtain the information we need. First, we invert
        // E[LD] to obtain LD[E]:
        Mesh::Connectivity LD_E;
        global.FunctionEntry("Invert E[LD]");
        E_LD.Inverse(LD_E, ndoftot);
        global.FunctionExit("Invert E[LD]");
        //
        // Now, there is one slight problem with our version of LD[E], in that
        // it includes LD which are actually just local numbers for remote dofs.
        // We do not need the stiffness rows for these.  In fact, we already
        // have created the stiffness blocks for these rows when we created the
        // communication buffers.  We know those dofs are all dofs with id's >
        // nglobdof.  If we don't take care of the problem now, we pay a hefty
        // memory and processing fee in the following steps.
        LD_E.Sync();
        assert(LD_E.Nelem() == ndoftot);
        ...
        //
        // Now update with the remote dof info.
        // *Note:  This part takes the list of remote dofs (some of which are
        // not even on
        //         the border) which talk to our local dofs and updates the
        //         appropriate local rows to contain the proper global ids of
        //         the remote dofs. These values were not present before this
        //         section of code. This is *not* a mapping operation.
        //
        for (unsigned int nn = 0; nn < info.nborder; nn++) {
          std::vector<Mesh::IndexType>::iterator bni =
              borders[nn].nrecv.begin();
          std::vector<Mesh::IndexType>::iterator Api =
              borders[nn].data.RecvAp.begin();
          // (1) For every node that I own on this border...
          while (bni != borders[nn].nrecv.end()) {
        ...
        while (ssIt != symbstiff.end()) {
          std::sort(ssIt->begin(), ssIt->end());
          std::vector<Mesh::IndexType> tmp(
              ssIt->begin(), std::unique(ssIt->begin(), ssIt->end()));
          ssIt->swap(tmp);
          ssIt++;
        }
        global.FunctionExit("K sort");
        ...
        symbstiff.Truncate();
        symbstiff.ShrinkWrap();
        //	FEM::DummyStiffness<double,Mesh::IndexType,Mesh::Connectivity,std::vector<Mesh::IndexType>
        //> k;
        k._dofs = &symbstiff;
        k.rank = rank;
        // Commented out for symbolic assembly
      ...
      if (do_dump_sparse) {
        ...
        Ostr << infiles[0] << "_sparse_rnm_" << ZeroStr.str() << rank + 1;
        std::ofstream Ouf;
        Ouf.open(Ostr.str().c_str());
        Ouf << ndof_global_total << std::endl;
        Ouf << k._ndof << std::endl;
        IRAD::Util::DumpContents(Ouf, Order, " ");
        Ouf << std::endl;
        k.Dump(Ouf);
        Ouf.close();
      }
      ...
      for (unsigned int iii = 0; iii < info.nlocal; iii++) {
        ...
        while (ndi != NodalDofs[iii].end() && !node_processed) {
          if (Order[*ndi] <= (unsigned int)my_first_row ||
            ...
            while (bei != dc[iii].end()) is_border_element[*bei++] = true;
            node_processed = true;
          }
          ndi++;
        }
    ...
    std::vector<std::pair<unsigned int, unsigned int> > boundary_node_map(
        info.nnodes - info.nlocal);
    for (unsigned int i = 0; i < info.nborder; i++) {
      std::vector<Mesh::IndexType>::iterator sni = borders[i].nsend.begin();
      unsigned int index = 0;
      while (sni != borders[i].nsend.end()) {
        boundary_node_map[*sni - info.nlocal - 1] =
            std::make_pair(i + 1, index + 1);
        sni++;
        index++;
      }
    }
    ...
    if (true) {
      ...
      std::list<Mesh::IndexType>::iterator ei = queue.begin();
      while (ei != queue.end()) {
        if (!element_processed[*ei - 1]) nonborder_queue.push_back(*ei);
        ei++;
      }
      if (LogOut && verblevel) {
        *LogOut << "Found " << border_elements.size() << " border elements."
        ...
        if (array_checking) {
          ...
          *LogOut << std::endl << "NonBorder Elements:" << std::endl;
          IRAD::Util::DumpContents(*LogOut, nonborder_queue, " ");
          *LogOut << std::endl;
        }
      }
      global.FunctionExit("Find Border Elements");
    ...
    if (true) {
      ...
      while (bei != border_elements.end()) {
        ...
        if (!((eindex + 1) > 0 && (eindex + 1) <= ElementDofs.Nelem())) {
          *ErrOut << "ERROR: ElementDofs[" << eindex
                  << "].size() = " << ElementDofs[eindex].size()
                  << " and ElementDofs.size() = " << ElementDofs.Nelem()
                  << std::endl;
          return (1);
        }
        Mesh::IndexType nedof = ElementDofs.Esize(eindex + 1);
        Mesh::IndexType search = FEM::AssembleBorderElementII(
            econ[eindex], ElementDofs[eindex], endof, nedof, NodalDofs,
            RemoteNodalDofs, k, borders, boundary_node_map, dofdat, info,
            LogOut, debug);
        bordersearch += search;
      }
    ...
    if (false) {
      ...
      while (ei != nonborder_queue.end()) {
        ...
        search = FEM::AssembleLocalElementII(econ[nnn], ElementDofs[nnn], endof,
                                             nedof, NodalDofs, k, dofdat, info);
        nsearches += search;
        //	}
      }
    ...
    if (true) {
      ...
      fast_searches =
          FEM::FastAssembleLocalElements(nonborder_queue, econ, k, NodalDofs,
                                         ElementDofs, element_ndofs, info);
  ...
  if (do_partitioning) {
    ...
    if (!Inf) {
      if (ErrOut) *ErrOut << "Failed to open " << MeshName << std::endl;
      return (1);
    }
    ...
    if (do_reorient) {  // don't do this for 2d geometry (yet)
      ...
      for (Mesh::IndexType element_being_processed = 1;
        ...
        Mesh::GenericElement ge(size_of_element);
        if (ge.Inverted(econ[index], nc)) ge.ReOrient(econ[index]);
      }
    ...
    ParMETIS_V3_PartKway(&vtxdist[0], &xadj[0], &adj[0], NULL, NULL, &wgtflag,
                         &numflag, NULL, &nparts, NULL, NULL, options,
                         &edgecuts, &part[0], &comm);
#else
    ...
    if (false) {
      if (StdOut && verblevel) *StdOut << "Partition Table: ";
      IRAD::Util::DumpContents(*StdOut, part, " ");
      if (StdOut && verblevel) *StdOut << std::endl;
    }
    part.resize(0);
    part.swap(part);
    ...
    for (Mesh::IndexType iii = 0; iii < Nparts; iii++) {
      ...
      if (report_each && StdOut)
        *StdOut << "Partition " << iii + 1 << ": " << nbsize << std::endl;
    ...
    IRAD::Util::Flatten<Mesh::Connectivity, std::vector<Mesh::IndexType>,
                        std::list<Mesh::IndexType> >(bordercon, bnodes);
    ...
    IRAD::Util::MapElements<Mesh::Connectivity, Mesh::Connectivity,
                            std::vector<Mesh::IndexType>,
                            std::map<Mesh::IndexType, Mesh::IndexType> >(
    ...
    while (bni != bcon_dual.end()) {
      ...
      for (Mesh::IndexType ci = 1; ci < color; ci++) bnici++;
      //      if(bordercon[*bnici-1].size() >
      //      static_cast<Mesh::IndexType>(mean_nbnodes))
    ...
    for (Mesh::IndexType colind = 0; colind < Nparts; colind++) {
      ...
      ColorInfo[colind].nborder =
          colorneighbors[colind].size();  // number of borders
      ColorInfo[colind].nnodes =
          colorsofnodes[colind].size();                      // total num nodes
      ColorInfo[colind].nshared = bordercon[colind].size();  // num shared nodes
      ColorInfo[colind].nown =
          dual_owner[colind].size();  // num shared nodes owned
    }
    ...
    Mesh::IndexVec psize;
    IRAD::Primitive::MultiContainer<Mesh::IndexType, Mesh::IndexType>::VecMap
        pglob2loc;
    ...
    for (Mesh::IndexType jjj = 0; jjj < number_of_nodes; jjj++) {
      ...
      if (nodecolors.Esize(node_id) == 1) {  // then it's not shared
        ...
        psize[thecolor - 1]++;
        pglob2loc[thecolor - 1].insert(
            std::make_pair(node_id, psize[thecolor - 1]));
      }
    }
    ...
    IRAD::Primitive::MultiContainer<Mesh::Border>::VecVec ColorBorders;
      ...
      while (nrcvIt != dual_owner[jjj].end()) {
        ...
        while (ncIt != nodecolors[global_bn_id - 1].end()) {
          ...
          if (remote_color_id != this_color) {
            ...
            assert(ColorBorders[jjj][local_border_id - 1].rpart ==
                   remote_color_id);
            ColorBorders[jjj][local_border_id - 1].nrecv.push_back(
                global_bn_id);
            // Add this node to the border data structure for remote procs
            Mesh::IndexType remote_border_id =
                colormap[remote_color_id - 1][this_color];
            assert(
                ColorBorders[remote_color_id - 1][remote_border_id - 1].rpart ==
                this_color);
            ColorBorders[remote_color_id - 1][remote_border_id - 1]
                .nsend.push_back(global_bn_id);
          }
        }
      }
    }
    ...
    for (Mesh::IndexType jjj = 0; jjj < Nparts; jjj++) {
      ...
      for (Mesh::IndexType iii = 0; iii < ColorInfo[jjj].nborder; iii++) {
        std::vector<Mesh::IndexType>::iterator bni =
            ColorBorders[jjj][iii].nsend.begin();
    ...
    if (Inf) {
      ...
      for (unsigned int propcount = 0; propcount < number_of_elements;
           propcount++)
    ...
    for (Mesh::IndexType jjj = 0; jjj < Nparts; jjj++) {
      ...
      for (Mesh::IndexType iii = 0; iii < ColorInfo[jjj].nnodes; iii++) {
        ...
        OutFile << nc.x(node_id) << " " << nc.y(node_id) << " " << nc.z(node_id)
                << std::endl;
        if (OutProp) OutProp << nodal_props[node_id - 1] << std::endl;
      }
      if (OutProp) OutProp << std::endl;
      ...
      for (Mesh::IndexType iii = 0; iii < ColorInfo[jjj].nelem; iii++) {
        ...
        while (eni != econ[eid - 1].end()) {
          ...
          if (eni != econ[eid - 1].end()) OutFile << " ";
          eni++;
        }
        OutFile << std::endl;
        if (OutProp) OutProp << elemental_props[eid - 1] << std::endl;
      }
      if (OutProp) OutProp.close();
      // Write the borders
      ...
      for (Mesh::IndexType iii = 0; iii < ColorInfo[jjj].nborder; iii++) {
        ...
        std::vector<Mesh::IndexType>::iterator rni =
            ColorBorders[jjj][iii].nrecv.begin();
  ...
  //     std::vector<Mesh::PartInfo> ColorInfo(nparts);
  //     for(Mesh::IndexType colind = 0;colind < Nparts;colind++){
  //       ColorInfo[colind].part    = colind+1;                      //
  //       partition id ColorInfo[colind].nelem   = dual_pcon[colind].size(); //
  //       number of elems ColorInfo[colind].nborder =
  //       colorneighbors[colind].size(); // number of borders
  //       ColorInfo[colind].nnodes  = colorsofnodes[colind].size();  // total
  //       num nodes ColorInfo[colind].nshared = bordercon[colind].size(); //
  //       num shared nodes ColorInfo[colind].nown    =
  //       dual_owner[colind].size();     // num shared nodes owned
  //     }
  ...
  //     Mesh::IndexVec psize;
  //     IRAD::Primitive::MultiContainer<Mesh::IndexType,Mesh::IndexType>::VecMap
  //     pglob2loc; pglob2loc.resize(Nparts); psize.resize(Nparts,0);
  //     for(Mesh::IndexType jjj = 0;jjj < number_of_nodes;jjj++){
  //       Mesh::IndexType node_id = jjj+1;
  //       if(nodecolors.Esize(node_id) == 1){ // then it's not shared
  // 	Mesh::IndexType thecolor = nodecolors[node_id-1][0];
  ...
  // 	    Mesh::IndexType remote_border_id =
  // colormap[remote_color_id-1][this_color];
  // 	    assert(ColorBorders[remote_color_id-1][remote_border_id-1].rpart ==
  // this_color);
  ...
  //       for(Mesh::IndexType iii = 0;iii < ColorInfo[jjj].nborder;iii++){
  // 	std::vector<Mesh::IndexType>::iterator bni =
  // ColorBorders[jjj][iii].nsend.begin(); 	while(bni !=
  // ColorBorders[jjj][iii].nsend.end()) 	  sendnodes.push_back(*bni++);
  //       }
  //       sendnodes.sort();
  //       sendnodes.unique();
  //       std::list<Mesh::IndexType>::iterator sni = sendnodes.begin();
  ...
  // 		<< ColorBorders[jjj][iii].nsend.size() << std::endl;
  // 	std::vector<Mesh::IndexType>::iterator rni =
  // ColorBorders[jjj][iii].nrecv.begin(); 	while(rni !=
  // ColorBorders[jjj][iii].nrecv.end()){ 	  OutFile << pglob2loc[jjj][*rni] << " "
  // << *rni << std::endl; 	  rni++;
  // 	}
  // 	rni = ColorBorders[jjj][iii].nsend.begin();
  // 	while(rni != ColorBorders[jjj][iii].nsend.end()){
  ...
  if (do_clone) {
    ...
    if (infiles.empty()) {
      if (ErrOut) *ErrOut << "Error: Nothing to clone." << std::endl;
      comm.SetExit(1);
    }
    if (comm.Check()) return (1);
    ...
    if (N > 0) npartitions = N;
    if (StdOut && verblevel)
      *StdOut << "Cloning " << MeshName << " into " << npartitions
              << " partitions." << std::endl;
    Inf.open(MeshName.c_str());
    if (!Inf) {
      if (ErrOut) *ErrOut << "Failed to open " << MeshName << std::endl;
    ...
    if (StdOut && verblevel)
      *StdOut << "Number of internal nodes/partition: " << nnbn << std::endl
              << "Number of shared nodes/partition: " << nshared_nodes
              << std::endl;
    ...
    Mesh::IndexType node_id = 1;
    while (ni != non_boundary.end()) NodeMap[*ni++ - 1] = node_id++;
    ni = left_owned.begin();
    while (ni != left_owned.end()) NodeMap[*ni++ - 1] = node_id++;
    ni = right_owned.begin();
    while (ni != right_owned.end()) NodeMap[*ni++ - 1] = node_id++;
    ni = left_remote.begin();
    while (ni != left_remote.end()) NodeMap[*ni++ - 1] = node_id++;
    ni = right_remote.begin();
    while (ni != right_remote.end()) NodeMap[*ni++ - 1] = node_id++;
    node_id--;
    ...
    for (Mesh::IndexType jjj = 0; jjj < N; jjj++) {
      ...
      while (nbni != non_boundary.end()) {
        OutFile << nc.x(*nbni) << " " << nc.y(*nbni) << " " << nc.z(*nbni)
                << std::endl;
        nbni++;
      }
      ...
      OutFile << (jjj == (N - 1) ? 1 : jjj + 2) << " " << nown_right << " "
...
// AssembleOrderedRow(DOFType &row,DOFContType &j,DOFDataContType &dofdat,KType
// &k) {
...
// Naive, straightforward graph generation based on block-structured node id's
void FiniteDifferenceGraph(Mesh::Connectivity graphcon, int nd, int *gridsize,
                           int stencil_type, int stencil_size) {
  Mesh::IndexType number_of_nodes = 1;
  for (int i = 0; i < nd; i++) number_of_nodes *= gridsize[i];
  graphcon.resize(number_of_nodes);
}
