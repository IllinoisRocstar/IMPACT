...
int main(int argc, char *argv[]) {
  ...
  if (do_order) std::cout << "Dof mapping read enabled." << std::endl;

  std::vector<std::string> infiles;
  for (int i = 0; i < (argc - offset); i++) infiles.push_back(argv[i + offset]);
  ...
  std::cout << "Creating image of sparsity pattern for " << file_base << " on "
  ...
  while (ai != infiles.end()) {
    ...
    if (total_number_of_dofs == 0) {
      ...
      //      std::cout << "Resizing stiffness_rows to " << total_number_of_dofs
      //      << std::endl;
    }
    ...
    if (do_order) {
      ...
      while (ldtgIt != local_dof_to_global.end()) Infile >> *ldtgIt++;
    }
    Infile >> dum;
    //    std::cout << "Read extra entry of " << dum << std::endl;
    assert(dum == ndofd);
    nglobal_dof +=
        ndofd;  // nglobal_dof is a running tally for later sanity check
    ...
    for (unsigned int i = 0; i < ndofd; i++) {
      Infile >> dum;
      if (!do_order) local_dof_to_global[i] = i + doffset;
    ...
    doffset += ndofd;
    //    std::cout << "doffset = " << doffset << std::endl;
    //    std::cout << "Ap read for processor " << proc << " (min/max): (" <<
    //    min_Ap
    //	      << "/" << max_Ap << ")  Total NZ entries:" << local_nz_entries
    //	      << std::endl;
    std::cout << "Ap read for processor " << proc
              << " Total NZ entries:" << local_nz_entries << std::endl;
    // Skip the next value from the file if do_skip is set (default)
    if (do_skip) Infile >> dum;
    ...
    for (size_t i = 0; i < ndofd; i++) {
      ...
      //      std::cout << "dof row size = " << nnz[current_dof_id-1] <<
      //      std::endl;
      stiffness_rows[current_dof_id - 1].resize(nnz[current_dof_id - 1], 0);
      //      std::cout << "dof row(" << nnz[current_dof_id-1] << "): ";
      std::vector<size_t>::iterator srIt =
          stiffness_rows[current_dof_id - 1].begin();
  ...
  for (size_t ii = 1; ii < nglobal_dof; ii++) {
    ...
    if (*scIt < (ii + 1)) skyline += ((ii + 1) - *scIt);
  }
  ...
  for (size_t ii = 0; ii < nglobal_dof; ii++) {
    ...
    while (scIt != stiffness_rows[ii].end()) {
      ...
      if (!std::binary_search(srIt[column_index].begin(),
                              srIt[column_index].end(), row_id))
