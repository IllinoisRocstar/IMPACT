...
static void read_file_rocin( const char *fname, const std::string &wname, 
  ...
  for (std::vector<std::string>::iterator it = itmNameVec.begin() ; it != itmNameVec.end(); ++it) {
     ...
     COM_clone_dataitem( (wname+tmpItmName).c_str(), (bufwin+tmpItmName).c_str(), 0);
  }

  COM_delete_window( bufwin.c_str());
  Inf << "\nRank #" << myRank << " Obtained window " << wname
...
void usage(char *exec, std::ostream& Inf) {
   ...
   Inf << exec << " [--help] [--test] window_hdf_file_name1 window_hdf_file_name2"
   ...
   << std::endl
   << "Example : " << std::endl
   << " mpirun -np 2 "<< exec << " --transfer \"ELMModule_window_proc_*.hdf\""
   << " \"OFModule_window_proc_*.hdf\" \"ELMModule_in_0.txt\" \"OFModule_in_0.txt\" \n" 
   << std::endl;
...
static void findWinName(std::string fnameIn, std::string& wname) {
   ...
   std::string fname;
   if ( n0 == std::string::npos) fname=fnameIn;
   else fname = fnameIn.substr(n0+1, fnameIn.size());
...
int main(int argc, char *argv[]) {
   ...
   int RFC_verbosity = COM_get_function_handle( ("RFC" + std::string(".set_verbose")).c_str()); 
   int RFC_overlay = COM_get_function_handle( ("RFC" + std::string(".overlay")).c_str());
   int RFC_write = COM_get_function_handle( ("RFC" + std::string(".write_overlay")).c_str());
   int RFC_read = COM_get_function_handle( ("RFC" + std::string(".read_overlay")).c_str());
   int RFC_load = COM_get_function_handle( ("RFC" + std::string(".load_transfer")).c_str()); 
   int RFC_clear = COM_get_function_handle( ("RFC" + std::string(".clear_overlay")).c_str());
   int RFC_transfer = COM_get_function_handle( ("RFC" + std::string(".least_squares_transfer")).c_str()); 
   int RFC_interpolate = COM_get_function_handle( ("RFC" + std::string(".interpolate")).c_str()); 
   ...
   if (testManOverlay) { 
	 ...
	 int solid_mesh = COM_get_dataitem_handle( (tmp_wname[0]+".mesh").c_str());
	 int fluid_mesh = COM_get_dataitem_handle( (tmp_wname[1]+".mesh").c_str());
	 COM_call_function( RFC_overlay, &solid_mesh, &fluid_mesh);
	 Inf << "\nRank #" << myRank << " Computing overlay finished successfully.";
   ...
   if (testManTransfer || testObjTransfer){
      ...
      for (int iWin=0; iWin<2; iWin++) {
         ...
         COM_new_dataitem( (wnames[iWin] + "." + attLst[iWin]).c_str(), 'n', COM_DOUBLE, 3, "m/s");
         for (int iPane=0; iPane<nPane[iWin]; iPane++) {
            // using COM_resize_array: sets to zero
	    COM_resize_array( (wnames[iWin] + "." + attLst[iWin]).c_str(), 
                          paneList[iWin][iPane]);
            // setting values
	    COM_get_size( (wnames[iWin] + "." + attLst[iWin]).c_str(), paneList[iWin][iPane], &dataItemSize);
            dataVec[iWin][iPane].resize(3*dataItemSize, 1-iWin);
            COM_set_array( (wnames[iWin] + "." + attLst[iWin]).c_str(), paneList[iWin][iPane], 
                           &dataVec[iWin][iPane][0]);

	    // accessing values to  make sure registered properly
	    COM_get_array( (wnames[iWin] + "." + attLst[iWin]).c_str(), paneList[iWin][iPane], 
                           &dataAry, &stride,&cap);
	    COM_get_size( (wnames[iWin] + "." + attLst[iWin]).c_str(), paneList[iWin][iPane], &dataItemSize);
   ...
   if (testManTransfer || testObjTransfer) { 
      ...
      COM_call_function( RFC_read, &mesh0, &mesh1, &comm, 
			 wnames[0].c_str(), wnames[1].c_str(), "HDF");

      Inf << "\nRank #" << myRank << " Loading the overlay finished successfully.";
   }
   MPI_Barrier(comm);
   ...
   if (testManTransfer) {
	 Inf << "\nRank #" << myRank << " Attempting to transfer an attribute manually...";
   ...
   if (testObjOverlay) {
	 ...
	 Inf << "\nRank #" << myRank << " Performing the overlay using transfer object\n";
	 transObj = new SolverUtils::TransferObject("trans");
	 int opRes = transObj->Overlay( wnames[0].c_str(), wnames[1].c_str());
	 if (opRes)
	    Inf << "\nRank #" << myRank 
