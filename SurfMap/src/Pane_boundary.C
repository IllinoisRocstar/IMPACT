...
double Pane_boundary::min_squared_edge_len(const std::vector<Facet_ID> &be) {
  const COM::DataItem *attr = _pane.dataitem(COM::COM_NC);
...
void Pane_boundary::determine_borders(const COM::DataItem *mesh,
  ...
  for (int i = 0, n = panes.size(); i < n; ++i) {
    ...
    COM::DataItem *bdl_pane = panes[i]->dataitem(isborder->id());
