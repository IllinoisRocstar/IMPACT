...
Pane_connectivity::Pane_connectivity(const COM::DataItem *mesh, MPI_Comm c)
    : _win(mesh->window()),
      _mani2(NULL),
...
void Pane_connectivity::collect_nodes(const std::vector<std::vector<int> > &ns,
  ...
  for (int i = 0, s = _panes.size(); i < s; ++i) {
    ...
    const COM::DataItem *attr = _panes[i]->dataitem(COM::COM_NC);
  ...
  if (!for_facet) {
    ...
    if (iso_local) {  // Determine all nodes incident on isolated nodes.
      ...
      if (pnts_g.size() > 0) {
        ...
        for (int i = 0, s = _panes.size(); i < s; ++i) {
          std::vector<bool> is_co;
          determine_coisolated_nodes(*_panes[i], pnts_g, tol, is_co, &tree);
...
static void make_kd_tree(const std::vector<int> &nodes,
                         ...
                         std::vector<int> &offsets, KD_tree_3 &tree) {
  if (nodes.empty()) return;
  assert(nodes.size() == pnts.size());
  std::vector<Point_3<Real> > b;
...
double Pane_connectivity::collect_boundary_nodes(std::vector<int> &nodes,
  ...
  collect_coincident_nodes(r_nodes_pre, r_pnts_pre, bbox, offsets, local_rtree,
                           tol, nodes, pnts);
...
void Pane_connectivity::create_b2map(std::vector<std::vector<int> > &b2v,
  ...
  try {
    ...
    std::vector<Point_3> bbox;
    bbox.reserve(2 * npanes);
    std::vector<int> offsets;
    KD_tree_3 tree;
    make_kd_tree(nodes, pnts, bbox, offsets, tree);
...
void Pane_connectivity::create_b2map(COM::DataItem *pconn, bool for_facet) {
  ...
  for (int i = 0, n = b2v.size(); i != n; ++i) {
    COM::DataItem *attr;
    attr = const_cast<COM::DataItem *>(_panes[i]->dataitem(pconn->id()));
...
void Pane_connectivity::compute_pconn(COM::DataItem *pconn_n,
                                      COM::DataItem *pconn_f) {
  if (pconn_n) create_b2map(pconn_n, false);
  if (pconn_f) create_b2map(pconn_f, true);
}

void Pane_connectivity::size_of_cpanes(const COM::DataItem *pconn,
                                       const int *pane_id, int *npanes_total,
                                       int *npanes_ghost) {
  const COM::Window *win = pconn->window();
  const COM::DataItem *pconn_pn;
  pconn_pn = pconn->window()->pane(*pane_id).dataitem(pconn->id());
...
int Pane_connectivity::pconn_nblocks(const COM::DataItem *pconn) {
  const COM::Window *win = pconn->window();
  ...
  for (int p = 0, np = panes.size(); p < np; ++p) {
    const COM::DataItem *pconn_pn = panes[p]->dataitem(pconn->id());
