...
Pane_connectivity::
Pane_connectivity( const COM::DataItem *mesh, MPI_Comm c) 
...
create_b2map( std::vector< std::vector<int> > &b2v, 
  ...
  try {
	...
	std::vector< Node_ID> ids; ids.reserve( nfound);
...
void Pane_connectivity::create_b2map( COM::DataItem *pconn, 
				      bool for_facet) throw(int)
{ 
...
void Pane_connectivity::compute_pconn( COM::DataItem *pconn_n,
				       COM::DataItem *pconn_f) throw(int) { 
