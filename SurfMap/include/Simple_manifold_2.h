...
class Facet_ID {
 public:
  ...
  bool operator==(const Facet_ID e) const { return _code == e._code; }
  bool operator!=(const Facet_ID e) const { return _code != e._code; }
 ...
 public:
  ...
  Simple_manifold_2()
      : _pane(NULL),
        ...
        _with_ghost(0),
        _nspe(0),
        _maxsize_real_nodes(-1),
        _maxsize_real_elmts(-1),
        _size_real_borders(0),
        _size_ghost_borders(0),
        _size_rg_borders(0) {}
  ...
  int size_of_isolated_nodes() const { return _isovIDs.size(); }
  //\}
  ...
  Edge_ID get_prev_real_edge(const Edge_ID &eID) const {
    if (!eID.is_border())
      ...
      while (!oeID.is_border())
        oeID =
            get_opposite_real_edge_interior(get_next_real_edge_interior(oeID));
      return oeID;
    }
  }
  ...
  Edge_ID get_next_real_edge(const Edge_ID &eID) const {
    if (!eID.is_border())
      ...
      while (!oeID.is_border())
        oeID =
            get_opposite_real_edge_interior(get_prev_real_edge_interior(oeID));
      return oeID;
    }
  }
  ...
  int get_origin(Edge_ID eID, Element_node_enumerator *ene_in = NULL) const {
    if (eID.is_border()) {
      ...
      if (is_real_border_edge(eID))
        opp = get_opposite_real_edge_border(eID);
      else
        opp = get_opposite_ghost_edge_border(eID);
  ...
  int get_destination(Edge_ID eID,
                      Element_node_enumerator *ene_in = NULL) const {
    if (eID.is_border()) {
      Edge_ID opp;
      if (is_real_border_edge(eID))
        opp = get_opposite_real_edge_border(eID);
      ...
      if (ene_in)
        return (
            *ene_in)[eID.lid() + 1 == ene_in->size_of_edges() ? 0
                                                              : eID.lid() + 1];
      else {
  ...
  int upperbound_edge_index() const { return size_of_elements() * _nspe; }
  ...
  void get_borders(std::vector<bool> &is_border, std::vector<bool> &is_isolated,
                   std::vector<Edge_ID> *b, int *ng = NULL) const;
 ...
 private:
  ...
  Edge_ID get_prev_edge_interior(const Edge_ID &eID) const {
    ...
    if (_is_str || !is_ghost_element(eID.eid())) {
      ...
      while (_oeIDs_real_or_str[eindex--] == Edge_ID()) {
        --i;
        assert(i);
      }
    } else {
      int eindex = get_edge_index(Edge_ID(eID.eid(), i), _maxsize_real_elmts);
      while (_oeIDs_ghost[eindex--] == Edge_ID()) {
        --i;
        assert(i);
      }
    }
