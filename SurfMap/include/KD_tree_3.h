...
class KD_tree_3 {
public:
  KD_tree_3() : _npnts(0), _maxout(0), 
                _tree(0), _bboxes(0), _indices(0) {}
  ...
  ~KD_tree_3() {
    if (_tree) { delete [] _tree; _tree = 0; }
    if (_bboxes) { delete [] _bboxes; _bboxes = 0; }
  ...
  void build( const double *pnts, int np, int maxn=0) {
    assert(np>=0);
    _npnts = np;
    if (maxn) _maxout = maxn; else _maxout = np;
   ...
   * This function must be called after build has been called.
   */
  int search( const double pnt[3], const double tol, int **indices=0, int start=0) {
    double range[6];
