...
class Pane_connectivity {
 ...
 public:
  ...
  void compute_pconn(COM::DataItem *pconn_n, COM::DataItem *pconn_f = NULL);
  ...
  static int pconn_offset() { return _pconn_offset; }
 ...
 protected:
   ...
   */
  void create_b2map(COM::DataItem *pconn, bool for_facet);

  /** Create b2v mapping (pane connectivity) for nodes or edges and store
   *  the results into a vector of vectors (with the same format as above).
   */
  void create_b2map(std::vector<std::vector<int> > &b2, bool for_facet);
  ...
  double get_local_boundary_nodes(std::vector<int> &nodes,
                                  std::vector<Point_3> &pnts, bool for_facet);
  ...
  void determine_coisolated_nodes(const COM::Pane &pn,
                                  std::vector<Point_3> &pnts, const double tol,
                                  std::vector<bool> &is_co,
                                  KD_tree_3 *tree = NULL);

 private:
  void collect_nodes(const std::vector<std::vector<int> > &ns,
                     std::vector<int> &nodes, std::vector<Point_3> &pnts,
                     bool for_facet);
