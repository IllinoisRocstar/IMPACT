...
class IM_Reader {
 ...
 public:
  ...
  int read_mesh(const char *fname, const std::string &wname,
    ...
    if (!is.is_open()) {  // used to be: "is == NULL". Fixes error no valid
                          // operator== between std::ifstream and a long int
    ...
    switch (mesh_type) {
      case 2:  // Structured mesh
        for (int i = 0; i < num_panes; ++i) {
          ...
          if (pid <= 0) {
            std::cerr << "Error: Found nonpositive pane id " << pid
                      << std::endl;
            exit(-1);
          }
      ...
      case 5:  // Mixed mesh with 3-node triangles and 4-node quadrilaterals
        ...
        break;
      default:
        std::cerr << "Error: File: " << fname << " has unknown mesh type"
                  << mesh_type << std::endl;
        exit(-1);
    }
 ...
 private:
  ...
  void read_pane_coors(std::istream &is, const std::string &wname, int pid,
                       int n, bool local) {
    ...
    for (int i = 0; i < n; ++i) {
      get_nextline(is, buf);
      if (local) {
        std::sscanf(buf, "%lf %lf %lf", &coors[i][0], &coors[i][1],
                    &coors[i][2]);
  ...
  void read_pane_elems(std::istream &is, const std::string &wname, int pid,
    ...
    if (local) {
      ...
      COM_allocate_array((wname + types[nodes_per_elem - 3]).c_str(), pid,
                         &(void *&)elems);
    }
    ...
    for (int i = 0; i < num_elems; ++i) {
      ...
      if (local) {
        ...
        switch (nodes_per_elem) {
          case 3:
            std::sscanf(buf, "%d %d %d", p, p + 1, p + 2);
            break;
          case 4:
            std::sscanf(buf, "%d %d %d %d", p, p + 1, p + 2, p + 3);
            break;
          case 6:
            std::sscanf(buf, "%d %d %d %d %d %d", p, p + 1, p + 2, p + 3, p + 4,
                        p + 5);
            break;
          default:
            assert(false);
  ...
  void read_pane_ij(std::istream &is, const std::string &wname, int pid,
                    bool local) {
    get_nextline(is, buf);
    int dims[2];
  ...
  void read_pane_uns(std::istream &is, const std::string &wname, int pid,
                     int nodes_per_elem, bool local) {
  ...
  void read_pane_mixed(std::istream &is, const std::string &wname, int pid,
                       bool local) {
