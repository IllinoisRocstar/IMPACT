...
class Generic_element_2 {
 ...
 public:
  ...
  template <class Field, class Value>
  void interpolate(const Field &f, const Real xi, Value *v) const;
  ...
  template <class Field>
  typename Field_traits<Field>::Value interpolate(const Field &f,
                                                  const Nat_coor &nc) const {
    typename Field_traits<Field>::Value v;
    this->interpolate(
        f, nc,
        &v);  // Use this-> to get around a stupid bug in SGI C++ compiler
    return v;
  }

  /// Interpolates the field data at a given point on an edge and return the
  /// result.
  template <class Field>
  typename Field_traits<Field>::Value interpolate(const Field &f,
                                                  const Real xi) const {
  ...
  template <class Field>
  typename Field_traits<Field>::Value interpolate_to_center(
      const Field &f) const {
    typename Field_traits<Field>::Value v;
  ...
  template <class Field>
  Real Jacobian_det(const Field &f1, const Field &f2, Real alpha,
                    const Nat_coor &nc2) const;
  //\}
 ...
 protected:
  ...
  /// Nonoptimized version of interpolation.
  template <class Field, class Value>
  void interpolate_nopt(const Field &f, const Real xi, Value *v) const;
...
template <class Field, class Value>
void Generic_element_2::interpolate(const Field &f, const Nat_coor &nc,
                                    Value *v) const {
...
template <class Field, class Value>
void Generic_element_2::interpolate_nopt(const Field &f, const Nat_coor &nc,
                                         Value *v) const {
  const Real xi = nc[0], eta = nc[1];
  ...
  switch (_nnodes) {
    ...
    case 6: {
      ...
      *v += ((f[1] - f[0]) * xi * (2. * xi - 1.)) +
            ((f[3] - f[0]) * 4. * xi * zeta) +
            ((f[2] - f[0]) * eta * (2. * eta - 1.)) +
            ((f[5] - f[0]) * 4. * eta * zeta) + ((f[4] - f[0]) * 4. * xi * eta);
      return;
    }
    case 8: {
      const Real xi_minus = 1. - xi;
      ...
      *v +=
          ((f[1] - f[0]) * eta * (2. * eta - 1.)) + ((f[3] - f[0]) * eta) +
          ((f[2] - f[3]) * xi * eta) -
...
template <class Field, class Value>
void Generic_element_2::interpolate(const Field &f, const Real xi,
                                    Value *v) const {
  *v = f[0];
  switch (_nnodes) {
    case 3:
...
template <class Field, class Value>
void Generic_element_2::interpolate_nopt(const Field &f, const Real xi,
                                         Value *v) const {
  *v = f[0];
  switch (_nnodes) {
    case 3:
...
template <class Field, class Value>
void Generic_element_2::interpolate_to_center(const Field &f, Value *v) const {
  if (_order == 1) {
    *v = f[0];
...
template <class Field>
void Generic_element_2::Jacobian(const Field &f, const Nat_coor &nc,
                                 Vector_3 J[2]) const {
  switch (_nnodes) {
    case 3:
      J[0] = f[1] - f[0];
    ...
    case 6: {
      ...
      J[0] = ((f[1] - f[0]) *= 4. * xi - 1.) +=
          ((f[3] - f[0]) *= 4. * zeta - 4 * xi) += ((f[4] - f[5]) *= 4. * eta);
      // J[1] = (4.*eta -1.)*(f[2]-f[0]) +
      //   (4.*zeta-4.*eta)*(f[5]-f[0]) + 4.*xi*(f[4] -f[3]);
      J[1] = ((f[2] - f[0]) *= 4. * eta - 1.) +=
          ((f[5] - f[0]) *= 4 * zeta - 4 * eta) += ((f[4] - f[3]) *= 4. * xi);
      return;
    }
    case 8: {
      const Real xi = nc[0], xi_minus = 1. - xi;
      ...
      J[0] = ((f[1] - f[0]) *= eta_minus) += ((f[2] - f[3]) *= eta) -=
          ((((f[0] - f[4]) += (f[1] - f[4])) *=
           ...
           (((f[1] - f[5]) += (f[2] - f[5]) -=
             ((f[0] - f[7]) += (f[3] - f[7]))) *= 2. * eta * eta_minus));
...
// This function evaluates the Jacobian at a given point.
template <class Field>
void Generic_element_2::Jacobian(const Field &f, const Real xi,
                                 Vector_3 &v) const {
  switch (_nnodes) {
    case 3:
    case 4:
    ...
    case 6:
      ...
      v = (((f[1] - f[0]) *= 4. * xi - 1.) += ((f[3] - f[0]) *= 4. - 8. * xi));
      return;
    case 8:
      // return (f[1]-f[0]) - (2.-4.*xi)*( (f[0]-f[4])+(f[1]-f[4]));
      v = ((f[1] - f[0]) -= (((f[0] - f[4]) += (f[1] - f[4])) *= 2. - 4 * xi));
      return;
    default:
      assert(false);
      v = Vector_3(0, 0, 0);
...
template <class Pnts>
void Generic_element_2::gradients(const Pnts &pnts, const Nat_coor &nc,
...
template <class Field>
Generic_element_2::Real Generic_element_2::Jacobian_det(
    const Field &f, const Nat_coor &nc) const {
  Vector_3 J[2];
  Jacobian(f, nc, J);
...
template <class Field>
Generic_element_2::Real Generic_element_2::Jacobian_det(
    const Field &f1, const Field &f2, const Real alpha,
    const Nat_coor &nc) const {
  Vector_3 J1[2], J2[2];
  if (alpha != 1.) {
    Jacobian(f1, nc, J1);
