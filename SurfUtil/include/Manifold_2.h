...
class Pane_manifold_2 : public Simple_manifold_2 {
 ...
 public:
  ...
  inline const Window_manifold_2 *window_manifold() const { return _wm; }
  inline Window_manifold_2 *window_manifold() { return _wm; }
  ...
  inline Halfedge get_opposite_edge(Edge_ID,
                                    Access_Mode mode = ACROSS_PANE) const;

  /// Obtain the previous edge in a given access mode
  //  Halfedge get_prev_edge( Edge_ID, Access_Mode mode) const;
  Halfedge get_prev_edge(Edge_ID, Access_Mode mode = ACROSS_PANE) const;

  /// Obtain the next edge in a given access mode
  //  Halfedge get_next_edge( Edge_ID, Access_Mode mode) const;
  Halfedge get_next_edge(Edge_ID, Access_Mode mode = ACROSS_PANE) const;
  ...
  int size_of_nodes(Access_Mode mode = ACROSS_PANE) const;
  ...
  int size_of_edges(Access_Mode mode = ACROSS_PANE) const;

  /// Obtain the number of faces.
  //  int size_of_faces( Access_Mode mode) const {
  int size_of_faces(Access_Mode mode = ACROSS_PANE) const {
    switch (mode) {
      case REAL_PANE:
      case ACROSS_PANE:
        return size_of_real_faces();
      default:
        return Simple_manifold_2::size_of_faces();
    }
  }

  /// Obtain the number of triangles.
  //  int size_of_triangles( Access_Mode mode) const {
  int size_of_triangles(Access_Mode mode = ACROSS_PANE) const {
    switch (mode) {
      case REAL_PANE:
      case ACROSS_PANE:
        return size_of_real_triangles();
      default:
        return Simple_manifold_2::size_of_triangles();
    }
  }

  /// Obtain the number of quadrilaterals.
  //  int size_of_quadrilaterals( Access_Mode mode) const {
  int size_of_quadrilaterals(Access_Mode mode = ACROSS_PANE) const {
    switch (mode) {
      case REAL_PANE:
      case ACROSS_PANE:
        return size_of_real_quadrilaterals();
      default:
        return Simple_manifold_2::size_of_quadrilaterals();
    }
  }
 ...
 protected:
  ...
  /// for a pane dataitem.
  void convert_pconn_edge2ghost(const COM::DataItem *pconn_e,
                                COM::DataItem *pconn_g);
  ...
  int get_bnode_index(int vID) const {
    ...
    int bid = eID.eid() - 1;
    if (bid >= int(_beIDs.size()))
      return bid - _size_ghost_borders;
    else
      return bid;
  }
...
class Window_manifold_2 {
 public:
  ...
  explicit Window_manifold_2(const COM::DataItem *pmesh)
      : _buf_window(NULL), _cc(NULL), _pconn_nb(0) {
    init(pmesh);
  }

  //  ~Window_manifold_2();
  virtual ~Window_manifold_2();
  ...
  Pane_manifold_2 *get_pane_manifold(int pid) { return _pms[_pi_map[pid]]; }
  ...
  /// Perform reduction over a given nodal dataitems.
  void reduce_on_shared_nodes(COM::DataItem *attr, MPI_Op op);
   ...
   *  dataitem normals. For nodal normals, use one of the following
   *  weighting schemes:
   ...
   */
  void compute_normals(COM::DataItem *normals, int scheme = E2N_ANGLE,
                       bool to_normalize = true);

  /** Update the normals for border faces. */
  void update_bd_normals(const COM::DataItem *normals,
                         bool to_normalize = false);

  /** Update the flags for border faces. */
  void update_bd_flags(const COM::DataItem *flags);

  /** Update bitmap for border edges. */
  void update_bdedge_bitmap(const COM::DataItem *bitmap);

  /** Accumulate the values for border faces */
  void accumulate_bd_values(const COM::DataItem *vals);

  // Compute mean-curvature normals and their Laplace-Beltrami operator.
  void compute_mcn(COM::DataItem *mcn_in, COM::DataItem *lbmcn_in);
   ...
   */
  void elements_to_nodes(const COM::DataItem *evals, COM::DataItem *nvals,
                         const int scheme = E2N_ONE,
                         const COM::DataItem *ews = NULL,
                         COM::DataItem *nws = NULL, const int tosum = false);

  /// Obtain shortest edge length of incident edges of each node or element.
  void shortest_edge_length(COM::DataItem *lens);
 ...
 protected:
  ...
  void init(const COM::DataItem *pmesh);
  virtual Pane_manifold_2 *init_pane_manifold(int pane_id) {
    return new Pane_manifold_2();
  }

  /// Determine the counterparts of pane-border edges for across-pane access.
  void determine_counterparts(const COM::DataItem *pconn_e);

  /// Determine the primary nodes for across-pane access.
  void determine_primaries(const COM::DataItem *pconn_n);

  /// Obtain shortest edge length of incident edges of each element.
  static void compute_shortest_edgelen_elements(COM::DataItem *lens);

  /// Obtain shortest edge length of incident edges of each nodes.
  void compute_shortest_edgelen_nodes(COM::DataItem *lens);
   ...
   */
  void compute_nodal_normals(COM::DataItem *nrm, int scheme = E2N_ANGLE,
                             bool to_normalize = true,
                             const COM::DataItem *weights = NULL);
   ...
   */
 public:
  static MPI_Op OP_MAX, OP_MIN, OP_SUM, OP_PROD, OP_MAXABS, OP_DIFF, OP_BOR,
      OP_BAND, OP_LOR, OP_LAND;
 ...
 protected:
  ...
  // one must inherit a user dataitem onto this window.
...
class Node {
 public:
  ...
  Node(const Pane_manifold_2 *pm, int vid, Access_Mode mode = ACROSS_PANE)
  ...
  /// Obtain the address of an dataitem associated with the node.
  /// Note: This function can be slow O(log(#panes)) if the
  /// dataitem is not on the current pane.
  const void *addr(const COM::DataItem *attr) const;

  /// Obtain the dataitem on the parent pane of the node. It takes
  /// O(log(#panes)) time.
  const COM::DataItem *attr(const COM::DataItem *a) const {
    COM_assertion_msg(a, "Unexpected NULL pointer");
    return a->window()->pane(_pm->pane()->id()).dataitem(a->id());
  }
...
class Halfedge {
 public:
  ...
  Halfedge(const Pane_manifold_2 *pm, Edge_ID e, Access_Mode mode = ACROSS_PANE)
      : _pm(pm), _eID(e), _mode(mode) {}

  /// Get the ID of the opposite edge of a given edge
  Halfedge opposite() const { return _pm->get_opposite_edge(_eID, _mode); }
  ...
  /// Obtain the (primary copy of the) origin of the edge
  Node origin() const { return _pm->get_primary(_pm->get_origin(_eID), _mode); }
  ...
  bool is_border() const { return _pm->is_border_edge(_eID, _mode); }

  /// Is the edge on the physical boundary?
  bool is_physical_border() const { return _pm->is_physical_border_edge(_eID); }
  ...
  Vector_3<Real> deformed_normal(const COM::DataItem *disp) const;

  /// Get the edge center of the given halfedge.
  void edge_center(Point_3<Real> &mid) const {
    edge_average(_pm->pane()->dataitem(COM::COM_NC),
  ...
  void face_center(Point_3<Real> &cnt) const {
    face_average(_pm->pane()->dataitem(COM::COM_NC),
  ...
  void edge_average(const COM::DataItem *attr, Value &val) const {
  ...
  template <class Value>
  void face_average(const COM::DataItem *attr, Value &val) const {
    // If this is a border halfedge, then return its edge center.
    if (is_border()) {
      edge_average(attr, val);
      return;
    }
  ...
  /// Obtain the address of an dataitem associated with its bounded element.
  /// Note: This function can be slow O(1) if the dataitem is not on the
  /// current pane, and is even slower (O(log(#panes)) if not owned by the
  /// owner window of the pane.
  const void *addr(const COM::DataItem *attr) const;

  /// Obtain the dataitem on the parent pane of the node.
  const COM::DataItem *attr(const COM::DataItem *a) const {
    COM_assertion_msg(a, "Unexpected NULL pointer");
    return a->window()->pane(_pm->pane()->id()).dataitem(a->id());
  }
...
class Face {
 public:
  ...
  Face(const Pane_manifold_2 *pm, int fid, Access_Mode mode = ACROSS_PANE)
      : _pm(pm), _fID(fid), _mode(mode) {}

  /// Get the face center of a given face.
  void face_center(Point_3<Real> &cnt) const {
    face_average(_pm->pane()->dataitem(COM::COM_NC),
                 reinterpret_cast<Vector_3<Real> &>(cnt));
  }

  /// Get the average value of the nodes of a given face.
  template <class Value>
  void face_average(const COM::DataItem *attr, Value &val) const {
    // Otherwise, compute the center
    val = Value(0);
    ...
    int count = 0;
    Halfedge h = Halfedge(_pm, Edge_ID(_fID, 0), _mode), h0 = h;
  ...
  Halfedge halfedge() const { return Halfedge(_pm, Edge_ID(_fID, 0), _mode); }

  /// Obtain the address of an dataitem associated with the element.
  const void *addr(const COM::DataItem *attr) const;

  /// Obtain the dataitem on the parent pane.
  const COM::DataItem *attr(const COM::DataItem *a) const {
    COM_assertion_msg(a, "Unexpected NULL pointer");
    return a->window()->pane(_pm->pane()->id()).dataitem(a->id());
  }

  /// Are two faces the same?
  bool operator==(const Face &f) const {
    return _pm == f._pm && _fID == f._fID;
  }

  /// Are two faces different?
  bool operator!=(const Face &f) const {
    return _pm != f._pm || _fID != f._fID;
  }
...
const Vector_3<Real> &Pane_manifold_2::get_bd_normal(Edge_ID eID) const {
  COM_assertion_msg(eID.is_border() && !_bd_nrms.empty(),
                    "get_bd_normal must be called on border edges and be "
                    "called after update_bd_normals() has been called.");
  return _bd_nrms[eID.eid() - 1];
}

int Pane_manifold_2::get_bd_flag(Edge_ID eID) const {
  COM_assertion_msg(eID.is_border() && !_bd_flgs.empty(),
                    "get_bd_flag must be called on border edges and be called "
                    "after update_bd_flags() has been called.");
...
bool Pane_manifold_2::get_bdedge_flag(Edge_ID eID) const {
  COM_assertion_msg(eID.is_border() && !_bd_bm.empty(),
                    "get_bdedge_flag must be called on border edges and be "
                    "called after update_bdedge_bitmap() has been called.");
  return _bd_bm[eID.eid() - 1];
}

// Obtain the opposite edge in a given access mode
Halfedge Pane_manifold_2::get_opposite_edge(Edge_ID eID,
  ...
  switch (mode) {
    case REAL_PANE:
      ...
      break;
    case WHOLE_PANE:
      opp = Simple_manifold_2::get_opposite_edge(eID);
      break;
    case ACROSS_PANE: {
      opp = get_opposite_real_edge(eID);
...
Halfedge Node::halfedge() const {
  ...
  switch (_mode) {
    case REAL_PANE:
      ...
      break;
    case WHOLE_PANE:
      eID = _pm->get_incident_edge(_vID);
      break;
    default: {
      eID = _pm->get_incident_real_edge(_vID);
      ...
      if (eID.is_border()) {
        ...
        //      if ( h.pane_manifold()) return h;
        Halfedge h = _pm->get_counterpart(eID), h0 = h;
        if (h.pane_manifold() && !h.id().is_border()) {
          do {
            if (h.id().is_border()) break;
          } while ((h = h.prev().opposite()) != h0);
          return h;
        }
      }
    }
...
/// Obtain the (primary copy of the) origin of the edge
Face Halfedge::face() const { return Face(_pm, _eID.eid(), _mode); }
...
/// Get the tangent of the given halfedge.
inline Vector_3<Real> get_tangent(const Halfedge &h) { return h.tangent(); }
