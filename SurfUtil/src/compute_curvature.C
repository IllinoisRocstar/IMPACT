...
compute_mcn( COM::DataItem *mcn_in, COM::DataItem *lbmcn_in) {
  ...
  areas = _buf_window->new_dataitem( "areas__MCNTEMP", 'n',
				      COM_DOUBLE, 1, "");
  _buf_window->resize_array( areas, NULL);

  weights = _buf_window->new_dataitem( "weights__MCNTEMP", 'e',
					COM_DOUBLE, 12, "");
  ...
  for (int i=0, local_npanes = panes.size(); i<local_npanes; ++i, ++it){ 
    ...
    Vector_3<Real> *mcn_ptr = (Vector_3<Real> *)
      (pane.dataitem( mcn->id())->pointer());
    Vector_3<Real> *ws_ptr = (Vector_3<Real> *)
      (pane.dataitem( weights->id())->pointer());
  ...
  for (int i=0, local_npanes = panes.size(); i<local_npanes; ++i, ++it) {
    ...
    Vector_3<Real> *lbmcn_ptr = (Vector_3<Real> *)
      (pane.dataitem( lbmcn->id())->pointer());
