...
Pane_manifold_2::Pane_manifold_2(const COM::Pane *p,
    ...
    : Simple_manifold_2(p, parent), _wm(NULL) {
  if (parent) {
...
void Window_manifold_2::init(const COM::DataItem *pmesh) {
  COM_assertion_msg(
  ...
  _buf_window->inherit(const_cast<COM::DataItem *>(pmesh), "", false, true,
                       NULL, 0);

  // If pconn is not given, allocate pconn by cloning the pconn from user space
  COM::DataItem *pconn_user =
      const_cast<COM::DataItem *>(w->dataitem(COM::COM_PCONN));
  ...
  COM::DataItem *pconn_n =
      nopconn ? _buf_window->inherit(pconn_user, "", true, true, NULL, 0)
              : _buf_window->dataitem(COM::COM_PCONN);

  // Create an dataitem for edge connectivity across panes
  COM::DataItem *pconn_e =
      _buf_window->new_dataitem("pconn_e", 'p', COM_INT, 1, "");
  ...
  for (int i = panes.size() - 1; i >= 0; --i) {
    ...
    //    _pms[i].init( panes[i]);
    //    mani2[i] = &_pms[i];
    _pms[i] = init_pane_manifold(panes[i]->id());
    _pms[i]->init(panes[i]);
    _pms[i]->_wm = this;
  ...
  if (!_pms.empty()) {
    const Simple_manifold_2 **mani = (const Simple_manifold_2 **)&_pms[0];
    MAP::Pane_connectivity pc(mani, w->get_communicator());
    pc.compute_pconn(nopconn ? pconn_n : (COM::DataItem *)NULL, pconn_e);
...
void Window_manifold_2::determine_counterparts(const COM::DataItem *pconn_e) {
  ...
  for (int i = 0, n = _pms.size(); i < n; ++i) {
    ...
    Pane_manifold_2 &pm = *_pms[i];
    const COM::DataItem *pconn_l = pm.pane()->dataitem(pconn_e->id());
...
void Window_manifold_2::determine_primaries(const COM::DataItem *pconn_n) {
  ...
  for (int i = 0, n = _pms.size(); i < n; ++i) {
    //    const COM::DataItem *pconn_l =
    //    _pms[i].pane()->dataitem(pconn_n->id());
...
void Window_manifold_2::shortest_edge_length(COM::DataItem *lens) {
  COM_assertion_msg(lens && (lens->is_nodal() || lens->is_elemental()),
                    "Argument must be nodal or elemental dataitem");
...
void Window_manifold_2::compute_shortest_edgelen_nodes(COM::DataItem *lens) {
...
/// Obtain shortest edge length of incident edges of each element.
void Window_manifold_2::compute_shortest_edgelen_elements(COM::DataItem *lens) {
  std::vector<COM::Pane *> panes;
  lens->window()->panes(panes);
  std::vector<COM::Pane *>::const_iterator it = panes.begin();
...
void Window_manifold_2::update_bd_normals(const COM::DataItem *normals,
                                          bool to_normalize) {
  ...
  if (normals->window() != _buf_window)
    normals_inherited =
        _buf_window->inherit(const_cast<COM::DataItem *>(normals),
                             "facenormals__CNNTEMP", false, true, NULL, 0);
  COM::DataItem *nrm =
      _buf_window->new_dataitem("normals__PMTEMP", 'p', COM_DOUBLE, 3, "");
  COM::DataItem *pconn_g =
      _buf_window->new_dataitem("pconn__PMTEMP", 'p', COM_INT, 1, "");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    ...
    for (int i = 0; i < n; ++i) {
      //      Edge_ID eid=it->get_opposite_real_edge(Edge_ID(i+1,
      //      Edge_ID::BndID));
      Edge_ID eid =
          (*it)->get_opposite_real_edge(Edge_ID(i + 1, Edge_ID::BndID));
...
void Window_manifold_2::update_bd_flags(const COM::DataItem *flags) {
  ...
  if (flags->window() != _buf_window) {
    flags_inherited =
        _buf_window->inherit(const_cast<COM::DataItem *>(flags),
                             "faceflags__CNNTEMP", false, true, NULL, 0);
  }
  COM::DataItem *flg =
      _buf_window->new_dataitem("flags__PMTEMP", 'p', COM_INT, 1, "");
  COM::DataItem *pconn_g =
      _buf_window->new_dataitem("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::DataItem *pconn_e = _buf_window->dataitem("pconn_e");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    ...
    COM::Pane *pn = const_cast<COM::Pane *>((*it)->pane());
    const int *flgs_face =
        reinterpret_cast<int *>(pn->dataitem(flags_inherited->id())->pointer());
...
void Window_manifold_2::update_bdedge_bitmap(const COM::DataItem *bitmap) {
  ...
  if (bm_inherited->window() != _buf_window)
    bm_inherited =
        _buf_window->inherit(const_cast<COM::DataItem *>(bitmap),
                             "facebm__CNNTEMP", false, true, NULL, 0);
  COM::DataItem *bm =
      _buf_window->new_dataitem("bm__PMTEMP", 'p', COM_CHAR, 1, "");
  COM::DataItem *pconn_g =
      _buf_window->new_dataitem("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::DataItem *pconn_e = _buf_window->dataitem("pconn_e");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    ...
    COM::Pane *pn = const_cast<COM::Pane *>((*it)->pane());
    const char *bms_face =
        reinterpret_cast<char *>(pn->dataitem(bm_inherited->id())->pointer());
...
void Window_manifold_2::accumulate_bd_values(const COM::DataItem *vals) {
  COM_assertion(vals->size_of_components() == 1);
  const COM::DataItem *vals_inherited = vals;
  // Allocate buffer arrays
  if (vals->window() != _buf_window)
    vals_inherited =
        _buf_window->inherit(const_cast<COM::DataItem *>(vals), "vals__CNNTEMP",
                             false, true, NULL, 0);
  COM::DataItem *buf =
      _buf_window->new_dataitem("buf__PMTEMP", 'p', vals->data_type(), 1, "");
  COM::DataItem *pconn_g =
      _buf_window->new_dataitem("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::DataItem *pconn_e = _buf_window->dataitem("pconn_e");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    //    COM::Pane *pn = const_cast<COM::Pane*>(it->pane());
    COM::Pane *pn = const_cast<COM::Pane *>((*it)->pane());
    const char *vals_face =
        reinterpret_cast<char *>(pn->dataitem(vals_inherited->id())->pointer());
...
void Pane_manifold_2::convert_pconn_edge2ghost(const COM::DataItem *pconn_e,
                                               COM::DataItem *pconn_g) {
...
void Window_manifold_2::compute_nodal_normals(COM::DataItem *nrms, int scheme,
                                              bool to_normalize,
  ...
  COM::DataItem *elem_normals = _buf_window->new_dataitem(
...
void Window_manifold_2::elements_to_nodes(
    const COM::DataItem *e_vals, COM::DataItem *n_vals, const int scheme,
    const COM::DataItem *e_weights, COM::DataItem *n_weights, const int tosum) {
  ...
  if (e_vals->window() != _buf_window)
    elem_vals =
        _buf_window->inherit(const_cast<COM::DataItem *>(e_vals),
  ...
  if (e_weights && e_weights->window() != _buf_window)
    elem_weights =
        _buf_window->inherit(const_cast<COM::DataItem *>(e_weights),
  ...
  for (int i = 0; i < local_npanes; ++i, ++it) {  // Loop through the panes
    ...
    const Point_3<Real> *pnts =
        reinterpret_cast<const Point_3<Real> *>(pane.coordinates());
    const COM::DataItem *elem_vals_pane = pane.dataitem(elem_vals->id());
  ...
  if (elem_vals != e_vals) _buf_window->delete_dataitem(elem_vals->name());
  if (nodal_vals != n_vals) _buf_window->delete_dataitem(nodal_vals->name());
  _buf_window->init_done(false);
}

void Window_manifold_2::compute_normals(COM::DataItem *normal, int scheme,
                                        bool to_normalize) {
...
void Window_manifold_2::perturb_mesh(double range) {
  // Allocate buffer arrays
  COM::DataItem *nrm =
      _buf_window->new_dataitem("normals__PMTEMP", 'n', COM_DOUBLE, 3, "");
  _buf_window->resize_array(nrm, NULL);

  COM::DataItem *rnd =
      _buf_window->new_dataitem("randoms__PMTEMP", 'n', COM_DOUBLE, 1, "");
  _buf_window->resize_array(rnd, NULL);

  COM::DataItem *lens =
      _buf_window->new_dataitem("lengths__PMTEMP", 'n', COM_DOUBLE, 1, "");
  ...
  // Deallocate buffers
  _buf_window->delete_dataitem(lens->name());
  _buf_window->delete_dataitem(rnd->name());
  _buf_window->delete_dataitem(nrm->name());
  _buf_window->init_done(false);
}

void Window_manifold_2::reduce_on_shared_nodes(COM::DataItem *attr, MPI_Op op) {
  COM_assertion(attr->is_nodal());

  COM::DataItem *attr_inherited = attr;
  if (attr->window() != _buf_window) {
    // If dataitem is not on buffer-window, inherit it.
    attr_inherited =
  ...
  if (attr_inherited != attr) {
    _buf_window->delete_dataitem(attr_inherited->name());
    _buf_window->init_done(false);
  }
}
...
const void *Node::addr(const COM::DataItem *a) const {
  COM_assertion_msg(a, "Unexpected NULL pointer");
  COM_assertion_msg(a->is_nodal(), "Expect a nodal dataitem");
  ...
  if (a->pane()->id() == pid)
    ...
    attr = pn->dataitem(a->id());
    COM_assertion_msg(attr, "DataItem does not exist");
  }
...
const void *Halfedge::addr(const COM::DataItem *a) const {
  COM_assertion(!is_border());
  COM_assertion_msg(a, "Unexpected NULL pointer");
  COM_assertion_msg(a->is_elemental(), "Expect a element-centered dataitem");
...
Vector_3<Real> get_deformed_normal(const Halfedge &h, const Vector_2<Real> &nc,
  ...
  COM_assertion_msg(disp->is_nodal(), "disp must be nodal dataitem");
...
Vector_3<Real> Halfedge::deformed_normal(const COM::DataItem *disp) const {
