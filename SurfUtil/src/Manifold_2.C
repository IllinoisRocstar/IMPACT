...
shortest_edge_length( COM::DataItem *lens) {
...
void Window_manifold_2::
compute_shortest_edgelen_nodes( COM::DataItem *lens) {
...
convert_pconn_edge2ghost( const COM::DataItem *pconn_e, 
...
void Window_manifold_2::compute_normals( COM::DataItem *normal,
					 int scheme,
					 bool to_normalize) {
...
void Window_manifold_2::perturb_mesh( double range) {
  ...
  COM::DataItem *rnd=_buf_window->new_dataitem( "randoms__PMTEMP", 
						  'n', COM_DOUBLE, 1, "");
  _buf_window->resize_array( rnd, NULL);

  COM::DataItem *lens=_buf_window->new_dataitem( "lengths__PMTEMP", 
						   'n', COM_DOUBLE, 1, "");
...
void Window_manifold_2::
reduce_on_shared_nodes( COM::DataItem *attr, MPI_Op op) {
...
deformed_normal( const COM::DataItem *disp) const {
