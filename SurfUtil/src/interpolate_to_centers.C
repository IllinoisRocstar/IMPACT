...
void Rocsurf::interpolate_to_centers( const COM::DataItem *x, 
				     COM::DataItem *z) {
  ...
  for( zit = zpanes.begin(), zend = zpanes.end(), xit = xpanes.begin();
    ...
    for(int i = 0; i < ndim; ++i) {
      COM::DataItem *z_pa = (*zit)->dataitem(z->id()+((ndim>1)?i+1:0));
      Real *zval = reinterpret_cast<Real *>(z_pa->pointer());
      int zstrd=z_pa->stride();

      const COM::DataItem *x_pa = (*xit)->dataitem(x->id()+((ndim>1)?i+1:0));
